...
1.  **Pre-Git validation:** Run initial validation checks before Git operations begin (during architectural stage from Task 002 Analysis Stage)
2.  **Post-Git validation:** Execute validation immediately after any rebase or merge operation completes successfully or after conflicts are resolved (during Git stage from Task 003 Clustering Stage), to catch merge artifacts, garbled text, and missing imports using Task 005 scripts
3.  **Semantic validation:** Execute comprehensive validation as the final stage (Task 004 Integration Stage), including pre-merge validation scripts (Task 003) and comprehensive merge validation framework (Task 008)
