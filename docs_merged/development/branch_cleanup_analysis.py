...
def get_branch_details(branch: str) -> Dict:
    ...
    try:
        ...
        if any(pattern in name_lower for pattern in ['backup', 'archive', 'old', 'deprecated']):
            info['is_backup'] = True

        # Check for duplicate patterns
        if any(pattern in name_lower for pattern in ['copy', 'duplicate', 'temp', 'test-', '-test']):
...
def analyze_deletable_branches():
    ...
    def print_branch_section(title, branches, color_code):
        ...
        for branch in branches[:10]:  # Show top 10
            ...
            print(f"  {branch['name']}")
            print(f"    Status: {status} | Age: {age} | Commits: {commits} | Safety: {score}%")
    ...
    for branch in safe_to_delete[:5]:  # Show first 5 as examples
        print(f"# git branch -D {branch['name']}  # {branch['deletion_reason'][:50]}...")
