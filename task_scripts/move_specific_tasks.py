...
def move_specific_and_completed_tasks(original_file_path, completed_tasks_file, specific_task_ids):
    ...
    backup_path = f"{original_file_path}.backup_specific_move_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    return {
        ...
        "completed_tasks_found": [t for t in tasks_to_move if t.get('status') == 'done' and t['id'] not in specific_task_ids],
...
def main():
    ...
    # Move tasks
    stats = move_specific_and_completed_tasks(original_tasks_path, completed_tasks_path, specific_task_ids)
    
    print(f"Specific move completed!")
    print(f"  - Specific tasks requested ({specific_task_ids}): {[t['id'] for t in stats['specific_tasks_found']]}")
