...
def finalize_task_move():
    ...
    with open(main_tasks_path, encoding="utf-8") as f:
        data = json.load(f)
    ...
    print(
        f"  - Completed tasks remaining in main file: {len([t for t in non_completed_tasks if t.get('status') == 'done'])}"
    )
    ...
    return {
        ...
        "completed_in_main_still": len(
            [t for t in non_completed_tasks if t.get("status") == "done"]
        ),
    }
