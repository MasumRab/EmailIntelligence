...
from taskmaster_common import FileValidator, SecurityValidator
        ...
        if "subtasks" in task:
            ...
            for subtask in subtasks:
                ...
                if "status" not in subtask:
                    ...
                    warnings.append(
                        f"Subtask {subtask_full_id} has invalid status: {subtask['status']}"
                    )
                            ...
                            if "." in dep:
                                ...
                                if len(parts) == 2:
                                    ...
                                    if parent_id == str(task_id):
                                        ...
                                        errors.append(
                                            f"Subtask {subtask_full_id} has invalid dependency: {dep}"
                                        )
                                else:
                                    errors.append(
                                        f"Subtask {subtask_full_id} has invalid dependency format: {dep}"
                                    )
                            else:
                                errors.append(
                                    f"Subtask {subtask_full_id} has invalid dependency: {dep}"
