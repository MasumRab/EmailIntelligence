...
class DataCollectionStrategy:
    ...
    def annotate_email(
        ...
        if external_analysis_results:
            ...
            )  # Assuming entities are not part of NLPEngine's typical output or need separate extraction.
            confidence = external_analysis_results.get(
                "confidence", 0.9
    ...
    def _predict_topic(self, text: str, is_fallback: bool = False) -> str:
        ...
        result = (
            ...
            if any(s > 0 for s in topic_scores.values())
            else "other"
        )
    ...
    def _predict_sentiment(self, text: str, is_fallback: bool = False) -> str:
        ...
        if sentiment_scores.get("positive", 0) > 0 and sentiment_scores[
            ...
            result = (
                ...
                if any(s > 0 for s in sentiment_scores.values())
                else "neutral"
            )
    ...
    def _predict_intent(self, text: str, is_fallback: bool = False) -> str:
        ...
        result = (
            ...
            if any(s > 0 for s in intent_scores.values())
            else "information"
        )
    ...
    def _predict_urgency(self, text: str, is_fallback: bool = False) -> str:
        ...
        result = (
            ...
            if any(s > 0 for s in urgency_scores.values())
            else "low"
        )
