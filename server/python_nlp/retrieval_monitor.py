...
class RetrievalMonitor:
    ...
    def _check_performance_alerts(self, metrics: RetrievalMetrics):
        ...
        if len(recent_metrics) >= 5:
            error_rate = sum(1 for m in recent_metrics[-10:] if m.error_count > 0) / min(10, len(recent_metrics))
    ...
    def _calculate_overall_status(self) -> Dict[str, Any]:
        ...
        # Determine status
        if (avg_efficiency >= self.alert_thresholds.min_efficiency and 
            avg_latency <= self.alert_thresholds.max_latency_ms and 
    ...
    def _get_strategy_performance_summary(self) -> List[Dict[str, Any]]:
        ...
        for strategy_name, metrics_list in self.metrics_buffer.items():
            ...
            summaries.append({
                ...
                'last_execution': recent_metrics[-1].timestamp.isoformat() if recent_metrics else None,
    ...
    def _get_performance_trends_summary(self) -> Dict[str, Any]:
        ...
        return {
            'last_hour': {
                'avg_efficiency': statistics.mean([t['efficiency'] for t in last_hour]) if last_hour else 0,
                'avg_latency': statistics.mean([t['latency'] for t in last_hour]) if last_hour else 0,
                'total_errors': sum([t['error_count'] for t in last_hour])
            },
            'last_24_hours': {
                'avg_efficiency': statistics.mean([t['efficiency'] for t in last_day]) if last_day else 0,
                'avg_latency': statistics.mean([t['latency'] for t in last_day]) if last_day else 0,
