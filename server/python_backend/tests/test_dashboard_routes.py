...
def mock_dashboard_dependencies():
    patches = [
        ...
        patch('server.python_backend.dashboard_routes.performance_monitor', mock_performance_monitor_dashboard_instance)
    ]
    for p in patches:
        p.start()
    yield
...
def test_get_dashboard_stats(client_dashboard):
    ...
    mock_stats_data_from_db = {
        ...
        "weeklyGrowth": { # Placeholder from DB logic
            "totalEmails": 100, "autoLabeled": 80, "categories": 2, "timeSaved": 1
        }
    }
    ...
    mock_stats_data_from_db_for_model = {
        "total_emails": 1000, "auto_labeled": 800, "categories": 10, "time_saved": "10 hours",
    ...
    assert response_json_rerun["weekly_growth"]["emails"] == mock_stats_data_from_db_for_model["weekly_growth"]["emails"]
...
def test_get_performance_overview(client_dashboard):
    ...
    mock_performance_monitor_dashboard_instance.get_real_time_dashboard.return_value = mock_overview_data
...
def test_get_performance_overview_error(client_dashboard):
    mock_performance_monitor_dashboard_instance.get_real_time_dashboard.side_effect = Exception("Perf Error")
