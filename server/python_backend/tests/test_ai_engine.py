...
from server.python_nlp.nlp_engine import \
    NLPEngine  # NLPEngine is used by AdvancedAIEngine
...
async def test_analyze_email_no_db_provided(ai_engine_instance: AdvancedAIEngine):
    ...
    ai_engine_instance.nlp_engine.analyze_email.return_value['categories'] = ['AI Category Suggestion 1']
...
async def test_analyze_email_with_db_category_match(ai_engine_instance: AdvancedAIEngine):
    ...
    ai_engine_instance.nlp_engine.analyze_email.return_value['categories'] = ['Work Related', 'Important Stuff']

    # Mock database categories
    mock_db_categories = [
        {'id': 1, 'name': 'Personal', 'description': '', 'color': '', 'count': 0},
        {'id': 5, 'name': 'Work Related', 'description': '', 'color': '', 'count': 0}, # This should match
    ...
    result = await ai_engine_instance.analyze_email(subject, content, db=mock_db_manager_for_ai_engine)
...
async def test_analyze_email_with_db_no_category_match(ai_engine_instance: AdvancedAIEngine):
    ...
    ai_engine_instance.nlp_engine.analyze_email.return_value['categories'] = ['Very New AI Category']
...
async def test_analyze_email_no_ai_categories_to_match(ai_engine_instance: AdvancedAIEngine):
    ...
    ai_engine_instance.nlp_engine.analyze_email.return_value['categories'] = [] # NLP engine returns no categories
