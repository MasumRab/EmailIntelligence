...
def mock_action_dependencies():
    patches = [
        ...
        patch(
            "server.python_backend.action_routes.performance_monitor",
            mock_performance_monitor_action_instance,
        ),
    ]
    for p in patches:
        p.start()
...
def test_extract_actions_from_text(client_action):
    ...
    mock_ai_result.action_items = [
        {
            "action_phrase": "follow up on this task",
            "context": "Please follow up on this task by Friday.",
        }
    ]
...
def test_extract_actions_from_text_with_subject(client_action):
    ...
    mock_ai_engine_action.analyze_email.assert_called_once_with(
        subject=request_data["subject"], content=request_data["content"], db=None
    )
