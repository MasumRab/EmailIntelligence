...
class EmailBase(BaseModel):
    ...
    senderEmail: str = Field(..., pattern=r'^[^@]+@[^@]+\.[^@]+$') # Changed regex to pattern
...
class CategoryBase(BaseModel):
    ...
    color: str = Field(default="#6366f1", pattern=r'^#[0-9A-Fa-f]{6}$') # Changed regex to pattern
...
class AIAnalysisResponse(BaseModel):
    ...
    class Config:
        validate_by_name = True # Changed from allow_population_by_field_name
...
class ServiceHealth(BaseModel):
    status: str = Field(pattern=r'^(healthy|degraded|unhealthy)$') # Changed regex to pattern
    error: Optional[str] = None
    timestamp: datetime
