...
class DatabaseManager:
    ...
    async def initialize(self):
        ...
        logger.info(
            "DatabaseManager initialized. Default categories seeding attempted if constructor called it."
        )
        pass
        ...
        for field in fields:
            if field in row and isinstance(row[field], str):
                try:
                    ...
                    logger.warning(
                        f"Failed to parse JSON for field {field} in row {row.get('id')}"
                    )
                    row[field] = (
                        {} if field == "analysisMetadata" or field == "metadata" else []
    ...
    async def create_email(
        ...
        params = (
            ...
            json.dumps(
                email_data.get(
                    "analysis_metadata", email_data.get("analysisMetadata", {})
                )
            ),
            ...
            email_data.get(
                "category"
            ),  # This field seems to be a string name, not an ID. schema.ts emails.category is text.
            ...
            email_data.get(
                "is_first_in_thread", email_data.get("isFirstInThread", True)
            ),
        )
        try:
            ...
            if result and result.get("id"):
                ...
                if category_id:
                    await self._update_category_count(
                        category_id
                    )  # Make sure this is adapted too
                return await self.get_email_by_id(
                    email_id
                )  # Adapted to fetch full dict
            return None
        except psycopg2.IntegrityError as e:
            # This IntegrityError check is for UNIQUE constraint on messageId
            logger.warning(
                f"Email with messageId {email_data.get('messageId')} likely already exists: {e}"
            )
    ...
    async def update_email_by_message_id(
        ...
        for key, value in update_data.items():
            ...
            if key == "message_id":
                continue  # Cannot update message_id itself, it's for the WHERE clause
            ...
            # "references" also needs to be handled if it can be updated.
                ...
                if key == "references":  # references is text[]
                    if not isinstance(value, list):
                        value = [str(value)]
                column_name = (
                    key  # Assumes these are already correct snake_case / single word
                )
            else:
                ...
                logger.warning(
                    f"update_email_by_message_id: Unhandled or potentially problematic key '{key}'. Skipping update for this field."
                )
                continue
        ...
        if not set_clauses:
            logger.warning(
                f"No valid fields to update for message_id: {message_id}"
            )  # Corrected f-string
            return await self.get_email_by_message_id(
                message_id
    ...
    async def create_activity(
        ...
        params = (
            ...
            ),  # Ensure ISO format for text timestamp, "timestamp" col name from schema
            activity_data.get(
                "icon", "default_icon"
            ),  # Provide defaults if not in activity_data
            activity_data.get(
                "icon_bg", activity_data.get("iconBg", "#ffffff")
            ),  # Provide defaults, prefer icon_bg
        )
    ...
    async def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        results = await asyncio.gather(
            ...
            self._execute_query(
                "SELECT COUNT(*) AS count FROM emails WHERE is_unread = TRUE",
                fetch_one=True,
            ),  # "isUnread" -> is_unread
            self._execute_query(
            ...
            self._execute_query(
                "SELECT COUNT(*) AS count FROM emails WHERE is_spam = TRUE",
                fetch_one=True,
            ),  # "isSpam" -> is_spam
            self._execute_query(
                "SELECT COUNT(*) AS count FROM categories", fetch_one=True
            ),
            self._execute_query(
    ...
    async def update_email(
        ...
        for key, value in update_data.items():
            column_name = key
            if key == "id":
                continue  # Cannot update id itself
            ...
            # Example: subject, content, sender, senderEmail, confidence
                ...
                column_name = '"time"'  # Keep quoted as it's a keyword
            elif key in [
                "subject",
                "content",
                "sender",
                ...
                temp_col_name = key.replace("Id", "_id").replace(
                    "Html", "_html"
                )  # basic common ones
                ...
                if temp_col_name != key:  # If a conversion happened
                    logger.info(
                        f"update_email: Converted key '{key}' to '{temp_col_name}' for SQL query."
                    )
                    column_name = temp_col_name
                else:
                    # If no conversion, and not in known list, warn and skip
                    logger.warning(
                        f"update_email: Unhandled key '{key}' or key not directly updatable by internal ID method. Skipping."
                    )
                    continue
