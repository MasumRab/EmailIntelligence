...
with gr.Blocks(title="No-Code Email Platform (Gradio UI)", theme=gr.themes.Soft()) as iface:
    with gr.Tab("Email Builder"):
        gr.Markdown("## Email Builder Interface (No-Code Style)") # Main title for the tab
        with gr.Row():
            ...
            with gr.Column(scale=3): # Canvas
                ...
                gr.Textbox(label="Canvas Area", lines=20, interactive=False, placeholder="Drag components here to build your email.")
    ...
    with gr.Tab("Campaigns"):
        ...
        gr.DataFrame(headers=["Campaign Name", "Status", "Sent", "Open Rate"],
                     value=[["Summer Sale", "Sent", 10000, "25%"],
    ...
    with gr.Tab("Analytics"):
        ...
        gr.DataFrame(headers=["Metric", "Value"],
                     value=[["Total Emails Sent", 15000],
    ...
    with gr.Tab("Settings"):
        ...
        with gr.Accordion("API Configuration"):
            gr.Textbox(label="Email Service API Key", placeholder="Enter your API key", type="password")
            gr.Dropdown(label="Email Provider", choices=["Provider A", "Provider B", "Provider C"])
