...
class PerformanceMonitor:
    ...
    async def record_email_processing(
        self, email_id: int, ai_analysis: Any, filter_results: Dict[str, Any]
    ):
    ...
    async def get_real_time_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_emails = [
                m
                for m in self.metrics_history["email_processing"]
                if current_time - m["timestamp"] < 3600  # Last hour
            ]

            recent_syncs = [
                m
                for m in self.metrics_history["sync_performance"]
                if current_time - m["timestamp"] < 3600  # Last hour
            ]
    ...
    def _generate_alerts(
        self, recent_emails: List[Dict], recent_syncs: List[Dict]
    ) -> List[Dict[str, Any]]:
    ...
    async def get_system_health(self) -> SystemHealth:
        ...
        try:
            ...
            status = "healthy"
            if (
                cpu_usage > self.alert_thresholds["cpu_usage"]
                or memory.percent > self.alert_thresholds["memory_usage"]
                or disk.percent > self.alert_thresholds["disk_usage"]
            ):
                status = "warning"

            if (
                cpu_usage > self.alert_thresholds["cpu_usage"] * 1.2
                or memory.percent > self.alert_thresholds["memory_usage"] * 1.2
            ):  # More stringent for critical
                status = "critical"
    ...
    async def get_metrics_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        relevant_alerts = [
            ...
            if datetime.fromisoformat(a["timestamp"]) > since_time and not a["resolved"]
        ]
    ...
    async def get_service_performance(
        ...
        return {
            ...
            "error_rate_percentage": (
                (total_errors / len(response_times) * 100) if response_times else 0
            ),  # Error rate based on response_time metrics
            "data_points": [
                vars(m) for m in service_data_points
            ],  # Convert dataclasses to dicts for output
        }
