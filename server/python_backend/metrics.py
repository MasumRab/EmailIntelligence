...
EMAIL_PROCESSING_TIME = Summary(
    "email_processing_seconds", "Time spent processing emails", ["operation"]
)

EMAIL_COUNT = Counter(
    "emails_processed_total", "Total number of emails processed", ["status", "category"]
)

AI_ANALYSIS_CONFIDENCE = Histogram(
    "ai_analysis_confidence", "Confidence scores of AI analysis", ["analysis_type"]
)

DATABASE_QUERY_TIME = Histogram(
    "database_query_seconds", "Time spent executing database queries", ["query_type"]
)
...
def record_email_processed(status: str, category: Optional[str] = None) -> None:
    ...
    """
    EMAIL_COUNT.labels(status=status, category=category or "unknown").inc()
