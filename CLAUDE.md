...
The `.taskmaster/scripts/` directory provides automation utilities that Claude Code can use for task management, Git operations, and orchestration workflows.
...
Create custom commands in `.claude/commands/` for script integration:

**Task Discovery Command:**
```bash
# .claude/commands/task-discover.sh
#!/bin/bash
python scripts/search_tasks.py "$1" --show-context
```
...
```javascript
// Use MCP tools for basic operations
next_task
get_task --id=7
set_task_status --id=7 --status=done
...
1. Use MCP tools for standard task operations (get_tasks, next_task, set_task_status)
2. Use scripts for advanced filtering, search, and analysis
3. Use scripts for Git and orchestration operations
4. Use scripts for task file management and recovery
5. Use scripts for generating summaries and reports

**Script Security:**
All Python scripts implement security validation:
- Path security prevents directory traversal attacks
- File size limits (50MB) prevent memory exhaustion
- Secure JSON loading validates content before parsing
- Backup mechanisms create backups before destructive operations

**Best Practices:**
1. Use `--dry-run` flag for generation and sync scripts
...
6. Use custom commands for frequently used script combinations
