...
def check_wsl_requirements():
    ...
    try:
        if subprocess.run(["xset", "-q"], capture_output=True, timeout=2).returncode != 0:
                ...
                with open(full_path, "r", encoding="utf-8") as f:
                    ...
                    for marker in conflict_markers:
                        if marker in content:
                            logger.error(
                                f"Unresolved merge conflict detected in {file_path} with marker: "
                                f"{marker.strip()}"
                            )
...
def activate_conda_env(env_name: str = None) -> bool:
    ...
    if not re.match(r"^[a-zA-Z0-9_-]+$", env_name):
        logger.error(
            f"Invalid conda environment name: {env_name}. "
...
def install_nodejs_dependencies(directory: Path, service_name: str, update: bool = False) -> bool:
    """Install Node.js dependencies in a given directory."""
    pkg_json_path = directory / "package.json"
    if not pkg_json_path.exists():
        logger.debug(f"No package.json in '{service_name}', skipping npm install.")
        return True
    ...
    desc = f"{'Updating' if update else 'Installing'} Node.js dependencies for '{service_name}'"
    return run_command(cmd, desc, cwd=directory, shell=(os.name == "nt"))


def start_client():
    """Start the Node.js frontend."""
    logger.info("Starting Node.js frontend...")
    if not install_nodejs_dependencies(ROOT_DIR / "client", "Frontend Client"):
        return None
...
def handle_setup(args, venv_path):
    ...
    if args.use_conda:
        ...
        install_nodejs_dependencies(ROOT_DIR, "Root")
        install_nodejs_dependencies(ROOT_DIR / "client", "Frontend Client")
        install_nodejs_dependencies(ROOT_DIR / "backend" / "server-ts", "TypeScript Backend")
...
def reinstall_torch(cpu_only=False):
    ...
    try:
        ...
        for package_spec in torch_packages:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "--upgrade"] + package_spec.split(),
                capture_output=True,
                text=True,
            )
