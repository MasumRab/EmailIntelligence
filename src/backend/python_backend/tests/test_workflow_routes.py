...
async def test_create_and_activate_new_workflow(client, mock_workflow_engine):
    ...
    mock_workflow_engine.create_and_register_workflow_from_config = AsyncMock(side_effect=mock_create)

    # 1. Create the new workflow
    response_create = client.post("/api/workflows", json=new_workflow_config)
    assert response_create.status_code == 200
    assert response_create.json()["message"] == "Legacy workflow 'my_brand_new_workflow' created successfully."
    ...
    mock_workflow_engine.create_and_register_workflow_from_config.assert_called_once_with(expected_config)
