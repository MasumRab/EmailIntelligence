...
EMAILS_INDEX_FILE = os.path.join(DATA_DIR, "emails_index.json.gz")
...
class JSONDataManager:
    """
    Pure JSON-based database manager with in-memory caching, write-behind,
    and hybrid on-demand content loading.

    This class provides an asynchronous interface for all CRUD (Create, Read,
    Update, Delete) operations. It handles loading data from JSON files into memory
    ...
    Attributes:
        emails_index_file (str): Path to the emails index JSON file.
    ...
    def __init__(self, data_dir: str = None):
        """Initializes the JSONDataManager, setting up file paths and data caches."""
        if data_dir:
    ...
    async def _load_data(self) -> None:
        ...
        for data_type, file_path, data_list_attr in [
            (DATA_TYPE_EMAILS, self.emails_index_file, "emails_data"),
    ...
    async def _save_data_to_file(
        ...
        if data_type == DATA_TYPE_EMAILS:
            file_path, data_to_save = self.emails_index_file, self.emails_data
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("JSONDataManager shutting down. Saving dirty data...")
...
async def get_db() -> JSONDataManager:
    """
    Provides the singleton instance of the JSONDataManager.

    This function is used for dependency injection in FastAPI routes. It ensures
    that only one instance of the JSONDataManager is used throughout the

    application's lifecycle.

    Returns:
        The singleton JSONDataManager instance.
    """
    global _db_manager_instance
    if _db_manager_instance is None:
        # This should ideally not be reached if startup event is properly set
        _db_manager_instance = JSONDataManager()
