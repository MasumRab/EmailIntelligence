...
class FictionalitySettings(BaseSettings):
    ...
    fictionality_rate_limit_rpm: int = Field(default=3, description="Fictionality analysis requests per minute")

    # Circuit breaker settings
    fictionality_circuit_breaker_threshold: int = Field(
        default=5, description="Circuit breaker failure threshold for fictionality analysis"
    )
    fictionality_circuit_breaker_timeout: int = Field(default=300, description="Circuit breaker timeout in seconds")

    # Caching configuration
    fictionality_cache_ttl: int = Field(
        default=3600, description="Fictionality analysis cache TTL in seconds"  # 1 hour
    )
    fictionality_cache_enabled: bool = Field(default=True, description="Enable fictionality analysis caching")

    # Analysis thresholds
    fictionality_default_threshold: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Default threshold for fictionality classification"
    )
    fictionality_high_threshold: float = Field(default=0.8, ge=0.0, le=1.0, description="High fictionality threshold")
    ...
    fictionality_max_tokens: int = Field(default=1500, description="Maximum tokens for fictionality analysis")
    fictionality_temperature: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Temperature for fictionality analysis (low for consistency)"
    )

    # Batch processing
    fictionality_batch_size: int = Field(default=10, description="Default batch size for fictionality analysis")
    fictionality_max_concurrent: int = Field(default=3, description="Maximum concurrent fictionality analyses")

    # Content hashing
    fictionality_content_hash_algorithm: str = Field(
        default="sha256", description="Hash algorithm for content deduplication"
    )
    fictionality_content_hash_length: int = Field(default=16, description="Length of content hash for caching")
    ...
    fictionality_fallback_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Fallback fictionality score when analysis fails"
    )
    fictionality_max_retries: int = Field(default=3, description="Maximum retries for fictionality analysis")
    ...
    fictionality_performance_tracking: bool = Field(
        default=True, description="Enable performance tracking for fictionality analysis"
    )
    fictionality_metrics_retention_hours: int = Field(
        default=168, description="Retention period for fictionality metrics in hours"  # 7 days
    )

    # Feature flags
    fictionality_enabled: bool = Field(default=True, description="Enable fictionality analysis feature")
    fictionality_batch_enabled: bool = Field(default=True, description="Enable batch fictionality analysis")
    fictionality_caching_enabled: bool = Field(default=True, description="Enable fictionality analysis caching")
    fictionality_monitoring_enabled: bool = Field(default=True, description="Enable fictionality analysis monitoring")

    # Integration settings
    fictionality_integration_mode: str = Field(default="auto", description="Integration mode: auto, manual, disabled")
    ...
    # API configuration
    fictionality_api_timeout: int = Field(default=60, description="API timeout for fictionality analysis in seconds")
    fictionality_api_version: str = Field(default="v1", description="API version for fictionality analysis")
...
class FictionalityAnalysisConfig:
    ...
    def get_cache_ttl_for_analysis(analysis_type: str) -> int:
        ...
        ttl_map = {"quick": 1800, "standard": 3600, "deep": 7200}  # 30 minutes  # 1 hour  # 2 hours
        return ttl_map.get(analysis_type, 3600)

    @staticmethod
    def should_enable_caching() -> bool:
        """Check if caching should be enabled"""
        return fictionality_settings.fictionality_cache_enabled and fictionality_settings.fictionality_caching_enabled
    ...
    def get_strategy_adjustment(fictionality_level: str) -> float:
        ...
        return fictionality_settings.fictionality_strategy_reduction_factors.get(fictionality_level, 0.0)
