...
class FictionalitySettings(BaseSettings):
    ...
    fictionality_rate_limit_rpm: int = Field(
        default=3,
        description="Fictionality analysis requests per minute"
    )
    ...
    fictionality_cache_ttl: int = Field(
        default=3600,  # 1 hour
        description="Fictionality analysis cache TTL in seconds"
    )
    fictionality_cache_enabled: bool = Field(
        default=True,
        description="Enable fictionality analysis caching"
    )
    ...
    fictionality_high_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="High fictionality threshold"
    )
    fictionality_uncertain_threshold: float = Field(
        default=0.4,
        ge=0.0,
        le=1.0,
        description="Uncertain fictionality threshold"
    )
    
    # Analysis parameters
    fictionality_analysis_depth: str = Field(
        default="standard",
        description="Default analysis depth: quick, standard, deep"
    )
    fictionality_max_tokens: int = Field(
        default=1500,
        description="Maximum tokens for fictionality analysis"
    )
    ...
    fictionality_batch_size: int = Field(
        default=10,
        description="Default batch size for fictionality analysis"
    )
    fictionality_max_concurrent: int = Field(
        default=3,
        description="Maximum concurrent fictionality analyses"
    )
    
    # Content hashing
    fictionality_content_hash_algorithm: str = Field(
        default="sha256",
        description="Hash algorithm for content deduplication"
    )
    fictionality_content_hash_length: int = Field(
        default=16,
        description="Length of content hash for caching"
    )
    ...
    fictionality_max_retries: int = Field(
        default=3,
        description="Maximum retries for fictionality analysis"
    )
    fictionality_retry_delay: float = Field(
        default=1.0,
    ...
    fictionality_metrics_retention_hours: int = Field(
        default=168,  # 7 days
        description="Retention period for fictionality metrics in hours"
    )
    
    # Feature flags
    fictionality_enabled: bool = Field(
        default=True,
        description="Enable fictionality analysis feature"
    )
    fictionality_batch_enabled: bool = Field(
        default=True,
        description="Enable batch fictionality analysis"
    )
    ...
    fictionality_monitoring_enabled: bool = Field(
        default=True,
        description="Enable fictionality analysis monitoring"
    )
    
    # Integration settings
    fictionality_integration_mode: str = Field(
        default="auto",
        description="Integration mode: auto, manual, disabled"
    )
    fictionality_strategy_integration: bool = Field(
        default=True,
        description="Integrate fictionality with strategy generation"
    )
    fictionality_conflict_integration: bool = Field(
        default=True,
        description="Integrate fictionality with conflict analysis"
    )
    
    # API configuration
    fictionality_api_timeout: int = Field(
        default=60,
        description="API timeout for fictionality analysis in seconds"
    )
    fictionality_api_version: str = Field(
        default="v1",
        description="API version for fictionality analysis"
    )
...
class FictionalityAnalysisConfig:
    ...
    def should_enable_caching() -> bool:
        ...
        return (
            fictionality_settings.fictionality_cache_enabled and
            fictionality_settings.fictionality_caching_enabled
        )
