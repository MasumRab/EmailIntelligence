...
class Settings(BaseSettings):
    ...
    allowed_origins: List[str] = Field(
        default=["http://localhost:3000", "http://localhost:5173"], description="Allowed CORS origins"
    )
    ...
    ai_max_complexity_analysis: int = Field(default=50, description="Max complexity score to analyze")
    ai_min_confidence_threshold: float = Field(default=0.7, description="Minimum confidence for AI suggestions")
    ai_fallback_enabled: bool = Field(default=True, description="Enable AI fallback strategies")
    ai_circuit_breaker_threshold: int = Field(default=5, description="Circuit breaker failure threshold")
    ai_circuit_breaker_timeout: int = Field(default=300, description="Circuit breaker timeout in seconds")
