...
class WorktreeManager:
    ...
    async def ensure_worktree_base(self) -> None:
        ...
        if not self.worktree_base.exists():
            ...
            if gitignore.exists():
                ...
                if settings.git_worktree_base not in content:
                    with gitignore.open("a", encoding="utf-8") as f:
                        f.write(f"\n{settings.git_worktree_base}/\n")
    ...
    async def remove_worktree(self, pr_id: str) -> None:
        ...
        try:
            ...
            logger.warning("Git worktree remove failed, forcing directory cleanup", error=str(e))
            ...
            try:
                ...
                logger.error("Failed to cleanup worktree directory", path=str(worktree_path), error=str(cleanup_error))
                raise GitOperationError(f"Failed to remove worktree: {str(cleanup_error)}") from cleanup_error
