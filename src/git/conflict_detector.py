...
class GitConflictDetector(IConflictDetector):
    ...
    async def detect_conflicts_between_branches(
        ...
        try:
            ...
            # Note: --name-only is not supported by merge-tree --write-tree in standard git
            # We parse the informational messages to find conflicts
            ...
            for line in lines[1:]:
                ...
                # Example: CONFLICT (content): Merge conflict in src/main.py
                if line.startswith("CONFLICT"):
                    file_path = None

                    file_path = None

                    # Pattern for content conflicts
                    content_match = re.search(r"Merge conflict in (.+)", line)
                    if content_match:
                        file_path = content_match.group(1).strip()
                    else:
                        # Pattern for modify/delete conflicts
                        # CONFLICT (modify/delete): <file> deleted in ... and modified in ...
                        modify_delete_match = re.search(
                            r"CONFLICT \([^)]+\):\s*(.+?)\s+deleted in", line
                        )
                        if modify_delete_match:
                            file_path = modify_delete_match.group(1).strip()

                    if file_path and file_path not in seen_files:
                        seen_files.add(file_path)
                        conflict = await self._analyze_file_conflict(
                            file_path, source_branch, target_branch, tree_oid
                        )
    ...
    async def _analyze_file_conflict(
        self, file_path: str, source_branch: str, target_branch: str, tree_oid: str
    ) -> Conflict:
        """
        Analyze a specific file conflict to extract details and blocks.
        """
        normalized_name = file_path.replace("/", "-").replace(".", "_")
        hash_suffix = hashlib.sha256(file_path.encode("utf-8")).hexdigest()[:8]
        conflict_id = f"conflict-{normalized_name}-{hash_suffix}"
        ...
        if "changed in both" in output:
            ...
            for line in output.splitlines():
                if "changed in both" in line:
                    ...
                    conflicts.append(
                        Conflict(
                            id=f"legacy-{file_path.replace('/', '-').replace('.', '_')}",
                            type=ConflictTypeExtended.MERGE_CONFLICT,
                            severity=RiskLevel.HIGH,
                            description=f"Merge conflict in {file_path}",
                            file_paths=[file_path],
