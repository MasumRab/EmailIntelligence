...
class ProjectConfig(BaseModel):
    ...
    enable_code_execution: bool = Field(default=False, description="Whether agents can execute code")
    enable_file_writing: bool = Field(default=False, description="Whether agents can write files")
    enable_shell_commands: bool = Field(default=False, description="Whether agents can run shell commands")
    ...
    custom_settings: Dict[str, Any] = Field(default_factory=dict, description="Project-specific custom settings")
...
class ContextProfile(BaseModel):
    ...
    # Environment mapping
    branch_patterns: List[str] = Field(default_factory=list, description="Git branch patterns that match this profile")

    # Context content
    allowed_files: List[str] = Field(default_factory=list, description="File patterns that agents can access")
    blocked_files: List[str] = Field(default_factory=list, description="File patterns that agents cannot access")

    # Agent behavior configuration
    agent_settings: Dict[str, Any] = Field(default_factory=dict, description="Agent-specific configuration settings")

    # Project configuration (User Story 2)
    project_config: Optional[ProjectConfig] = Field(default=None, description="Project-specific configuration settings")
...
class AgentContext(BaseModel):
    ...
    # Context boundaries
    accessible_files: List[str] = Field(default_factory=list, description="Files the agent can currently access")
    restricted_files: List[str] = Field(default_factory=list, description="Files the agent cannot access")

    # Project configuration (User Story 2)
    profile_config: Optional[ProjectConfig] = Field(default=None, description="Project-specific configuration settings")
    ...
    # Security tracking
    access_log: List[Dict[str, Any]] = Field(default_factory=list, description="Log of file access attempts")
