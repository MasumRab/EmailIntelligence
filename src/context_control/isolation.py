...
class ContextIsolator:
    ...
    def __init__(self, context: AgentContext, config=None):
        ...
        # Optimization: Use a single combined regex for O(1) matching complexity
        self._restricted_pattern = self._compile_patterns(context.restricted_files)
        self._accessible_pattern = self._compile_patterns(context.accessible_files)
    ...
    def _compile_patterns(self, patterns: List[str]) -> Optional[Pattern]:
        ...
        for p in patterns:
            try:
                ...
                logger.error(f"Failed to convert pattern '{p}': {e}")
        ...
        try:
            ...
            # This allows the regex engine to optimize the state machine
    ...
    def is_file_accessible(self, file_path: str) -> bool:
        ...
        if self._matches_patterns(normalized_path, self._restricted_pattern):
            self._log_access(normalized_path, False, "blocked")
            return False

        # Check allowed files (allow list)
        if self._matches_patterns(normalized_path, self._accessible_pattern):
    ...
    def _matches_patterns(self, file_path: str, pattern: Optional[Pattern]) -> bool:
        """Check if a file path matches the combined pattern.
        ...
        # Optimize: Calculate basename once outside the check
        try:
