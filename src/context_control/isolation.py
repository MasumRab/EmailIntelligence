...
class ContextIsolator:
    ...
    def __init__(self, context: AgentContext, config=None):
        ...
        # OPTIMIZATION: Compile all patterns into a single regex for O(1) matching
        self._restricted_patterns: Optional[Pattern] = self._compile_patterns(
            context.restricted_files
        )
        self._accessible_patterns: Optional[Pattern] = self._compile_patterns(
    ...
    def _compile_patterns(self, patterns: List[str]) -> Optional[Pattern]:
        ...
        for p in patterns:
            try:
                ...
                logger.error(f"Failed to translate pattern '{p}': {e}")

        if not regex_strs:
            return None

        try:
            # Join with OR operator to create single regex
