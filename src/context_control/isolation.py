...
class ContextIsolator:
    ...
    def __init__(self, context: AgentContext, config=None):
        ...
        # Optimization: We now compile into a single regex for O(1) matching complexity (relative to pattern count)
    ...
    def _compile_patterns(self, patterns: List[str]) -> Optional[Pattern]:
        ...
        for p in patterns:
            try:
                ...
                # It returns a regex string anchored with \Z that we can compile
                regex_str = fnmatch.translate(p)
                # Wrap in non-capturing group to ensure precedence when joined
    ...
    def _matches_patterns(self, file_path: str, pattern: Optional[Pattern]) -> bool:
        ...
        try:
            ...
            # This supports patterns that are meant to match filenames regardless of directory
