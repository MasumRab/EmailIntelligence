...
class ContextIsolator:
    ...
    def _compile_patterns(self, patterns: List[str]) -> Optional[Pattern]:
        """Compile glob patterns into a single regex object.
        ...
        for p in patterns:
            try:
                ...
                valid_regexes.append(regex_str)
        ...
        try:
            # Optimize: Combine all patterns into one regex for O(1) matching vs O(N)
            combined_regex = '|'.join(valid_regexes)
    ...
    def _matches_patterns(self, file_path: str, patterns: Optional[Pattern]) -> bool:
        ...
        Args:
            ...
            patterns: Compiled regex pattern (optional)
