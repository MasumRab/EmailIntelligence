...
class ContextIsolator:
    ...
    def __init__(self, context: AgentContext, config=None):
        ...
        # Optimization: Combine all patterns into a single regex for O(1) matching complexity relative to number of patterns
    ...
    def _compile_patterns(self, patterns: List[str]) -> Optional[Pattern]:
        ...
        Returns:
            Compiled regex object matching any of the patterns, or None if no patterns
        """
        if not patterns:
            return None
        ...
        try:
            # Combine all regexes with OR operator
