...
class TaskMasterIntegration:
    ...
    async def update_task_status(
        ...
        if constitutional_validation and status in [
            TaskStatus.DONE,
            TaskStatus.BLOCKED,
        ]:
    ...
    async def get_task_dependencies(
        self, task_id: str, include_blocked: bool = True
    ) -> Dict[str, Any]:
        """
                ...
                if dep_id in self.tasks:
                    ...
                    if dep_task.status != TaskStatus.DONE:
                        blocked_reasons.append(
                            f"Dependency {dep_id} ({dep_task.title}) is not completed"
                        )
    ...
    async def validate_constitutional_compliance(
        ...
        try:
            ...
            if validation_type == "specification":
                compliance_result = (
                    await self.constitutional_engine.validate_specification_template(
    ...
    def get_task_summary(self) -> Dict[str, Any]:
        ...
        compliance_scores = [
            t.constitutional_score for t in self.tasks.values() if t.constitutional_score > 0
        ]
