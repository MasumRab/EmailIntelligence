...
class TaskMasterIntegration:
    ...
    async def create_resolution_task(
        ...
        logger.info("Resolution task created", task_id=task_id, title=title, phase=phase, parallel_group=parallel_group)

        return task_id

    async def update_task_status(
        self, task_id: str, status: TaskStatus, notes: str = None, constitutional_validation: bool = True
    ) -> bool:
        """
        Update task status
        ...
        logger.info("Task status updated", task_id=task_id, old_status=old_status.value, new_status=status.value)

        return True

    async def create_worktree_environment(
        self, task_id: str, branch_name: str = None, isolated: bool = True, cleanup_on_complete: bool = True
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error("Failed to create worktree", task_id=task_id, error=result.stderr)
                return None
            ...
            logger.info(
                "Worktree environment created", task_id=task_id, worktree_path=worktree_path, branch_name=branch_name
            )

            return worktree_path

        except Exception as e:
            logger.error("Failed to create worktree environment", task_id=task_id, error=str(e))
            return None
    ...
    async def validate_constitutional_compliance(
        self, task_id: str, validation_type: str, content: str, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                "Constitutional validation failed", task_id=task_id, validation_type=validation_type, error=str(e)
            )
            return {"error": str(e)}
            ...
            for task_id in tasks_to_cleanup:
                if task_id in self.worktree_environments:
                    ...
                    if result.returncode == 0:
                        ...
                        logger.error("Failed to cleanup worktree", task_id=task_id, error=result.stderr)
    ...
    def _get_default_completion_criteria(self, phase: str) -> List[str]:
        ...
        criteria_map = {
            ...
            "Phase 4": ["CLI integration completed", "Performance optimization implemented", "Final validation passed"],
        }
    ...
    def _get_default_validation_steps(self, phase: str) -> List[str]:
        ...
        steps_map = {
            ...
            "Phase 1": ["Template generation validation", "Constitutional compliance check", "Integration testing"],
            "Phase 2": [
                "Strategy generation testing",
                "Constitutional validation testing",
                "Multi-strategy comparison validation",
            ],
            "Phase 3": ["Unit test execution", "Integration test execution", "Performance benchmark validation"],
            "Phase 4": ["End-to-end testing", "Performance validation", "Final constitutional compliance check"],
        }
    ...
    async def _validate_task_constitutional_compliance(self, task: ResolutionTask):
        ...
        try:
            ...
            logger.error("Constitutional compliance validation failed", task_id=task.id, error=str(e))
        ...
        for task_id, task in self.tasks.items():
            if completed_task_id in task.dependencies and task.status == TaskStatus.BLOCKED:
                ...
                if all_deps_satisfied:
                    ...
                    task.updated_at = datetime.utcnow()
                    logger.info("Dependent task unblocked", task_id=task_id, dependency=completed_task_id)
    ...
    def get_task_summary(self) -> Dict[str, Any]:
        ...
        for task in self.tasks.values():
            ...
            if phase not in phase_distribution:
                phase_distribution[phase] = {"total": 0, "completed": 0, "in_progress": 0, "blocked": 0}
        ...
        compliance_scores = [t.constitutional_score for t in self.tasks.values() if t.constitutional_score > 0]
        avg_constitutional_score = sum(compliance_scores) / len(compliance_scores) if compliance_scores else 0.0
