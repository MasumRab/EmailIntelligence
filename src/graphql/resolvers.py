...
class Resolvers:
    ...
    async def resolve_pull_requests(
        root, info, status=None, author=None, assigned=None, limit=50, offset=0
    ):
        """Resolver for getting all PRs with optional filtering"""
        start_time = time.time()
        try:
            prs = await pr_dao.get_all_prs(
                status=status, author_id=author, limit=limit, offset=offset
            )
    ...
    async def resolve_conflicts(root, info, severity=None, conflict_type=None, limit=50):
        ...
        try:
            conflicts = await conflict_dao.get_conflicts(
                severity=severity, conflict_type=conflict_type, limit=limit
            )
            logger.info(
                "Conflicts resolved",
                count=len(conflicts),
    ...
    async def resolve_escalate_pr(root, info, pr_id, reason):
        ...
        try:
            ...
            result = EscalationResultType(
                success=True, escalated_to="senior_developer", reason=reason
            )
