...
class ResolutionStep(BaseModel):
    ...
    code_changes: List[CodeChange] = Field(
        default_factory=list, description="Code changes for this step"
    )
    validation_steps: List[str] = Field(
        default_factory=list, description="Validation steps to perform"
    )
...
class ResolutionStrategy(BaseModel):
    ...
    requires_approval: bool = Field(
        False, description="Whether this strategy requires human approval"
    )
    ...
    prompt_context: Dict[str, Any] = Field(
        default_factory=dict, description="Context used in AI prompt"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "cons": [
                    "May require review",
                    "Complex conflicts might need adjustments",
                ],
                "confidence": 0.92,
                "estimated_time": 120,
                "risk_level": "MEDIUM",
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "severity": "CRITICAL",
                "layer_violations": [
                    {"from": "UI", "to": "Database", "description": "Direct access"}
                ],
            }
        }
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "confidence": 0.9,
                "evidence": [
                    "pr1 returns success for condition A",
                    "pr2 returns failure for condition A",
                ],
                "resolution_suggestions": [
                    "Refactor to use strategy pattern",
                    "Combine logic with conditional",
                ],
            }
        }
