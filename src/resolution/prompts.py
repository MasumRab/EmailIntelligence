...
{{
  "strategy": {{
    ...
    "rollback_strategy": "Incremental rollback with partial refactoring"
            ...
            "semantic_merge_code": {
                "template": (
                    """You are a senior software engineer with expertise in semantic code merging.
...
"""
            ...
            "code_quality_validation": {
                "template": (
                    """You are a code quality expert. Validate the following resolution """
                    """for quality issues.
...
Result: Successfully merged with enhanced validation functionality.""",
    ...
    def _extract_template_variables(
        ...
        if isinstance(conflict_data, MergeConflict):
            ...
            return {
                ...
                "cycle_path": (
                    " -> ".join(conflict_data.cycle_path)
                    if conflict_data.cycle_path
                    else "No cycle"
                ),
    ...
    def _add_few_shot_examples(
        ...
        relevant_examples = [
            ex for ex in self.examples if ex.input["conflict_type"] == context.conflict_type.value
        ]
    ...
    def _add_constraints(self, prompt: str, context: PromptContext) -> str:
        ...
        if context.performance_requirements.get("max_time", 0) > 0:
            constraints_text += (
                ...
                f"{context.performance_requirements['max_time']} seconds\\n"
            )

        # Risk constraints
        if context.risk_tolerance == "LOW":
            constraints_text += (
                "- Risk tolerance is LOW: prefer safer, more conservative approaches\\n"
            )
        elif context.risk_tolerance == "HIGH":
            constraints_text += (
                "- Risk tolerance is HIGH: consider more aggressive optimizations\\n"
            )
...
Please provide a comprehensive strategy with steps, risk assessment, and validation approach."""
    ...
    def _generate_fallback_code_prompt(
        ...
        return (
            f"Generate code to resolve the conflict based on strategy: "
...
4. Recommend validation steps"""
    ...
    def create_iterative_refinement_prompt(
        self, original_prompt: str, feedback: Dict[str, Any], iteration: int
    ) -> str:
