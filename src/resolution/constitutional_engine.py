...
class ConstitutionalEngine:
    ...
    async def validate_specification_template(
        ...
        for rule in applicable_rules:
            try:
                violation = await self._check_rule_compliance(rule, template_content, "specification_template", context)
        ...
        logger.info(
            "Specification template validation completed", overall_score=overall_score, violation_count=len(violations)
        )
    ...
    async def validate_resolution_strategy(
        ...
        Returns:
            ComplianceResult with detailed analysis
        """
        logger.info("Validating resolution strategy", strategy_id=strategy.id, execution_phase=execution_phase)
        ...
        for rule in applicable_rules:
            try:
                violation = await self._check_rule_compliance(rule, strategy_json, "resolution_strategy", context)
        ...
        strategy_violations = await self._validate_strategy_requirements(strategy, conflict_data, context)
        ...
        self.compliance_history.append(result)
        logger.info(
            "Resolution strategy validation completed", overall_score=overall_score, violation_count=len(violations)
        )
    ...
    async def validate_execution_phase(
        ...
        for rule in applicable_rules:
            try:
                violation = await self._check_rule_compliance(rule, phase_json, "execution_phase", context)
        ...
        result = ComplianceResult(
            ...
            failed_rules=failed_rules,
            analysis_timestamp=datetime.utcnow(),
            analysis_context={"phase_name": phase_name, "phase_data_keys": list(phase_data.keys())},
        )

        self.compliance_history.append(result)
        logger.info(
            "Execution phase validation completed", overall_score=overall_score, violation_count=len(violations)
        )
    ...
    async def get_constitutional_scoring(
        ...
        """
        if weights is None:
            weights = {"specification_template": 0.25, "resolution_strategy": 0.50, "execution_phase": 0.25}
        ...
        scoring_result = {
            ...
            "recommendations": self._generate_scoring_recommendations(constitutional_score, violations_by_type),
    ...
    async def _load_constitutional_rules(self) -> None:
        ...
        rule_files = ["specification-rules.json", "strategy-rules.json", "execution-rules.json", "general-rules.json"]
        ...
        for rule_file in rule_files:
            ...
            if os.path.exists(rule_path):
                try:
                    ...
                    logger.debug(
                        "Loaded constitutional rules", file=rule_file, rule_count=len(rules_data.get("rules", []))
                    )

                except Exception as e:
                    logger.error("Failed to load constitutional rules", file=rule_file, error=str(e))

    async def _compile_rule_patterns(self) -> None:
        """Compile regex patterns for rule validation"""
        for rule_id, rule in self.rules.items():
            try:
                self.violation_patterns[rule_id] = re.compile(rule.rule_pattern, re.MULTILINE | re.IGNORECASE)
            except re.error as e:
    ...
    async def _check_rule_compliance(
        self, rule: ConstitutionalRule, content: str, validation_type: str, context: Dict[str, Any]
        ...
        if not strategy.validation_approach:
            violations.append(
                ViolationResult(
                    ...
                    remediation="Add comprehensive validation approach including testing and verification",
                    auto_fixable=True,
                    confidence=1.0,
                )
            )
    ...
    def _calculate_compliance_score(self, violations: List[ViolationResult], total_rules: int) -> float:
    ...
    def _generate_recommendations(self, violations: List[ViolationResult], validation_type: str) -> List[str]:
        ...
        if ViolationType.CRITICAL in by_type:
            recommendations.append("🚨 CRITICAL: Address critical constitutional violations before proceeding")

        if ViolationType.MAJOR in by_type:
            recommendations.append("⚠️ MAJOR: Resolve major violations to ensure constitutional compliance")

        if ViolationType.MINOR in by_type:
            recommendations.append("ℹ️ MINOR: Consider addressing minor violations for improved compliance")
        ...
        if validation_type == "specification_template":
            ...
            recommendations.append("⚡ Ensure execution phases include proper validation checkpoints")
    ...
    def _generate_scoring_recommendations(
        ...
        if constitutional_score < 0.7:
            recommendations.append("🎯 FOCUS: Constitutional compliance requires immediate attention")
        ...
        if violations_by_type[ViolationType.CRITICAL.value] > 0:
            recommendations.append("🚨 URGENT: Critical violations must be resolved before proceeding")

        if constitutional_score >= 0.9:
            recommendations.append("✅ EXCELLENT: Constitutional compliance is at an excellent level")
