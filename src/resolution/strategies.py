...
from .types import (
    ...
    ResolutionStrategy, ResolutionStep, RiskLevel, PromptContext
)
...
class StrategyGenerator:
    ...
    async def generate_strategies(
        ...
        conflict_data: Union[
            ...
            SemanticConflict, ResourceConflict
        ],
        ...
        try:
            ...
            strategy_prompts = await self._generate_strategy_prompts(
                conflict_data, prompt_context, max_strategies
            )
            ...
            for i, response in enumerate(ai_responses):
                try:
                    strategy = await self._parse_strategy_response(
                        response, conflict_data, i + 1
                    )
...
{{
  ...
  "decision_factors": {
    ...
    async def _generate_strategy_prompts(
        ...
        approaches = [
            "conservative", "aggressive", "innovative", "hybrid", "minimal"
        ]
        ...
        for i in range(max_strategies):
            ...
            base_prompt = self.prompt_engine.generate_strategy_prompt(
                conflict_data, prompt_context
            )
    ...
    def _get_approach_instruction(self, approach: str) -> str:
        ...
        instructions = {
            "conservative": (
                "Focus on minimal changes and proven patterns. "
    ...
    def _rank_strategies(self, strategies: List[ResolutionStrategy]) -> List[ResolutionStrategy]:
        ...
        def calculate_quality_score(strategy: ResolutionStrategy) -> float:
            ...
            # Completeness factor
            completeness = (
                len(strategy.steps) / 10.0 +  # More steps = more thorough
                len(strategy.pros) / 5.0 +    # More pros = better analysis
                len(strategy.success_criteria) / 5.0  # More criteria = better planning
            ) / 3.0
    ...
    def _update_stats(self, strategies: List[ResolutionStrategy], execution_time: float):
        ...
        if strategies:
            ...
            self.generation_stats["average_confidence"] = (
                (self.generation_stats["average_confidence"] * 
                 (self.generation_stats["successful_generations"] - 1) + avg_confidence) /
