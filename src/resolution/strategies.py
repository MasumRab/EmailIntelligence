  ...
  "decision_factors": {{
    ...
    def _get_approach_instruction(self, approach: str) -> str:
        ...
        instructions = {
            ...
            "innovative": (
                "Explore novel solutions and emerging patterns. "
                "Consider cutting-edge approaches and technologies."
            ),
            "hybrid": (
                "Combine multiple approaches strategically. "
                "Balance risk and benefit across different aspects."
            ),
            "minimal": (
                "Focus on the simplest possible solution that resolves the immediate conflict "
                "with minimal disruption."
            ),
        }

        return instructions.get(
            approach, "Generate a balanced approach that considers multiple factors."
        )
    ...
    def _rank_strategies(self, strategies: List[ResolutionStrategy]) -> List[ResolutionStrategy]:
        ...
        def calculate_quality_score(strategy: ResolutionStrategy) -> float:
            ...
            completeness = (
                ...
                + len(strategy.success_criteria)
                / 5.0  # More pros = better analysis  # More criteria = better planning
            ) / 3.0
        ...
        return {
            **self.generation_stats,
            ...
            "cached_strategies": len(
                [s for strategies in self.strategy_cache.values() for s in strategies]
            ),
        }
