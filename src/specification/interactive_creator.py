...
class GuidedPromptSystem:
            ...
            "conflict_analysis": {
                "user_levels": {
                    "beginner": {
                        "questions": [
                            {
                                ...
                                "type": "text",
                                "prompt": (
                                    "Can you describe the conflict you're trying to resolve? "
                                    "(e.g., 'Two different implementations of the same feature')"
                                ),
                                "validation": "non_empty",
                                "help": (
                                    "A clear description helps identify the right "
                                    "resolution approach"
                                ),
                            },
                            {
                                ...
                                "type": "multiselect",
                                "prompt": (
                                    "Which files are involved in this conflict? "
                                    "(select from your changed files)"
                                ),
                        ...
                        "context_prompts": [
                            (
                                "Since you're new to conflict resolution, "
                                "I'll guide you through a systematic approach."
                            ),
                            (
                                "Don't worry if you're unsure about any technical details - "
                        ...
                        "questions": [
                            {
                                ...
                                "validation": "single_option",
                                "help": (
                                    "Identifying the conflict type helps choose the "
                ...
                "questions": [
                    {
                        ...
                        "validation": "single_option",
                        "help": (
                            "Higher compliance levels provide more validation but take more time"
                        ),
                    },
                    {
                        "id": "quality_gates",
    ...
    async def _collect_conflict_analysis(self, prompt_context: PromptContext):
        ...
        templates = self.prompt_templates["conflict_analysis"]["user_levels"][
            prompt_context.user_level
        ]
    ...
    def _validate_response(self, question: Dict[str, Any], response: Any) -> Dict[str, Any]:
        ...
        if validation_type == "non_empty":
            if not response_str.strip():
                ...
                result["feedback"] = (
                    "This information is required for effective conflict resolution."
                )
            else:
    ...
    async def _generate_final_specification(self) -> Dict[str, Any]:
        ...
        project_context = {
            ...
            "user_profile": {
                ...
                "experience": self.current_session.user_responses.get(
        ...
        team_context = {
            ...
            "experience_level": self.current_session.user_responses.get(
                "user_level", "intermediate"
            ),
            "constraints": self.current_session.user_responses.get("resolution_constraints", ""),
            "preferences": {
                "risk_tolerance": self.current_session.user_responses.get(
                    "risk_tolerance", "Moderate"
                ),
                "execution_approach": self.current_session.user_responses.get(
                    "execution_approach", "sequential"
                ),
                "rollback_strategy": self.current_session.user_responses.get(
        ...
        spec_phase = (
            SpecificationPhase.BASELINE if phase_type == "baseline" else SpecificationPhase.IMPROVED
        )
        ...
        try:
            ...
            self.current_session.quality_score = specification.get("template_metadata", {}).get(
                "quality_score", 0.0
            )
            ...
            return {
                ...
                "template_content": {
                    "overview": {
                        "description": "Basic specification created from interactive session"
                    },
                    "implementation_strategy": {"approach": "Manual implementation required"},
                },
                "quality_recommendations": {
                    "general": [
                        "Complete specification generation failed - manual intervention required"
                    ]
                },
            }
    ...
    async def _create_conflict_metadata(self):
        ...
        conflict_type_str = self.current_session.user_responses.get(
