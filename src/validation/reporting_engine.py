...
class ValidationReportingEngine:
        ...
        try:
            ...
            report_data = ValidationReport(
                ...
                quality_gates={
                    ...
                    "gates_passed": sum(
                        1
                        for check in validation_result.basic_checks.values()
        ...
        try:
            ...
            report_data = ValidationReport(
                ...
                detailed_results={
                    ...
                    "constitutional_analysis": self._format_constitutional_context(
                        constitutional_context
                    ),
                    "validation_statistics": (
                        self.standard_validator.get_standard_validation_statistics()
                    ),
                },
                metrics_summary={
        ...
        try:
            ...
            report_data = ValidationReport(
                ...
                detailed_results={
                    ...
                    "validation_statistics": (
                        self.comprehensive_validator.get_comprehensive_validation_statistics()
                    ),
                },
                metrics_summary={
    ...
    def _generate_report_file(
        self, report_data: ValidationReport, output_format: str, report_id: str
    ) -> str:
        """Generate report file in specified format"""

        safe_report_id = os.path.basename(str(report_id))
        filename = f"{safe_report_id}.{output_format}"
...
**Report ID:** {report_data.report_id}  # noqa: W291
**Timestamp:** {report_data.timestamp}  # noqa: W291
...
"""
        ...
        if report_data.validation_level == "standard":
            md_content += (
                f"| Constitutional Score | "
                f"{report_data.overall_summary.get('constitutional_score', 0.0):.3f} |\n"
            )
            md_content += (
                f"| Feature Preservation Score | "
                f"{report_data.overall_summary.get('feature_preservation_score', 0.0):.3f} |\n"
            )
            md_content += (
                f"| Readiness Assessment | "
                f"{report_data.overall_summary.get('readiness_assessment', 'Unknown')} |\n"
            )
        elif report_data.validation_level == "comprehensive":
            md_content += (
                f"| Workflow Score | "
                f"{report_data.overall_summary.get('workflow_score', 0.0):.3f} |\n"
            )
            md_content += (
                f"| Performance Score | "
                f"{report_data.overall_summary.get('performance_score', 0.0):.3f} |\n"
            )
            md_content += (
                f"| Execution Readiness | "
                f"{report_data.overall_summary.get('execution_readiness', 'Unknown')} |\n"
            )
            md_content += (
                f"| Deployment Readiness | "
                f"{report_data.overall_summary.get('deployment_readiness', 'Unknown')} |\n"
            )
...
<head>
    ...
    <style>
        ...
        .summary-card {{ background: #fff; border: 1px solid #ddd; padding: 15px;
                        border-radius: 8px; }}
        .score {{ font-size: 2em; font-weight: bold; color: #333; }}
        .status-pass {{ color: #28a745; }}
        .status-warning {{ color: #ffc107; }}
        .status-fail {{ color: #dc3545; }}
        .recommendations {{ background: #f8f9fa; padding: 15px; border-radius: 8px;
                           margin: 20px 0; }}
        .gates-table {{ width: 100%; border-collapse: collapse; margin: 20px 0; }}
        .gates-table th, .gates-table td {{ border: 1px solid #ddd; padding: 8px;
                                           text-align: left; }}
        .gates-table th {{ background-color: #f2f2f2; }}
        .executive-summary {{ background: #e9ecef; padding: 20px; border-radius: 8px;
                             margin: 20px 0; }}
    </style>
</head>
<body>
...
"""
    ...
    def _generate_quick_executive_summary(self, result: QuickValidationResult) -> str:
        ...
        summary = (
            f"This quick validation assessment indicates {status_desc} readiness for "
            f"conflict resolution with an overall score of {result.overall_score:.3f}.\n\n"
            f"The resolution is {result.resolution_readiness.replace('_', ' ')} "
            "for implementation. Key findings include:\n\n"
        )
    ...
    def _generate_standard_executive_summary(self, result: StandardValidationResult) -> str:
        ...
        summary = (
            f"This standard validation assessment shows {status_desc} constitutional "
            f"compliance and feature preservation capabilities with an overall score of "
            f"{result.overall_score:.3f}.\n\n"
            "Key metrics indicate:\n"
            f"- Constitutional compliance: {const_score:.3f} "
            f"({'✅ Compliant' if const_score >= 0.7 else '⚠️ Needs attention'})\n"
            f"- Feature preservation: {preserv_score:.3f} "
            f"({'✅ Adequate' if preserv_score >= 0.7 else '⚠️ Needs attention'})\n"
            f"- Resolution readiness: {result.readiness_assessment.replace('_', ' ').title()}\n\n"
        )
    ...
    def _generate_comprehensive_executive_summary(
        ...
        quality_status = (
            ...
            if result.overall_score >= 0.9
                ...
                "✅ Good"
                if result.overall_score >= 0.8
                else "⚠️ Fair" if result.overall_score >= 0.7 else "❌ Poor"
            )
        )

        summary = (
            f"This comprehensive validation assessment indicates the resolution strategy is "
            f"{status_desc} for deployment with an overall quality score of "
            f"{result.overall_score:.3f}.\n\n"
            "Assessment Summary:\n"
            f"- Overall Quality: {result.overall_score:.3f} ({quality_status})\n"
            f"- Workflow Quality: {workflow_score:.3f}\n"
            f"- Performance Score: {perf_score:.3f}\n"
            f"- Execution Readiness: {result.execution_readiness.replace('_', ' ').title()}\n"
            f"- Deployment Readiness: {result.deployment_readiness.replace('_', ' ').title()}\n\n"
        )
    ...
    def _generate_trend_analysis(self, results: List[Any]) -> Dict[str, Any]:
        ...
        if len(scores) >= 3:
            ...
            if recent_avg > earlier_avg + 0.05:
                trend = "improving"
                message = (
                    f"Scores showing improvement trend "
                    f"(recent: {recent_avg:.3f}, earlier: {earlier_avg:.3f})"
                )
            elif recent_avg < earlier_avg - 0.05:
                trend = "declining"
                message = (
                    f"Scores showing declining trend "
                    f"(recent: {recent_avg:.3f}, earlier: {earlier_avg:.3f})"
                )
            else:
                trend = "stable"
                message = (
                    f"Scores remaining stable "
                    f"(recent: {recent_avg:.3f}, earlier: {earlier_avg:.3f})"
                )
        else:
        ...
        return {"trend": trend, "message": message, "score_progression": scores}
    ...
    def _generate_comparative_recommendations(self, results: List[Any]) -> List[str]:
        ...
        if len(scores) > 1:
            ...
            if score_variance > 0.1:
                recommendations.append(
                    "High variance in validation scores suggests inconsistent resolution quality"
                )
                recommendations.append(
                    "Consider standardizing resolution approach to achieve more consistent results"
                )
        ...
        if any(getattr(result, "status", {}).value == "fail" for result in results):
            recommendations.append(
                "Some validations failed quality gates - review and address failing criteria"
            )
    ...
    def _generate_comparative_executive_summary(self, analysis: Dict[str, Any]) -> str:
        ...
        summary = (
            f"Comparative analysis of {total_validations} validation results shows "
            f"{status_desc} resolution quality with an average score of {avg_score:.3f}.\n\n"
            "Key findings:\n"
        )
    ...
    def _generate_comparative_report_file(
        self,
        ...
        if output_format == "json":
            ...
            if detailed:
                ...
                for item in detailed:
                    md_content += (
                        f"| {item['validation_number']} | {item['validation_type']} | "
                        f"{item['overall_score']:.3f} | {item['status']} | "
                        f"{item['validation_time']:.2f}s |\n"
                    )
    ...
    def _format_constitutional_context(
        self, constitutional_context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _format_performance_context(
        self, performance_context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def get_reporting_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reports_generated": (
                len(os.listdir(self.output_directory))
