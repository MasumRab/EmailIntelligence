...
from ..resolution.constitutional_engine import ConstitutionalEngine, ValidationResult
...
class StandardValidator:
    ...
    def __init__(self):
        ...
        self.quality_gates = {
            ...
            "feature_preservation": {
                "threshold": 0.8,
                "description": "Feature preservation minimum threshold"
            },
            "overall_quality": {
                "threshold": 0.75,
                "description": "Overall quality minimum threshold"
            }
        }
        ...
        **kwargs
        ...
        try:
            ...
            quality_gates = self._assess_quality_gates(
                constitutional_score, preservation_score, overall_score
            )
            ...
            self._update_standard_stats(
                constitutional_score, preservation_score
            )
    ...
    async def _validate_constitutional_compliance(
        ...
        try:
            ...
            if specification_data:
                spec_compliance = await self._validate_specification_compliance(
                    resolution_strategy, specification_data
                )
        ...
        try:
            ...
            for step in resolution_strategy.steps:
                if step.validation_steps:
                    ...
                    if any(keyword in validation_text for keyword in [
                        "feature", "functionality", "preservation", "enhancement"
                    ]):
                        preservation_validation_steps += 1
            
            if preservation_validation_steps > 0:
                preservation_indicators.append(min(1.0, preservation_validation_steps / len(resolution_strategy.steps) * 0.7 + 0.3))
            else:
            ...
            if not hasattr(resolution_strategy, 'enhancement_preservation') or not resolution_strategy.enhancement_preservation:
                issues.append({
    ...
    def _assess_standard_readiness(
        ...
        if (constitutional_score >= 0.8 and 
            preservation_score >= 0.8 and 
            passed_gates >= 0.8):
            return "ready_for_implementation"
        elif (constitutional_score >= 0.7 and 
              preservation_score >= 0.7 and 
              passed_gates >= 0.6):
            return "needs_minor_improvements"
        elif (constitutional_score >= 0.6 and 
    ...
    def _update_standard_stats(self, constitutional_score: float, preservation_score: float):
        ...
        self.validation_stats["average_constitutional_score"] = (
            (current_const_avg * (total - 1) + constitutional_score) / total
        )
        self.validation_stats["average_preservation_score"] = (
            (current_preserv_avg * (total - 1) + preservation_score) / total
        )
        ...
        if constitutional_score >= self.quality_gates["constitutional_compliance"]["threshold"]:
            ...
            self.validation_stats["compliance_pass_rate"] = (
                (current_pass_rate * (total - 1) + 1) / total
            )
        else:
            current_pass_rate = self.validation_stats["compliance_pass_rate"]
            self.validation_stats["compliance_pass_rate"] = (
                (current_pass_rate * (total - 1)) / total
            )
