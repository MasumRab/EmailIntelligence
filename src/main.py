...
def create_system_status_tab():
    ...
    with gr.Tabs():
        with gr.TabItem("Overview"):
            with gr.Row():
                with gr.Column():
                    gr.Textbox(label="System Status", value="🟢 Online", interactive=False)
    ...
    def get_gmail_strategies():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if strategies:
                    strategy_list = "\n".join(
                        [f"• {s.get('name', 'Unknown')}: {s.get('description', '')}" for s in strategies]
                    )
        ...
        with gr.TabItem("Sync Control"):
            with gr.Row():
                with gr.Column():
                    ...
                    max_emails_input = gr.Number(label="Max Emails to Sync", value=100, minimum=1, maximum=1000)
        ...
        with gr.TabItem("Account Status"):
            with gr.Row():
                with gr.Column():
                    ...
                    gr.Textbox(label="Last Sync", interactive=False)
                    gr.Textbox(label="API Quota Status", interactive=False)
