...
class StrategyEfficiencyOptimizer:
            ...
            with ThreadPoolExecutor(max_workers=self.config.parallel_generation_threads) as executor:
                ...
                for group_key, requests in grouped_requests.items():
                    future = executor.submit(
                        self._process_strategy_group, group_key, requests
                    )
    ...
    async def _generate_strategies_optimized(
        ...
        try:
            ...
            strategy_types = generation_plan.get("strategy_types", ["conservative", "feature_preservation", "architectural"])
            ...
            with ThreadPoolExecutor(max_workers=self.config.parallel_generation_threads) as executor:
                ...
                for strategy_type in strategy_types:
                    future = executor.submit(
                        ...
                        conflict_data, strategy_type, context
                    )
    ...
    async def _calculate_efficiency_metrics(
        ...
        efficiency_score = (
            (1.0 - min(generation_time / 5.0, 1.0)) * 0.25 +  # Time component
            cache_hit_rate * 0.25 +  # Cache component
            parallel_efficiency * 0.25 +  # Parallel component
            selection_accuracy * 0.25  # Accuracy component
        )
    ...
    async def _calculate_batch_efficiency_metrics(
        ...
        total_strategies_generated = sum(
            len(r.get("strategies", [])) for r in successful_results
        )
