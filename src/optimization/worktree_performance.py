...
class WorktreePerformanceOptimizer:
    ...
    async def create_optimized_worktree(
        self, branch_name: str, worktree_path: str, base_path: str = "."
        ...
        try:
            logger.info("Creating optimized worktree", branch=branch_name, path=worktree_path)
            ...
            creation_result = await self._create_worktree_with_optimizations(branch_name, worktree_path, base_path)
            ...
            performance_metrics = await self._calculate_worktree_metrics(worktree_path, creation_time)
            ...
            logger.error("Exception during optimized worktree creation", branch=branch_name, error=str(e))
            return False, {"success": False, "creation_time": time.time() - start_time, "error": str(e)}

    async def cleanup_optimized_worktree(
        self, worktree_path: str, force_cleanup: bool = False
    ) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            logger.info("Cleaning up optimized worktree", path=worktree_path, force=force_cleanup)
            ...
            if cleanup_result["success"]:
                logger.info("Optimized worktree cleaned up successfully", path=worktree_path, cleanup_time=cleanup_time)
            else:
                logger.error(
                    "Failed to cleanup worktree", path=worktree_path, error=cleanup_result.get("error", "Unknown error")
                )

            return cleanup_result["success"], result

        except Exception as e:
            logger.error("Exception during optimized worktree cleanup", path=worktree_path, error=str(e))
            return False, {"success": False, "cleanup_time": time.time() - start_time, "error": str(e)}
            ...
            if expired_paths:
                with ThreadPoolExecutor(max_workers=min(len(expired_paths), 5)) as executor:
                    ...
                    for worktree_path in expired_paths:
                        future = executor.submit(self._run_async_cleanup, worktree_path)
                    ...
                    for worktree_path, future in cleanup_futures:
                        try:
                            ...
                            logger.error("Batch cleanup failed for worktree", path=worktree_path, error=str(e))
                            cleanup_results["failed_cleanups"] += 1
            ...
            for config_key, config_value in gc_settings:
                try:
                    ...
                    result = subprocess.run(
                        ["git", "config", config_key, config_value], capture_output=True, text=True, check=True
                    )
    ...
    async def _optimize_worktree_cache(self) -> Dict[str, Any]:
        ...
        try:
            ...
            temp_dirs = [os.path.join(os.getcwd(), ".git", "worktrees"), tempfile.gettempdir()]
    ...
    async def _optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        memory_optimizations = {"gc_forced": False, "index_cleared": False, "pack_files_optimized": False}

        try:
            # Force garbage collection
            gc_result = subprocess.run(
                ["git", "gc", "--force", "--aggressive"], capture_output=True, text=True, cwd=os.getcwd()
            )
            ...
            index_result = subprocess.run(
                ["git", "read-tree", "--empty"], capture_output=True, text=True, cwd=os.getcwd()
            )
    ...
    async def _optimize_disk_usage(self) -> Dict[str, Any]:
        ...
        disk_optimizations = {"pack_files_repacked": False, "garbage_collected": False, "tmp_directories_cleaned": 0}
        ...
        try:
            ...
            gc_result = subprocess.run(["git", "gc", "--prune=now"], capture_output=True, text=True, cwd=os.getcwd())
            ...
            try:
                ...
                with self._lock:
                    self.performance_history.append(
                        {"timestamp": datetime.now().isoformat(), "metrics": current_metrics}
                    )
    ...
    async def _pre_creation_optimizations(self) -> Dict[str, Any]:
        ...
        pre_opt = {"gc_performed": False, "pack_optimized": False, "index_prepared": False}

        try:
            # Pre-creation garbage collection
            gc_result = subprocess.run(["git", "gc", "--auto"], capture_output=True, text=True, cwd=os.getcwd())
    ...
    async def _create_worktree_with_optimizations(
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, cwd=base_path, timeout=300)  # 5 minute timeout
            ...
            return {"success": False, "error": "Worktree creation timed out"}
        except Exception as e:
    ...
    async def _post_creation_optimizations(self, worktree_path: str) -> Dict[str, Any]:
        ...
        post_opt = {"index_optimized": False, "config_optimized": False}

        try:
            # Optimize worktree index
            if os.path.exists(worktree_path):
                index_result = subprocess.run(
                    ["git", "update-server-info"], capture_output=True, text=True, cwd=worktree_path
                )
    ...
    async def _calculate_worktree_metrics(self, worktree_path: str, creation_time: float) -> WorktreePerformanceMetrics:
    ...
    async def _register_worktree(self, worktree_path: str, branch_name: str, metrics: WorktreePerformanceMetrics):
        ...
        with self._lock:
            ...
            self.performance_stats["peak_concurrent_usage"] = max(
                self.performance_stats["peak_concurrent_usage"], current_usage
            )
    ...
    async def shutdown(self):
        ...
        logger.info(
            "Worktree performance optimizer shutdown complete", cleaned_worktrees=cleanup_result.get("cleaned_up", 0)
        )
