...
class WorktreePerformanceOptimizer:
    ...
    async def _run_git_command(self, *args, cwd=None, timeout=None) -> Dict[str, Any]:
        ...
        try:
            ...
            for config_key, config_value in gc_settings:
                try:
                    ...
                    result = await self._run_git_command("git", "config", config_key, config_value)
    ...
    async def _optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            gc_result = await self._run_git_command(
                "git", "gc", "--force", "--aggressive", cwd=os.getcwd()
            )
            ...
            if status_result["returncode"] != 0:
                logger.warning("Skipping index clear - staged changes detected")
                memory_optimizations["index_cleared"] = False
            else:
                index_result = await self._run_git_command(
                    "git", "read-tree", "--empty", cwd=os.getcwd()
                )
    ...
    async def _optimize_disk_usage(self) -> Dict[str, Any]:
        ...
        try:
            # Repack pack files
            repack_result = await self._run_git_command(
                "git", "repack", "-a", "-d", "--depth=250", "--window=250", cwd=os.getcwd()
            )
            ...
            gc_result = await self._run_git_command("git", "gc", "--prune=now", cwd=os.getcwd())
    ...
    async def _pre_creation_optimizations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            gc_result = await self._run_git_command("git", "gc", "--auto", cwd=os.getcwd())
    ...
    async def _post_creation_optimizations(self, worktree_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(worktree_path):
                index_result = await self._run_git_command(
                    "git", "update-server-info", cwd=worktree_path
                )
    ...
    async def _calculate_worktree_metrics(
        self, worktree_path: str, creation_time: float
        ...
        performance_score = self._calculate_performance_score(
            creation_time, memory_usage_mb, disk_usage_mb
        )
    ...
    async def _register_worktree(
        self, worktree_path: str, branch_name: str, metrics: WorktreePerformanceMetrics
    ):
