...
def main():
    ...
    # Install sub-command
    install_parser = subparsers.add_parser("install", help="Install the command line tools for easy access.")
    install_parser.add_argument("--bin-dir", type=str, help="Directory to install the command (default: ~/.local/bin)")
    install_parser.add_argument("--alias", type=str, help="Custom alias name (default: git-verifier)")
    install_parser.add_argument("--force", action="store_true", help="Overwrite existing installations")
    ...
    if args.command == "install":
        ...
        return handle_detect_rebased_command(args)
...
def handle_install_command(args):
    """
    Handle the install command - creates command-line tool installations.
    """
    try:
        # Determine the bin directory
        bin_dir = args.bin_dir or os.path.expanduser("~/.local/bin")
        alias = args.alias or "git-verifier"
        
        # Ensure the bin directory exists
        os.makedirs(bin_dir, exist_ok=True)
        
        # Check if the target already exists
        target_path = os.path.join(bin_dir, alias)
        if os.path.exists(target_path) and not args.force:
            print(f"Installation already exists at {target_path}. Use --force to overwrite.")
            return 1
...
"{python_path}" "{project_root}/src/cli/main.py" %*
"""
        else:
...
"""
        ...
        print(f"Successfully installed '{alias}' to {target_path}")
        
        # Check if bin_dir is in PATH
        if bin_dir not in os.environ.get("PATH", ""):
            print(f"\nNote: {bin_dir} is not in your PATH.")
            if platform.system() == "Windows":
                print("You may need to add it to your PATH in System Properties > Environment Variables.")
            else:
        ...
        if verify_installation(target_path, alias):
            print(f"✓ Installation verified! Try running: {alias} --help")
            return 0
        else:
            print(f"✗ Installation verification failed. Check the installation.")
            return 1
...
def verify_installation(install_path, alias):
    """
    Verify that the installation works correctly.
    """
    try:
        # Test with --help
        result = subprocess.run([install_path, "--help"], 
...
async def handle_detect_rebased_command(args):
    """
    ...
    try:
        ...
        if args.since:
            ...
            # comparing with the timestamp of reflog entries.
