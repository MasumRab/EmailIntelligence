...
class EnhancedPerformanceMonitor:
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            cpu_percent = psutil.cpu_percent(
                interval=0.1
            )  # Short interval for more accurate reading
    ...
    def get_performance_summary(
        self, operation: str = None, hours: int = 24
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_metrics = [
                m.get("system_memory_percent", 0)
                for m in metrics
                if "system_memory_percent" in m
            ]

            avg_cpu = sum(cpu_metrics) / len(cpu_metrics) if cpu_metrics else 0
            avg_memory = (
                sum(memory_metrics) / len(memory_metrics) if memory_metrics else 0
            )
