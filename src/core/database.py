...
class DatabaseManager(DataSource):
    ...
    def _get_searchable_text(self, email: Dict[str, Any]) -> str:
        ...
        subject = email.get(FIELD_SUBJECT) or ""
        sender = email.get(FIELD_SENDER) or ""
        sender_email = email.get(FIELD_SENDER_EMAIL) or ""
        return f"{subject} {sender} {sender_email}".lower()
    ...
    async def get_emails(
        ...
        if is_unread is not None:
            source_emails = [
                e for e in source_emails if e.get(FIELD_IS_UNREAD) == is_unread
            ]
