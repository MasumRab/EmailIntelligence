...
class DatabaseManager(DataSource):
    ...
    async def search_emails_with_limit(self, search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        # Use sorted emails so we can return the top 'limit' matches immediately
        for email_light in self._get_sorted_emails():
        ...
        # We already have sorted emails (since we iterated _get_sorted_emails), so no need to sort again
