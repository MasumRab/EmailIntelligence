...
class DatabaseManager(DataSource):
    ...
    def __init__(self, config: DatabaseConfig = None):
        ...
        self._search_index: Dict[int, str] = {}  # Optimization: ID -> lowercased search text
    ...
    async def search_emails_with_limit(self, search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        # Optimization: Use pre-computed search index to avoid repetitive lowercasing
        for email_light in self.emails_data:
            email_id = email_light.get(FIELD_ID)

            # Check in-memory index first (fastest)
            # Use cached search string if available, otherwise compute and cache
            search_text = self._search_index.get(email_id)
            if search_text is None:
                search_text = self._get_searchable_text(email_light)
                self._search_index[email_id] = search_text
            ...
            # If not found in light data, check heavy content on disk (slower)
            content_path = self._get_email_content_path(email_id)
            if os.path.exists(content_path):
                try:
                    with gzip.open(content_path, "rt", encoding="utf-8") as f:
