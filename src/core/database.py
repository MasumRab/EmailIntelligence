...
class DatabaseManager(DataSource):
    ...
    def _get_searchable_text(self, email: Dict[str, Any]) -> str:
        """Generates a lowercase searchable string from email fields. Optimized with f-strings."""
        return f"{str(email.get(FIELD_SUBJECT) or '')} {str(email.get(FIELD_SENDER) or '')} {str(email.get(FIELD_SENDER_EMAIL) or '')}".lower()
