...
class DatabaseManager(DataSource):
    ...
    async def search_emails_with_limit(self, search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        # Check query cache
        search_key = f"search:{search_term_lower}:{limit}"
        cached_result = self.caching_manager.get_query_result(search_key)
        if cached_result is not None:
            logger.info(f"Query cache hit for term: '{search_term_lower}'")
        ...
        # Cache the result
        self.caching_manager.put_query_result(search_key, result)
