...
class DatabaseManager(DataSource):
    ...
    def __init__(self, config: DatabaseConfig = None):
        ...
        # Search index: Maps email ID to lowercased search text (subject + sender + sender_email)
        # Optimized for O(1) lookup during search iteration
    ...
    def _get_search_text(self, email: Dict[str, Any]) -> str:
        """
        Generates the search text for an email.
        Concatenates subject, sender, and sender_email with a null separator
        to allow fast substring search without cross-field false positives.
        """
        return (
            (email.get(FIELD_SUBJECT) or "").lower() + "\0" +
            (email.get(FIELD_SENDER) or "").lower() + "\0" +
            (email.get(FIELD_SENDER_EMAIL) or "").lower()
        )
    ...
    async def update_email_by_message_id(
        ...
        if changed_fields:
            ...
            self.email_search_index[email_id] = self._get_search_text(email_to_update)
            idx = next(
    ...
    async def search_emails_with_limit(self, search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for email_light in self.emails_data:
            email_id = email_light.get(FIELD_ID)
            # Use search index for fast in-memory lookup
