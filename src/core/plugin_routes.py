...
async def execute_plugin_method(
    ...
    try:
        ...
        result = await manager.execute_plugin_method(plugin_id, method_name, **params)
...
async def get_registered_hooks(manager: PluginManager = Depends(get_plugin_manager)):
