...
class MemoryCacheBackend(CacheBackendInterface):
    """In-memory cache backend using LRU strategy via OrderedDict"""

    def __init__(self, config: CacheConfig):
        self.config = config
        # OrderedDict stores keys in insertion order.
        # We will use move_to_end(last=True) to mark recently used items.
        # The oldest items will be at the beginning (popitem(last=False)).
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        if key in self._cache:
            ...
            # Update access order for LRU (O(1))
