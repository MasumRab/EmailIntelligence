...
class MemoryCacheBackend(CacheBackendInterface):
    """In-memory cache backend using LRU strategy with O(1) complexity"""
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        # Ensure it's marked as recently used (redundant if just inserted, but safe)
        # For new keys, they are added at the end. For existing, we already moved.
        # But if we just rely on insert, existing keys might not move if we don't call move_to_end first or after.
        # So we can just move_to_end after set/update to be sure.
        self._cache.move_to_end(key)

        # Enforce max items limit (LRU eviction)
        while len(self._cache) > self.config.max_memory_items:
            # popitem(last=False) removes from the beginning (FIFO/LRU)
