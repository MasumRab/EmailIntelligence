...
class ModernAIEngine(BaseAIEngine):
    ...
    async def analyze_email(
        ...
        try:
            ...
            # Pre-compute lowercase version for efficient fallback analysis
            ...
            result_data = {
                ...
                "keywords": self._extract_keywords(full_text_lower),
    ...
    async def _analyze_sentiment(
        self, text: str, text_lower: Optional[str] = None
        ...
        return self._simple_sentiment_analysis(text, text_lower)
    ...
    async def _analyze_topics(
        ...
        return self._rule_based_topics(text, text_lower)
    ...
    async def _analyze_intent(
        ...
        return self._simple_intent_analysis(text, text_lower)
    ...
    async def _analyze_urgency(
        ...
        return self._simple_urgency_analysis(text, text_lower)

    def _simple_sentiment_analysis(
        self, text: str, text_lower: Optional[str] = None
    ) -> Dict[str, Any]:
        """Simple keyword-based sentiment analysis."""
        # Use pre-computed lower text if available
        text_processed = text_lower if text_lower is not None else text.lower()

        positive_count = sum(1 for word in POSITIVE_WORDS if word in text_processed)
        negative_count = sum(1 for word in NEGATIVE_WORDS if word in text_processed)
    ...
    def _rule_based_topics(
        ...
        for topic, keywords in TOPIC_PATTERNS.items():
            if any(keyword in text_processed for keyword in keywords):
                topics.append(topic)
    ...
    def _simple_intent_analysis(
        ...
        if any(word in text_processed for word in INTENT_KEYWORDS["question"]):
            intent_type = "question"
        elif any(word in text_processed for word in INTENT_KEYWORDS["request"]):
            intent_type = "request"
        elif any(word in text_processed for word in INTENT_KEYWORDS["apology"]):
            intent_type = "apology"
        elif any(word in text_processed for word in INTENT_KEYWORDS["gratitude"]):
    ...
    def _simple_urgency_analysis(
        self, text: str, text_lower: Optional[str] = None
    ) -> Dict[str, Any]:
        """Simple urgency analysis."""
        text_processed = text_lower if text_lower is not None else text.lower()

        has_urgency = any(indicator in text_processed for indicator in URGENCY_INDICATORS)
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        """Extract important keywords from text. Assumes text is already lower case or suitable."""
