...
INTENT_KEYWORDS_QUESTION = {"?", "what", "how", "when", "where", "why"}
INTENT_KEYWORDS_REQUEST = {"please", "can you", "would you", "help"}
...
class ModernAIEngine(BaseAIEngine):
    ...
    async def analyze_email(
        ...
        try:
            ...
            # Compute lower case text once to avoid repetition in helper methods
            text_lower = full_text.lower()

            # Perform analysis using available models
            sentiment = await self._analyze_sentiment(full_text, text_lower)
            topics = await self._analyze_topics(full_text, text_lower)
            intent = await self._analyze_intent(full_text, text_lower)
            urgency = await self._analyze_urgency(full_text, text_lower)
            ...
            result_data = {
                ...
                "keywords": self._extract_keywords(full_text, text_lower),
    ...
    async def _analyze_sentiment(self, text: str, text_lower: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self._model_manager and hasattr(self._model_manager, "get_sentiment_model"):
                model = await self._model_manager.get_sentiment_model()
                if model:
        ...
        return await self._simple_sentiment_analysis(text_lower or text.lower())
    ...
    async def _analyze_topics(self, text: str, text_lower: Optional[str] = None) -> List[str]:
        ...
        try:
            if self._model_manager and hasattr(self._model_manager, "get_topic_model"):
                model = await self._model_manager.get_topic_model()
                if model:
        ...
        return await self._rule_based_topics(text_lower or text.lower())
    ...
    async def _analyze_intent(self, text: str, text_lower: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self._model_manager and hasattr(self._model_manager, "get_intent_model"):
                model = await self._model_manager.get_intent_model()
                if model:
                    return await model.analyze(text)
        except Exception as e:
            logger.debug(f"Intent model analysis failed: {e}")

        return await self._simple_intent_analysis(text_lower or text.lower())
    ...
    async def _analyze_urgency(self, text: str, text_lower: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self._model_manager and hasattr(self._model_manager, "get_urgency_model"):
                model = await self._model_manager.get_urgency_model()
                if model:
                    return await model.analyze(text)
        except Exception as e:
            logger.debug(f"Urgency model analysis failed: {e}")

        return await self._simple_urgency_analysis(text_lower or text.lower())

    async def _simple_sentiment_analysis(self, text_lower: str) -> Dict[str, Any]:
    ...
    async def _rule_based_topics(self, text_lower: str) -> List[str]:
    ...
    async def _simple_intent_analysis(self, text_lower: str) -> Dict[str, Any]:
        """Simple intent analysis based on keywords."""
        # Using module level constants
        if any(word in text_lower for word in INTENT_KEYWORDS_QUESTION):
            intent_type = "question"
        elif any(word in text_lower for word in INTENT_KEYWORDS_REQUEST):
            intent_type = "request"
        elif any(word in text_lower for word in INTENT_KEYWORDS_APOLOGY):
            intent_type = "apology"
        elif any(word in text_lower for word in INTENT_KEYWORDS_GRATITUDE):
    ...
    async def _simple_urgency_analysis(self, text_lower: str) -> Dict[str, Any]:
    ...
    def _extract_keywords(self, text: str, text_lower: Optional[str] = None) -> List[str]:
        ...
        # Use constant set for O(1) lookups
