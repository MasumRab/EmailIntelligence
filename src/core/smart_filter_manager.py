...
class SmartFilterManager:
    ...
    async def _apply_filter_to_email(self, filter_obj: EmailFilter, email: Dict[str, Any], precomputed: Dict[str, Any] = None) -> bool:
        """Applies a single filter's criteria to an email."""
        criteria = filter_obj.criteria
        
        # Use precomputed values if available to avoid redundant processing
        ...
        if "sender_domain" in criteria:
            if "sender_domain" in precomputed:
                domain = precomputed["sender_domain"]
            else:
        ...
        if "subject_keywords" in criteria:
            if "subject_lower" in precomputed:
        ...
        if "content_keywords" in criteria:
            if "content_lower" in precomputed:
        ...
        if "from_patterns" in criteria:
            if "sender_lower" in precomputed:
                sender_email = precomputed["sender_lower"]
            else:
    ...
    async def apply_filters_to_email(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Pre-compute invariant values to optimize filter matching
        ...
        for filter_obj in active_filters:
            try:
                if await self._apply_filter_to_email(filter_obj, email_data, precomputed=precomputed):
