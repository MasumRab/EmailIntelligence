...
class _EmailContext:
    ...
    during filter application loop. Uses lazy evaluation for expensive strings.
    """

    def __init__(self, email: Dict[str, Any], domain_extractor):
        self.email = email
        self._domain_extractor = domain_extractor
        self._sender_domain = None
        self._subject_lower = None
        self._content_lower = None
        self._sender_lower = None
        self._sender_email = self.email.get(
            "sender_email", self.email.get("sender", "")
        )

    @property
    def sender_domain(self) -> str:
        if self._sender_domain is None:
            self._sender_domain = self._domain_extractor(self._sender_email)
    ...
    def content_lower(self) -> str:
        if self._content_lower is None:
            self._content_lower = self.email.get(
                "content", self.email.get("body", "")
            ).lower()
    ...
    def sender_lower(self) -> str:
        if self._sender_lower is None:
            self._sender_lower = self._sender_email.lower()
...
class SmartFilterManager:
    ...
    def _load_filter_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "financial_documents": {
                "criteria": {
                    "subject_keywords": ["invoice", "statement", "payment", "bill"]
                },
    ...
    async def create_intelligent_filters(
    ...
    async def _create_filters_from_templates(
    ...
    def _is_automated_email(self, email: Dict[str, Any]) -> bool:
        ...
        return any(
            ind in sender for ind in ["noreply", "automated", "notification", "admin"]
        )

    async def _should_create_filter_async(
        ...
        for keyword in template_keywords:
            if any(
                keyword.lower() in pattern_kw.lower()
                for pattern_kw in patterns.get("subject_keywords", [])
            ):
                return True
        return False
    ...
    async def _create_custom_filters(
        self, patterns: Dict[str, Any]
    ) -> List[EmailFilter]:
    ...
    async def _evaluate_filter_for_pruning(self, filter_obj: EmailFilter) -> str:
        ...
        if (
            ...
            < self.pruning_criteria["effectiveness_threshold"]
        ):
            return "disable"
    ...
    async def _apply_filter_to_email(
        ...
        if not isinstance(context, _EmailContext):
            ctx = _EmailContext(email=context, domain_extractor=self._extract_domain)
        else:
            ctx = context
    ...
    async def apply_filters_to_email(
        ...
        # Use lazy evaluation context to avoid repeated processing in the loop
        # and skip expensive evaluations (like content.lower()) if no filters need them.
        email_context = _EmailContext(
            email=email_data, domain_extractor=self._extract_domain
        )
    ...
    async def _batch_update_filter_usage(self, filters: List[EmailFilter]):
        """
        Updates usage statistics for multiple filters efficiently.

        This method updates the database and in-memory objects without
