...
class EmailFilter:
    ...
    def __post_init__(self):
        """Initialize precomputed values for faster filtering."""
        if self._precomputed is None:
            self._precomputed = {}
            if self.criteria:
                if "subject_keywords" in self.criteria:
                    self._precomputed["subject_keywords_lower"] = [k.lower() for k in self.criteria["subject_keywords"]]
                if "content_keywords" in self.criteria:
                    self._precomputed["content_keywords_lower"] = [k.lower() for k in self.criteria["content_keywords"]]
...
class SmartFilterManager:
    ...
    async def _apply_filter_to_email(self, filter_obj: EmailFilter, context: Union[Dict[str, Any], '_EmailContext']) -> bool:
        ...
        if "subject_keywords" in criteria:
            # Optimization: check if any keyword matches using precomputed values
            keywords = filter_obj._precomputed.get("subject_keywords_lower") if filter_obj._precomputed else [k.lower() for k in criteria["subject_keywords"]]
            if not any(keyword in ctx.subject_lower for keyword in keywords):
                return False

        # Check content keywords
        if "content_keywords" in criteria:
            keywords = filter_obj._precomputed.get("content_keywords_lower") if filter_obj._precomputed else [k.lower() for k in criteria["content_keywords"]]
    ...
    async def update_filter(self, filter_id: str, **kwargs) -> bool:
        ...
        # If criteria changed, recompute optimized values
        if "criteria" in kwargs:
            existing_filter._precomputed = None
            existing_filter.__post_init__()
