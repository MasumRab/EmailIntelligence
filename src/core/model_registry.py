    ...
    async def _load_sklearn_model(self, metadata: ModelMetadata) -> Optional[Any]:
        ...
        try:
            ...
            if model_path.exists():
                if not verify_model_safety(
                    model_path, getattr(metadata, "expected_hash", None)
                ):
                    logger.error(
    ...
    async def _validate_model_file(self, metadata: ModelMetadata) -> Dict[str, Any]:
        ...
        try:
            if metadata.framework == "sklearn":
                ...
                if not verify_model_safety(
                    model_file, getattr(metadata, "expected_hash", None)
                ):
                    return {
                        "passed": False,
                        "issues": ["Model path or signature validation failed"],
                    }
