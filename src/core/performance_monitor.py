...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        # Create logs directory if it contains a path
        if self.log_file.parent.name:
    ...
    def log_performance(self, log_entry: Dict[str, Any]) -> None:
        """
        Legacy method to support the @log_performance decorator.
        Maps the dictionary entry to the optimized record_metric method.
        """
        try:
            ...
            # Convert seconds to ms for consistency with other metrics
            ...
            # Fallback to avoid crashing even if extraction fails
            logger.debug(f"Error recording legacy performance metric: {e}")
        ...
        try:
            with self._aggregation_lock:
                ...
                if not self._aggregated_metrics:
                    return
                metrics_to_write = list(self._aggregated_metrics.values())
...
# Global performance monitor instance - keeping the name for compatibility
performance_monitor = PerformanceMonitor()
# Alias for the new name if used elsewhere
OptimizedPerformanceMonitor = PerformanceMonitor
