...
class ConflictBlock(BaseModel):
    ...
    conflict_marker_type: str = Field(
        "git", description="Type of conflict marker (git, diff3, etc.)"
    )
...
class Conflict(BaseModel):
    ...
    details: Dict[str, Any] = Field(
        default_factory=dict, description="Type-specific conflict details"
    )
    blocks: List[ConflictBlock] = Field(
        default_factory=list, description="Specific conflict blocks if applicable"
    )
...
class AnalysisResult(BaseModel):
    ...
    impact_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="Impact on other components"
    )
    constitutional_violations: List[str] = Field(
        default_factory=list, description="Violated constitutional rules"
    )
