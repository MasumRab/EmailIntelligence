...
class PullRequest(Node):
    ...
    estimated_resolution_time: int = Field(
        default=0, description="Estimated resolution time in minutes"
    )
...
class Developer(Node):
    ...
    conflict_history_ids: List[str] = Field(
        default_factory=list, description="Conflict history IDs"
    )
...
class PRResolution(BaseModel):
    ...
    applied_at: datetime = Field(
        default_factory=datetime.utcnow, description="Application timestamp"
    )
