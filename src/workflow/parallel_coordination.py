...
class ParallelCoordination:
    ...
    def _has_circular_dependency(
        self, task_id: str, all_tasks: List[str], visited: Set[str]
    ) -> bool:
        """Check for circular dependencies"""
        ...
        if task_id in self.task_master.tasks:
            ...
            for dep_id in task.dependencies:
                if dep_id in all_tasks and self._has_circular_dependency(
                    dep_id, all_tasks, visited.copy()
                ):
                    return True
    ...
    def _assess_parallel_viability(
        self, task_ids: List[str], dependency_graph: Dict[str, Any]
    ) -> bool:
    ...
    def _generate_dependency_recommendations(self, validation_result: Dict[str, Any]) -> List[str]:
        ...
        if validation_result["circular_dependencies"]:
            recommendations.append(
                "🔄 BREAK: Resolve circular dependencies before parallel execution"
            )

        if validation_result["blocked_tasks"]:
            recommendations.append(
                "🚫 UNBLOCK: Complete blocked tasks or resolve blocking dependencies"
            )

        if not validation_result["parallel_execution_viable"]:
            recommendations.append(
                "🔄 SEQUENTIAL: Consider sequential execution for dependent tasks"
            )
        ...
        for task_id, task in execution_tasks:
            try:
                ...
                await self.task_master.update_task_status(
                    task_id,
                    TaskStatus.DONE,
                    f"Completed in parallel coordination {session_id}",
                )
                ...
                await self.task_master.update_task_status(
                    task_id,
                    TaskStatus.BLOCKED,
                    f"Failed in parallel coordination: {str(e)}",
                )
    ...
    async def cleanup_coordination_session(self, session_id: str) -> bool:
        ...
        try:
            if session_id in self.active_coordinations:
                ...
                for task_id in coordination["tasks"]:
                    allocation_keys = [
                        key for key in self.resource_allocations.keys() if key.startswith(task_id)
                    ]
...
def get_coordination_status(self, session_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "active_barriers": [
            ...
            for barrier_id in coordination["barriers"]
            if (
                barrier_id in self.coordination_barriers
                and self.coordination_barriers[barrier_id].triggered_at is None
            )
        ],
    }
