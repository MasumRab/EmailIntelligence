...
class GraphQueryBuilder:
    ...
    def select(
        self, node_type: QueryNodeType, alias: str, properties: Optional[List[QueryProperty]] = None
    ) -> "GraphQueryBuilder":
        """Add SELECT clause"""
        self.select_clauses.append(
            {"type": "select", "node_type": node_type.value, "alias": alias, "properties": properties or []}
        )
        return self

    def match(self, pattern: str, parameters: Optional[Dict[str, Any]] = None) -> "GraphQueryBuilder":
        """Add MATCH clause"""
        self.match_clauses.append({"pattern": pattern, "parameters": parameters or {}})
        return self
    ...
    def where_raw(self, condition: str, parameters: Optional[Dict[str, Any]] = None) -> "GraphQueryBuilder":
        ...
        self.where_clauses.append({"type": "raw", "condition": condition, "parameters": parameters or {}})
        return self
    ...
    def order_by(self, property_name: str, direction: str = "ASC") -> "GraphQueryBuilder":
        """Add ORDER BY clause"""
        self.order_by_clauses.append({"property": property_name, "direction": direction})
        return self
    ...
    def return_clause(self, items: List[Union[str, QueryProperty]], distinct: bool = False) -> "GraphQueryBuilder":
        """Add RETURN clause"""
        self.return_clauses.append({"items": items, "distinct": distinct})
        return self
    ...
    def param(self, name: str, value: Any, type: str = "string") -> "GraphQueryBuilder":
    ...
    def build(self) -> str:
        ...
        for select in self.select_clauses:
            ...
            if select["properties"]:
                props = ", ".join([f"n.{prop.name} as {prop.alias or prop.name}" for prop in select["properties"]])
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameters": [{"name": p.name, "value": p.value, "type": p.type} for p in self.parameters.values()],
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GraphQueryBuilder":
        ...
        for param_data in data.get("parameters", []):
            builder.parameters[param_data["name"]] = QueryParameter(
                name=param_data["name"], value=param_data["value"], type=param_data["type"]
            )
...
class ConflictQueryBuilder(GraphQueryBuilder):
    ...
    def find_conflicts_by_severity(self, severities: List[str]) -> "ConflictQueryBuilder":
        ...
        return (
            self.select(QueryNodeType.CONFLICT, "conflict")
...
class PRQueryBuilder(GraphQueryBuilder):
    ...
    def find_prs_by_status(self, statuses: List[str]) -> "PRQueryBuilder":
        """Find PRs by status"""
        return (
            self.select(QueryNodeType.PULL_REQUEST, "pr")
    ...
    def find_similar_prs(self, pr_id: str, limit: int = 10) -> "PRQueryBuilder":
        ...
        return (
            self.select(QueryNodeType.PULL_REQUEST, "pr1")
...
class AnalyticsQueryBuilder(GraphQueryBuilder):
    ...
    def calculate_centrality(self, node_type: QueryNodeType) -> "AnalyticsQueryBuilder":
        ...
        return (
            self.select(node_type, "node")
            .with_clause("node, size((node)-[:DEPENDS_ON]->(:PullRequest)) as dependency_count")
            .return_clause(
                [QueryProperty("node", alias="id"), QueryProperty("dependency_count", alias="centrality_score")]
            )
    ...
    def find_bottleneck_prs(self) -> "AnalyticsQueryBuilder":
        ...
        return (
            ...
            .return_clause([QueryProperty("pr", alias="id"), QueryProperty("blocking_count", alias="bottleneck_score")])
...
class RealTimeQueryMonitor:
    ...
    async def subscribe_to_conflicts(self, handler: callable) -> str:
        ...
        self.subscribed_queries[subscription_id] = {"query": query, "handler": handler, "type": "conflict"}
    ...
    async def check_for_changes(self):
        ...
        for subscription_id, query_info in self.subscribed_queries.items():
            try:
                results = await connection_manager.execute_query(query_info["query"], query_info.get("parameters", {}))

                if results:
                    await query_info["handler"](results, subscription_id)
            except Exception as e:
                logger.error("Error checking for changes", subscription_id=subscription_id, error=str(e))
