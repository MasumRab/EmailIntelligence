...
from .query_builder import query_builder, conflict_query_builder, pr_query_builder
from .scoring import conflict_scoring_engine
from .specialized import specialized_engine, find_file_conflicts, detect_dependency_conflicts, find_architecture_violations
from .cache import graph_cache, graph_monitor, cache_traversal_operation, get_cached_traversal_result
...
from ...models.graph_entities import PullRequest, Conflict, ConflictType, ConflictSeverity
...
class GraphTraversalSystem:
    ...
    async def analyze_pull_request(
        ...
        try:
            ...
            result = {
                ...
                "recommendations": await self._generate_recommendations(conflicts, conflict_scores, specialized_results)
            }
    ...
    async def analyze_pull_requests_batch(
        ...
        logger.info("Batch PR analysis completed",
                   total_prs=len(pr_ids),
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        if performance_summary.get("metrics", {}).get("graph_query_execution_time_general", {}).get("avg", 0) > self.performance_thresholds["traversal_query"]:
    ...
    async def optimize_system_performance(self) -> Dict[str, Any]:
        ...
        avg_query_time = performance_summary.get("metrics", {}).get("graph_query_execution_time_general", {}).get("avg", 1.0)
    ...
    async def _analyze_pr_traversal(self, pr: PullRequest) -> Optional[Any]:
        ...
        try:
            ...
            # Cache result
            await self.cache_manager.cache_traversal_result(
                "pr_analysis", {"pr_id": pr.id}, result
            )
