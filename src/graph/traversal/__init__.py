...
class GraphTraversalEngine:
    ...
    async def breadth_first_search(
        ...
        while queue:
            ...
            if current_node.distance >= max_depth:
                continue
        ...
        result.metadata = {
            ...
            "found_target": target_node_id is not None
            and any(p.id == target_node_id for p in result.visited_nodes),
        }
            ...
            for neighbor_id, rel_type in adjacent:
                if neighbor_id not in visited:
                    ...
                    if neighbor_node:
                        ...
                        if await dfs_recursive(neighbor_node, depth + 1):
                            return True
                        ...
                        if neighbor_id not in forward_visited:
                            ...
                            if neighbor_node:
                                ...
                                if neighbor_id in backward_visited:
                                    ...
                                    self.performance_stats["total_traversals"] += 1
                                    logger.info(
                                        "Bidirectional search found path",
    ...
    async def detect_cycles(
        ...
        async def dfs_cycle_detection(node: GraphNode, path: List[str]):
            if node.id in recursion_stack:
                ...
                result.add_cycle(
                    [await self.get_node(node_id, "PullRequest") for node_id in cycle_path]
                )
                return
            ...
            adjacent = await self.get_adjacent_nodes(
                node.id, relationship_types, TraversalDirection.FORWARD
            )
    ...
    def _reconstruct_bidirectional_path(
        self,
        meeting_node_id: str,
        forward_parents: Dict[str, str],
        backward_parents: Dict[str, str],
    ) -> List[GraphNode]:
