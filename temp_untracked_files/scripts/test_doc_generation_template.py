...
def test_template_registration():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert retrieved_template.template_id == "test-template", "Template ID should match"
        assert retrieved_template.template_name == "Test Template", "Template name should match"
...
def test_document_generation():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert retrieved_document.document_id == "test-doc-1", "Document ID should match"
...
def test_section_content_update():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert updated_section.content == "# Introduction\n\nThis is a test introduction.", "Content should match"
...
def test_document_progress():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if document.sections:
            ...
            updated_progress = template_manager.get_document_progress("progress-test-doc")

            print(f"Updated progress: {updated_progress}")

            # Verify progress was updated
            assert updated_progress['completed_sections'] == 1, "Should have 1 completed section"
            assert updated_progress['progress_percentage'] > 0, "Progress percentage should be > 0"
...
def test_template_complexity():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert complexity['total_sections'] >= 0, "Total sections should be non-negative"
        assert complexity['total_dependencies'] >= 0, "Total dependencies should be non-negative"
        assert complexity['max_dependency_depth'] >= 0, "Max dependency depth should be non-negative"
        assert complexity['complexity_score'] >= 0, "Complexity score should be non-negative"
...
def test_template_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert loaded_document.title == "Persistence Test Document", "Title should match"

        # Check if section was loaded correctly
        if loaded_document.sections:
            loaded_section = loaded_document.sections[0]
            assert loaded_section.content == "# Persistent Content\n\nThis content should persist.", "Content should match"
            assert loaded_section.agent_id == "persistence-agent", "Agent ID should match"
