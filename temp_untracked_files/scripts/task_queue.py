...
class Agent:
    def __init__(self, name: str, capabilities: List[str], max_concurrent_tasks: int = 3):
        self.name = name
...
class TaskQueue:
    ...
    def get_next_task(self, agent: Agent) -> Optional[Task]:
        ...
        sorted_tasks = sorted(self.tasks, key=lambda t: (
            t.priority.value,
            len([d for d in t.dependencies if not self._is_dependency_completed(d)])
        ))

        for task in sorted_tasks:
            if (task.status == TaskStatus.PENDING and
                agent.can_handle_task(task) and
    ...
    def get_queue_stats(self) -> Dict:
        ...
        for name, queue in self.queues.items():
            stats[name] = {
                ...
                'assigned': len([t for t in queue.tasks if t.status == TaskStatus.ASSIGNED]),
