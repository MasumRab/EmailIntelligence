...
class AtomicCommitManager:
    ...
    def create_commit_group(self, name: str, description: str, files: List[str],
                          worktrees: List[str]) -> str:
        """Create a new atomic commit group."""
        # Generate unique group ID
        group_id = hashlib.sha256(f"{name}{datetime.now().isoformat()}".encode()).hexdigest()[:16]
        ...
        try:
            ...
            for worktree in group.worktrees:
                ...
                if worktree_path.exists():
                    commit_hash = self._commit_to_worktree(worktree_path, group.files, commit_message)
                    if commit_hash:
    ...
    def _commit_to_worktree(self, worktree_path: Path, files: List[str],
    ...
    def get_statistics(self) -> Dict:
        ...
        pending_groups = len([g for g in self.commit_groups.values() if g.status == "pending"])
        completed_groups = len([g for g in self.commit_groups.values() if g.status == "completed"])
        failed_groups = len([g for g in self.commit_groups.values() if g.status == "failed"])
        rolled_back_groups = len([g for g in self.commit_groups.values() if g.status == "rolled_back"])
        ...
        return {
            ...
            "success_rate": (completed_groups / total_groups * 100) if total_groups > 0 else 0
        }
