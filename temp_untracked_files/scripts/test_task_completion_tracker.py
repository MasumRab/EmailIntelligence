...
def test_task_completion_tracking():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert abs(overall_rate - 66.67) < 0.01, f"Expected ~66.67%, got {overall_rate}%"
...
def test_completion_trends():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert abs(overall_rate - 50.0) < 0.01, f"Expected 50% overall, got {overall_rate}%"
...
def test_performance_reports():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert agent_report['completion_rate'] == 100.0, "Agent should have 100% completion rate"
        ...
        assert task_report['completion_rate'] == 100.0, "Task type should have 100% completion rate"
        ...
        print(f"System completion rate: {system_report['overall_completion_rate']:.1f}%")
        
        assert system_report['total_tasks_tracked'] == 5, "System should have 5 tracked tasks"
...
def test_performance_alerts():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert agent_alerts[0]['completion_rate'] == 10.0, "Agent completion rate should be 10%"
...
def test_tracking_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert abs(initial_rate - loaded_rate) < 0.01, "Completion rate should match after loading"
