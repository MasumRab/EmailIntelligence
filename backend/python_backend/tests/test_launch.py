...
def test_check_python_version_too_high():
    ...
    with patch("sys.version_info", (3, 13, 0)):
        assert check_python_version() is True  # Still returns True but should log a warning
...
def test_is_venv_available_unix(mock_exists):
    ...
    if platform.system() != "Windows":
        with patch("os.name", "posix"):
            ...
            # The first call is to the venv dir, the second to the python executable
...
def test_prepare_environment_existing_compatible_venv(
    ...
    # Mock the venv Python version check
...
def test_run_application_dev_mode(mock_start_gradio, mock_start_backend, mock_args):
    ...
    # Mock the processes to prevent them from actually running
    mock_backend_proc = MagicMock()
    mock_backend_proc.poll.return_value = None  # Indicates it's running
    ...
    # Patch time.sleep to raise KeyboardInterrupt to exit the loop
