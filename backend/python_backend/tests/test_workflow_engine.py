...
from backend.python_backend.workflow_engine import WorkflowEngine, DefaultWorkflow, FileBasedWorkflow, WORKFLOWS_DIR
...
async def test_discover_workflows(workflow_engine, mock_ai_engine, mock_filter_manager, mock_db_manager):
    ...
    with patch("os.path.exists") as mock_exists, \
        ...
        await workflow_engine.discover_workflows(mock_ai_engine, mock_filter_manager, mock_db_manager)
...
@pytest.mark.asyncio
async def test_create_and_register_workflow(workflow_engine, mock_ai_engine, mock_filter_manager, mock_db_manager):
    """Tests creating, saving, and registering a new workflow from a config."""
    new_config = {
        "name": "new_api_workflow",
        "models": {"topic": "topic-default"}
    }
