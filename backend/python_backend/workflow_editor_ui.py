...
try:
    from src.core.security import get_security_manager, SecurityLevel, Permission
...
def create_workflow_editor_ui():
    ...
    with gr.Blocks(title="Email Intelligence - Workflow Editor") as workflow_editor:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                available_nodes = gr.Dropdown(
                    ...
                    value=workflow_manager.get_registered_node_types()[0] if workflow_manager.get_registered_node_types() else None
                )
                ...
                workflow_list = gr.Dropdown(
                    choices=workflow_manager.list_workflows(),
            ...
            with gr.Column(scale=5):
                ...
                workflow_json = gr.Code(
                    ...
                    value=json.dumps({
                        ...
                        "nodes": [
                            {"id": "node1", "type": "email_input", "position": {"x": 0, "y": 0}, "parameters": {}},
                            {"id": "node2", "type": "nlp_processor", "position": {"x": 200, "y": 100}, "parameters": {}},
                            {"id": "node3", "type": "email_output", "position": {"x": 400, "y": 200}, "parameters": {}}
                        ],
                        "connections": [
                            {"source_node_id": "node1", "source_output": "email", "target_node_id": "node2", "target_input": "email"},
                            {"source_node_id": "node2", "source_output": "analysis", "target_node_id": "node3", "target_input": "analysis"}
                        ]
                    }, indent=2)
                )
        ...
        def add_node_to_workflow(node_type: str):
            ...
            workflow_data = {
                "nodes": [{"id": f"node_{len(workflow_manager.get_registered_node_types())}", "type": node_type, "position": {"x": 100, "y": 100}, "parameters": {}}],
                "connections": []
            }
        ...
        def save_current_workflow(name: str, description: str, workflow_json_str: str):
            try:
                ...
                if success:
                    return f"✅ Workflow '{name}' saved successfully!", workflow_manager.list_workflows()
                else:
        ...
        def load_selected_workflow(workflow_filename: str):
            try:
                ...
                if workflow:
                    ...
                    return workflow.name, workflow.description, workflow_json_str, f"✅ Workflow '{workflow.name}' loaded"
                else:
        ...
        load_workflow_btn.click(
            ...
            outputs=[workflow_name, workflow_description, workflow_json, gr.Textbox(label="Load Status")]
        )
        
        execute_btn.click(
            fn=execute_current_workflow,
