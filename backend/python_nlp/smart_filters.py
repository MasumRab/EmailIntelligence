...
class SmartFilterManager:
    ...
    def __init__(self, json_path: str = "smart_filters.json"):
        self.json_path = json_path
        self.logger = logging.getLogger(__name__)
        self.filters: Dict[str, EmailFilter] = {}
        self.performance_history: Dict[str, List[FilterPerformance]] = defaultdict(list)
        # In-memory flag to track if a filter is active
            ...
            if os.path.exists(self.json_path):
                with open(self.json_path, 'r') as f:
                    ...
                    for filter_id, filter_data in data.items():
                        self.filters[filter_id] = EmailFilter(
                            ...
                            effectiveness_score=filter_data.get('effectiveness_score', 0.0),
                            created_date=datetime.fromisoformat(filter_data['created_date']),
                            last_used=datetime.fromisoformat(filter_data['last_used']),
                            usage_count=filter_data.get('usage_count', 0),
                            false_positive_rate=filter_data.get('false_positive_rate', 0.0),
                            performance_metrics=filter_data.get('performance_metrics', {})
                        )
                        self.active_filters[filter_id] = filter_data.get('is_active', True)
                self.logger.info(f"Loaded {len(self.filters)} filters from {self.json_path}")
        except (IOError, json.JSONDecodeError) as e:
            self.logger.error(f"Error loading filters from {self.json_path}: {e}. Starting with an empty set.")
            self.filters = {}
            self.active_filters = {}

    def _save_filters_to_json(self):
        """Save all filters to the JSON file."""
        try:
            with open(self.json_path, 'w') as f:
                data_to_save = {
                    filter_id: {
                        ...
                        'is_active': self.active_filters.get(filter_id, True)
                    }
                    for filter_id, f in self.filters.items()
                }
                json.dump(data_to_save, f, indent=4)
            self.logger.debug(f"Saved {len(self.filters)} filters to {self.json_path}")
        except IOError as e:
            self.logger.error(f"Error saving filters to {self.json_path}: {e}")

    def close(self):
        """Saves filters to JSON file on close. No-op for db connection."""
            ...
            "financial_documents": {
                "criteria": {
                    ...
                    "attachment_types": [
                        ...
                        "xlsx",
                        "csv",
                    ],  # Case-insensitive usually, but good to be consistent
                    "content_keywords": [
                        "amount",
                        "due",
                        "balance",
            ...
            "personal_communications": {
                "criteria": {
                    ...
                    "exclude_patterns": [
                        r".*noreply.*",
                        r".*automated.*",
                    ],  # These are regex, not keywords for keyword list
                    "subject_exclude": [
                        "newsletter",
                        "promotion",
                        "unsubscribe",
                    ],  # Already lowercase
                    "content_personal_indicators": [
                        "how are you",
                        "hope you",
                        "let me know",
                        "talk soon",
    ...
    def create_intelligent_filters(self, email_samples: List[Dict[str, Any]]) -> List[EmailFilter]:
        ...
        custom_filters = self._create_custom_filters(
            patterns
        )  # This method already saves the filters
    ...
    def _analyze_email_patterns(self, email_samples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        aggregated_patterns = {
            ...
            "time_patterns": defaultdict(
                list
            ),  # time_patterns not currently populated by _extract_patterns_from_single_email
        ...
        for email in email_samples:
            ...
            if "sender_domain" in individual_email_patterns:
                aggregated_patterns["sender_domains"][
                    individual_email_patterns["sender_domain"]
                ] += 1

            if "subject_keywords" in individual_email_patterns:
                aggregated_patterns["subject_keywords"].update(
                    individual_email_patterns["subject_keywords"]
                )

            if "content_keywords" in individual_email_patterns:
                aggregated_patterns["content_keywords"].update(
                    individual_email_patterns["content_keywords"]
                )

            if "category" in individual_email_patterns:
                aggregated_patterns["category_distributions"][
                    individual_email_patterns["category"]
                ] += 1

            if "importance_keywords" in individual_email_patterns:
                aggregated_patterns["importance_indicators"].update(
                    individual_email_patterns["importance_keywords"]
                )

            if "automation_flags" in individual_email_patterns:
                aggregated_patterns["automation_indicators"].update(
                    individual_email_patterns["automation_flags"]
                )
    ...
    def _should_create_filter(self, template: Dict[str, Any], patterns: Dict[str, Any]) -> bool:
        ...
        if "subject_keywords" in criteria:
            keyword_matches = sum(
                patterns["subject_keywords"][keyword]
    ...
    def _create_domain_filter(self, domain: str, frequency: int) -> EmailFilter:
        ...
        return EmailFilter(
            ...
            actions={"add_label": f"Domain/{domain}", "mark_important": False},
            priority=4,
    ...
    def _create_keyword_filter(self, keywords: List[str], score: float) -> EmailFilter:
        ...
        return EmailFilter(
            ...
            actions={"add_label": f"Keywords/{keyword_str}", "mark_important": False},
            priority=3,
    ...
    def prune_ineffective_filters(self) -> Dict[str, Any]:
        ...
        pruning_results = { "pruned_filters": [], "disabled_filters": [], "updated_filters": [], "total_analyzed": 0, "performance_improvements": {}, }
        all_filters = self._load_all_filters()
        pruning_results["total_analyzed"] = len(all_filters)
        active_filters_map = { f.filter_id: f for f in all_filters if self._is_filter_active(f.filter_id) }
        actions_to_take = defaultdict(list)
        filters_to_keep_for_redundancy_check = []
        for filter_obj in all_filters:
            evaluation_action = self._evaluate_filter_for_pruning(filter_obj)
            if evaluation_action == "prune": actions_to_take["prune"].append(filter_obj)
            elif evaluation_action == "disable":
                if filter_obj.filter_id in active_filters_map: actions_to_take["disable"].append(filter_obj)
                else: filters_to_keep_for_redundancy_check.append(filter_obj)
            elif evaluation_action == "optimize":
                if filter_obj.filter_id in active_filters_map: actions_to_take["optimize"].append(filter_obj)
                else: filters_to_keep_for_redundancy_check.append(filter_obj)
            else:
                if filter_obj.filter_id in active_filters_map: filters_to_keep_for_redundancy_check.append(filter_obj)
        ...
        for filter_obj in actions_to_take["optimize"]:
            ...
            active_filters_map[filter_obj.filter_id] = optimized_filter
        final_filters_for_redundancy_check = []
        for f_obj in filters_to_keep_for_redundancy_check:
            if f_obj.filter_id in active_filters_map: final_filters_for_redundancy_check.append(active_filters_map[f_obj.filter_id])
        self._prune_redundant_filters(final_filters_for_redundancy_check, pruning_results)
        self.logger.info(f"Pruning completed: {len(pruning_results['pruned_filters'])} pruned, " f"{len(pruning_results['disabled_filters'])} disabled, " f"{len(pruning_results['updated_filters'])} optimized")
        return pruning_results

    def _is_filter_active(self, filter_id: str) -> bool:
        """Check if a filter is currently marked as active in memory."""
        return self.active_filters.get(filter_id, True)
    ...
    def _prune_redundant_filters(self, active_kept_filters: List[EmailFilter], pruning_results: Dict[str, Any]):
        ...
        for i, filter1 in enumerate(sorted_filters):
            if filter1.filter_id in deleted_ids_due_to_redundancy: continue
            for j in range(i + 1, len(sorted_filters)):
                filter2 = sorted_filters[j]
                if filter2.filter_id in deleted_ids_due_to_redundancy: continue
                similarity = self._calculate_filter_similarity(filter1, filter2)
                if similarity > self.pruning_criteria["redundancy_threshold"]:
                    self._delete_filter(filter2.filter_id)
                    pruning_results["pruned_filters"].append({ "filter_id": filter2.filter_id, "name": filter2.name, "reason": f"Redundant with {filter1.name} (similarity: {similarity:.2f})", })
    ...
    def _evaluate_filter_for_pruning(self, filter_obj: EmailFilter) -> str:
        ...
        if filter_obj.effectiveness_score < criteria["effectiveness_threshold"] * 0.5: return "prune"
        if filter_obj.false_positive_rate > criteria["false_positive_threshold"]: return "prune"
        days_since_last_used = (datetime.now() - filter_obj.last_used).days
        if (filter_obj.usage_count < criteria["usage_threshold"] and days_since_last_used > criteria["age_threshold_days"]): return "disable"
        performance = self._get_filter_performance(filter_obj.filter_id)
        if performance and (performance.accuracy < criteria["accuracy_threshold"] or performance.f1_score < criteria["f1_threshold"]):
            if (filter_obj.effectiveness_score > criteria["effectiveness_threshold"] * 0.7): return "optimize"
            else: return "disable"
        if filter_obj.effectiveness_score < criteria["effectiveness_threshold"]: return "disable"
        return "keep"
    ...
    def _optimize_filter(self, filter_obj: EmailFilter) -> EmailFilter:
        ...
        if not performance_history: return filter_obj
    ...
    def evaluate_filter_performance(self, filter_id: str, test_emails: List[Dict[str, Any]]) -> FilterPerformance:
        ...
        if not filter_obj: raise ValueError(f"Filter {filter_id} not found")
        ...
        performance = FilterPerformance(filter_id=filter_id, accuracy=metrics_dict["accuracy"], precision=metrics_dict["precision"], recall=metrics_dict["recall"], f1_score=metrics_dict["f1_score"], processing_time_ms=processing_time_ms, emails_processed=len(test_emails), true_positives=metrics_dict["true_positives"], false_positives=metrics_dict["false_positives"], false_negatives=metrics_dict["false_negatives"])
        self._save_filter_performance(performance)
        filter_obj.effectiveness_score = performance.f1_score
    ...
    def _calculate_performance_metrics_from_results(self, results: List[tuple[bool, bool]], total_emails: int) -> Dict[str, float]:
        ...
        return { "true_positives": true_positives, "false_positives": false_positives, "false_negatives": false_negatives, "true_negatives": true_negatives, "precision": precision, "recall": recall, "accuracy": accuracy, "f1_score": f1_score, "false_positive_rate": false_positive_rate, }
    ...
    def _check_subject_keywords(self, criteria_block: List[str], email: Dict[str, Any], operator: str, min_matches: int) -> bool:
        ...
        if operator == "AND": return keyword_matches == len(criteria_block)
        elif operator == "OR": return keyword_matches >= min_matches
        return False
    ...
    def _check_exclusion_patterns(self, criteria_block: List[str], email: Dict[str, Any]) -> bool:
        ...
        text_to_check = f"{email.get('subject', '')} {email.get('content', '')}"
        if any(re.search(pattern, text_to_check, re.IGNORECASE) for pattern in criteria_block): return False
        return True
    ...
    def _apply_filter_to_email(self, filter_obj: EmailFilter, email: Dict[str, Any]) -> bool:
        ...
        if "from_patterns" in criteria and not self._check_sender_patterns(criteria["from_patterns"], email): return False
        if "subject_keywords" in criteria:
            operator = criteria.get("keyword_operator", "OR")
            min_matches = criteria.get("min_keyword_matches", 1)
            if operator == "AND": min_matches = len(criteria["subject_keywords"])
            if not self._check_subject_keywords(criteria["subject_keywords"], email, operator, min_matches): return False
        if "content_keywords" in criteria and not self._check_content_keywords(criteria["content_keywords"], email): return False
        if "exclude_patterns" in criteria and not self._check_exclusion_patterns(criteria["exclude_patterns"], email): return False
        return True

    def _save_filter(self, filter_obj: EmailFilter):
        """Save filter to in-memory dictionary."""
        self.filters[filter_obj.filter_id] = filter_obj
        self.active_filters[filter_obj.filter_id] = True

    def _load_filter(self, filter_id: str) -> Optional[EmailFilter]:
        """Load a specific filter from memory."""
        return self.filters.get(filter_id)

    def _load_all_filters(self) -> List[EmailFilter]:
        """Load all filters from memory."""
    ...
    def _delete_filter(self, filter_id: str):
        """Permanently delete filter and its performance metrics from memory."""
        self.filters.pop(filter_id, None)
        self.active_filters.pop(filter_id, None)
        self.performance_history.pop(filter_id, None)

    def _disable_filter(self, filter_id: str):
        """Disable filter in memory."""
        if filter_id in self.filters:
            self.active_filters[filter_id] = False

    def _save_filter_performance(self, performance: FilterPerformance):
        """Save filter performance metrics in memory."""
        self.performance_history[performance.filter_id].append(performance)

    def _get_filter_performance(self, filter_id: str) -> Optional[FilterPerformance]:
        """Get latest performance metrics for a filter from memory."""
        history = self.performance_history.get(filter_id)
        if history:
            return history[-1]
        return None

    def _get_filter_performance_history(self, filter_id: str, days: int = 30) -> List[FilterPerformance]:
        """Get performance history for a filter from memory."""
        return self.performance_history.get(filter_id, [])

    def get_active_filters_sorted(self) -> List[EmailFilter]:
        """Loads all active filters from memory, sorted by priority."""
        active_filter_objects = [f for f_id, f in self.filters.items() if self.active_filters.get(f_id, True)]
        return sorted(active_filter_objects, key=lambda f: f.priority, reverse=True)
    ...
    def _execute_filter_actions(self, email_data: Dict[str, Any], actions: Dict[str, Any]) -> List[str]:
        ...
        if actions.get("add_label"):
            ...
            if "labels" not in email_data: email_data["labels"] = []
            if label not in email_data["labels"]: email_data["labels"].append(label)
            actions_taken_summary.append(f"Added label: {label}")
        if actions.get("mark_important"):
            email_data["is_important_override"] = True
    ...
    def apply_filters_to_email_data(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        email_actions_summary = { "filters_matched": [], "actions_taken": [], "modified_fields": {}, }
...
def main():
    ...
    sample_emails = [
        { "senderEmail": "manager@company.com", "subject": "Urgent: Budget Review Meeting", "content": "We need to discuss the quarterly budget immediately.", "isImportant": True, "category": "work_business", },
        { "senderEmail": "noreply@bank.com", "subject": "Your Monthly Statement", "content": "Your account statement is ready for download.", "category": "finance_banking", },
    ]
    ...
    print(f"Pruning results: {len(pruning_results['pruned_filters'])} pruned, " f"{len(pruning_results['disabled_filters'])} disabled")
