...
class SmartFilterManager:
    ...
    def prune_ineffective_filters(self) -> Dict[str, Any]:
        ...
        )  # Load all, including inactive ones for this evaluation
        ...
        for filter_obj in all_filters:
            ...
            evaluation_action = self._evaluate_filter_for_pruning(
                filter_obj
            )  # This method might need adjustment
            ...
            if evaluation_action == "prune":
                ...
                if (
                    filter_obj.filter_id in active_filters_map
                ):  # Only active filters are kept for redundancy
                    filters_to_keep_for_redundancy_check.append(filter_obj)
            ...
            pruning_results["pruned_filters"].append(
                {
                    ...
                    "reason": "Pruned by evaluation",
                }
            )
            if filter_obj.filter_id in active_filters_map:
                del active_filters_map[filter_obj.filter_id]
        ...
        for filter_obj in actions_to_take["optimize"]:
            optimized_filter = self._optimize_filter(
                filter_obj
            )  # Assume _optimize_filter returns the modified object
            self._update_filter(optimized_filter)
            pruning_results["updated_filters"].append(
                {
                    "filter_id": optimized_filter.filter_id,
                    "name": optimized_filter.name,
                    "changes": "Criteria optimized",
                }
            )
            ...
            active_filters_map[filter_obj.filter_id] = (
                optimized_filter  # Update with optimized version
            )
    ...
    def _is_filter_active_in_db(self, filter_id: str) -> bool:
        ...
        row = self._db_fetchone(
            "SELECT is_active FROM email_filters WHERE filter_id = ?", (filter_id,)
        )
        return row["is_active"] == 1 if row else False

    def _prune_redundant_filters(
        self, active_kept_filters: List[EmailFilter], pruning_results: Dict[str, Any]
    ):
        ...
        sorted_filters = sorted(
            active_kept_filters, key=lambda f: f.effectiveness_score, reverse=True
        )
    ...
    def _evaluate_filter_for_pruning(self, filter_obj: EmailFilter) -> str:
        ...
        if (
            filter_obj.effectiveness_score < criteria["effectiveness_threshold"] * 0.5
        ):  # Stricter for direct prune
            return "prune"
        ...
        days_since_last_used = (
            datetime.now() - filter_obj.last_used
        ).days  # More relevant than created_date
        ...
        if (
            filter_obj.usage_count < criteria["usage_threshold"]
            and days_since_last_used > criteria["age_threshold_days"]
        ):
            return "disable"  # Candidate for disabling if it was active

        # Rule 4: Performance suggests optimization (for active filters)
        # if _is_filter_active_in_db(filter_obj.filter_id): (check added to caller)
        performance = self._get_filter_performance(
            filter_obj.filter_id
        )  # This fetches latest performance
        if performance and (
            performance.accuracy < criteria["accuracy_threshold"]
            or performance.f1_score < criteria["f1_threshold"]
        ):
    ...
    def evaluate_filter_performance(
        self, filter_id: str, test_emails: List[Dict[str, Any]]
    ) -> FilterPerformance:
        ...
        application_results, processing_time_ms = self._get_filter_application_results(
            filter_obj, test_emails, start_time
        )

        # Calculate performance metrics
        metrics_dict = self._calculate_performance_metrics_from_results(
            application_results, len(test_emails)
        )
    ...
    def _get_filter_application_results(
        ...
        for email in test_emails:
            ...
            )  # Assuming 'expected_filter_match' key exists
            results.append((predicted, actual))

        processing_time_ms = (datetime.now() - start_time).total_seconds() * 1000
        return results, processing_time_ms

    def _calculate_performance_metrics_from_results(
        self, results: List[tuple[bool, bool]], total_emails: int
    ) -> Dict[str, float]:
    ...
    def _apply_filter_to_email(self, filter_obj: EmailFilter, email: Dict[str, Any]) -> bool:
        ...
        if "subject_keywords" in criteria:
            ...
            if not self._check_subject_keywords(
                criteria["subject_keywords"], email, operator, min_matches
            ):
                return False
    ...
    def _get_filter_performance_history(
        self, filter_id: str, days: int = 30
    ...
    def _execute_filter_actions(
        self, email_data: Dict[str, Any], actions: Dict[str, Any]
    ) -> List[str]:
        """
    ...
    def apply_filters_to_email_data(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for filter_obj in sorted_filters:
            ...
            if match_result:
                ...
                if filter_obj.actions.get("mark_as_spam") or filter_obj.actions.get(
                    "set_priority_high"
                ):
                    self.logger.info(
                        f"Stopping further filter processing due to action in {filter_obj.name}"
                    )
                    break
...
def main():
    ...
    print(
        f"Pruning results: {len(pruning_results['pruned_filters'])} pruned, "
        f"{len(pruning_results['disabled_filters'])} disabled"
    )
