...
class SmartRetrievalManager(GmailRetrievalService):
    ...
    """
    def __init__(self, checkpoint_db_path: str = DEFAULT_CHECKPOINT_DB_PATH, credentials_path: str = CREDENTIALS_PATH, 
                 token_path: str = TOKEN_JSON_PATH):
    ...
    async def execute_smart_retrieval(
        ...
        self.logger.info(f"Starting smart retrieval with {len(strategies)} strategies, "
        ...
        for strategy in strategies:
            ...
            api_call_count += min(strategy.max_emails_per_run // strategy.batch_size + 1, 10)  # Rough estimate

        total_time = (datetime.now() - start_time).total_seconds() / 60
        self.logger.info(f"Smart retrieval completed in {total_time:.2f} minutes, "
