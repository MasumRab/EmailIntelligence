...
async def test_resource_limits():
    ...
    completed_count = sum(1 for r in results if not isinstance(r, Exception) and r.metadata.get('status') == 'completed')
...
async def test_scalability():
    ...
    completed_count = sum(1 for r in results if not isinstance(r, Exception) and getattr(r, 'metadata', {}).get('status') == 'completed')
