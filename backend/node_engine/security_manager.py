    ...
    def validate_node_execution(self, node_type: str, config: Dict[str, Any]) -> bool:
        ...
        if node_type not in self.trusted_node_types:
            ...
            if config.get("code", "") or config.get("script", ""):
                self.logger.warning(f"Untrusted node {node_type} has code configuration - access denied")
                return False
        ...
        for port_name, expected_type in expected_types.items():
            if port_name in inputs:
                if not isinstance(inputs[port_name], expected_type):
                    self.logger.error(f"Input validation failed: {port_name} expected {expected_type}, got {type(inputs[port_name])}")
                    return False
        return True
...
class AuditLogger:
    ...
    def log_node_execution(self, workflow_id: str, node_id: str, node_name: str, status: str, duration: float):
        """Log node execution."""
        self.logger.info(f"NODE_EXEC: workflow={workflow_id}, node_id={node_id}, name={node_name}, status={status}, duration={duration}s")
...
class ResourceManager:
    ...
    async def acquire_resources(self, workflow_id: str, required_resources: ResourceLimits) -> bool:
        ...
        self.logger.info(f"Resources acquired for workflow {workflow_id}. Current: {self.current_workflows}/{self.max_concurrent_workflows}")
        return True
    ...
    def release_resources(self, workflow_id: str):
        ...
        self.logger.info(f"Resources released for workflow {workflow_id}. Current: {self.current_workflows}/{self.max_concurrent_workflows}")
...
for node_type in ["EmailSourceNode", "PreprocessingNode", "AIAnalysisNode", "FilterNode", "ActionNode"]:
