...
*   **New Command**: `plan-rebase`
    *   **Arguments**:
        *   `target_branch` (default: `origin/scientific`)
...
Update `eai.py` and `src/cli/commands.py` to include the `plan-rebase` command.
...
python eai.py analyze-history --branch scientific --output analysis_report.md

# Generate the rebase todo list
python eai.py plan-rebase --branch scientific --output rebase-todo.md
```
