...
def format_task_details(task, indent=0):
    ...
    if subtasks:
        ...
        for subtask in subtasks:
            lines.append(
                f"{prefix}    - {subtask['id']}: {subtask['title']} ({subtask.get('status', 'N/A')})"
            )
...
def main():
    ...
    parser.add_argument(
        "--invalid", action="store_true", help="Search in tasks_invalid.json instead"
    )
    ...
    if args.invalid:
        ...
        pattern = (
            rf'\{{[^}}]*"id":\s*{args.task_id}[^}}]*"title":\s*"([^"]+)"[^}}]*"status":\s*"([^"]+)"'
        )
