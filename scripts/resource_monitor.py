...
class ResourceMonitor:
    def __init__(self, monitoring_file: Path = None, max_history: int = 1000):
        ...
        self.process_resources: Dict[int, deque] = defaultdict(
    ...
    def get_system_resource_history(self, hours: int = 1) -> List[ResourceSnapshot]:
        ...
        with self._lock:
            return [
                snapshot
                for snapshot in self.system_resources
                if snapshot.timestamp >= cutoff_time
            ]

    def get_process_resource_history(
        self, pid: int, hours: int = 1
    ) -> List[ProcessResourceUsage]:
        ...
        with self._lock:
            ...
            if resource_type == "cpu":
                sorted_processes = sorted(
                    latest_usage.values(), key=lambda x: x.cpu_percent, reverse=True
                )
            elif resource_type == "memory":
                sorted_processes = sorted(
                    latest_usage.values(), key=lambda x: x.memory_percent, reverse=True
                )
            else:
    ...
    def get_system_resource_stats(self, hours: int = 1) -> Dict[str, Any]:
        ...
        return {
            ...
            "cpu": {
                ...
                "stdev": statistics.stdev(cpu_percentages)
            ...
            "memory": {
                ...
                "stdev": statistics.stdev(memory_percentages)
                if len(memory_percentages) > 1
                else 0.0,
    ...
    def detect_resource_alerts(
        self,
        cpu_threshold: float = 80.0,
        memory_threshold: float = 85.0,
        process_threshold: int = 500,
    ) -> List[ResourceAlert]:
        ...
        with self._lock:
            ...
            if latest.cpu_percent > cpu_threshold:
                severity = (
                    "critical" if latest.cpu_percent > cpu_threshold * 1.2 else "high"
                )
                alert = ResourceAlert(
            ...
            if latest.memory_percent > memory_threshold:
                severity = (
                    "critical"
                    if latest.memory_percent > memory_threshold * 1.1
                    else "high"
                )
                alert = ResourceAlert(
            ...
            if latest.process_count > process_threshold:
                severity = (
                    "high"
                    if latest.process_count > process_threshold * 1.5
                    else "medium"
                )
                alert = ResourceAlert(
...
class ResourceDashboard:
    ...
    def display_resource_summary(self):
        ...
        print(
            f"Load Average: {sys_resources['load_average'][0]:.2f} (1m), "
        ...
        for proc in summary["top_processes"]["cpu"][:3]:
            print(
                f"  PID {proc['pid']} ({proc['name']}): {proc['cpu_percent']:.1f}% CPU"
            )

        print("\nTop Memory Processes:")
        for proc in summary["top_processes"]["memory"][:3]:
            print(
                f"  PID {proc['pid']} ({proc['name']}): {proc['memory_percent']:.1f}% Memory "
