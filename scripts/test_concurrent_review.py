...
def test_review_session_creation():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        session_id = review_manager.start_review_session(
            document_id="test-doc-1", reviewers=["agent-1", "agent-2", "agent-3"]
        )
...
def test_adding_comments():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create a review session
        session_id = review_manager.start_review_session(
            document_id="comment-doc-1", reviewers=["agent-1", "agent-2"]
        )
        ...
        assert comment_1.comment_text == "This section needs more detail.", (
...
def test_resolving_comments():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        session_id = review_manager.start_review_session(
            document_id="resolve-doc-1", reviewers=["agent-1"]
        )
...
def test_adding_feedback():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create a review session
        session_id = review_manager.start_review_session(
            document_id="feedback-doc-1", reviewers=["agent-1", "agent-2"]
        )
...
def test_adding_votes():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create a review session
        session_id = review_manager.start_review_session(
            document_id="vote-doc-1", reviewers=["agent-1", "agent-2", "agent-3"]
        )
        ...
        assert voting_results["reviewers_participated"] == 3, (
            "Should have 3 reviewers participated"
        )
...
def test_comment_summary():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        session_id = review_manager.start_review_session(
            document_id="summary-doc-1", reviewers=["agent-1", "agent-2"]
        )
        ...
        assert comment_summary["resolved_comments"] == 1, (
            "Should have 1 resolved comment"
        )
        assert comment_summary["open_comments"] == 3, "Should have 3 open comments"
        assert comment_summary["severity_breakdown"]["low"] == 2, (
            "Should have 2 low severity comments"
        )
        assert comment_summary["severity_breakdown"]["medium"] == 1, (
            "Should have 1 medium severity comment"
        )
        assert comment_summary["severity_breakdown"]["high"] == 1, (
            "Should have 1 high severity comment"
        )
...
def test_voting_results():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert voting_results["reviewers_participated"] == 4, (
            "Should have 4 reviewers participated"
        )
        ...
        assert section1_results["vote_breakdown"]["approve"] == 2, (
            "Section-1 should have 2 approve votes"
        )
        assert section1_results["vote_breakdown"]["reject"] == 1, (
            "Section-1 should have 1 reject vote"
        )
        ...
        assert section2_results["vote_breakdown"]["approve"] == 2, (
            "Section-2 should have 2 approve votes"
        )
...
def test_session_progress():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create a review session
        session_id = review_manager.start_review_session(
            document_id="progress-doc-1", reviewers=["agent-1", "agent-2", "agent-3"]
        )
        ...
        assert progress["reviewers_with_votes"] == 0, (
            "Should have 0 reviewers with votes initially"
        )
        assert progress["reviewers_with_feedback"] == 0, (
            "Should have 0 reviewers with feedback initially"
        )
        ...
        assert updated_progress["reviewers_with_votes"] == 1, (
            "Should have 1 reviewer with votes"
        )
        assert updated_progress["reviewers_with_feedback"] == 1, (
            "Should have 1 reviewer with feedback"
        )
        assert updated_progress["participation_rate"] > 0, (
            "Participation rate should be > 0"
        )
...
def test_review_consensus():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        session_id = review_manager.start_review_session(
            document_id="consensus-test-doc", reviewers=reviewers
        )
        ...
        assert consensus["participation_consensus"] == 1.0, (
            "Participation consensus should be 100%"
        )
...
def test_review_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        session_id = review_manager1.start_review_session(
            document_id="persistence-doc-1", reviewers=["agent-1", "agent-2"]
        )
        ...
        assert loaded_session.document_id == "persistence-doc-1", (
        ...
        assert comments[0].comment_text == "Persistence test comment", (
            "Comment text should match"
        )
        ...
        assert feedback[0].feedback_text == "Persistence test feedback", (
            "Feedback text should match"
        )
...
def test_review_dashboard():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create a review session
        session_id = review_manager.start_review_session(
            document_id="dashboard-doc-1", reviewers=["agent-1", "agent-2", "agent-3"]
        )
