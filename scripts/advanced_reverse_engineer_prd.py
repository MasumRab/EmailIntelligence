#!/usr/bin/env python3
"""
Advanced Reverse Engineer PRD from Task Markdown Files
...
This advanced version implements first-order improvements identified by the 
architectural analysis:
- Enhanced capability-feature mapping
- Improved dependency graph with proper topological ordering
- Better metadata extraction and standardization
- Optimized for task-master parsing fidelity

Usage:
    python advanced_reverse_engineer_prd.py --input-dir /path/to/task/files --output /path/to/generated_prd.md
"""
...
def extract_task_info_from_md(file_path: str) -> Dict[str, Any]:
    """
    Extract key information from a task markdown file with enhanced parsing.
...
def create_meaningful_capability_name(task_info: Dict[str, Any]) -> str:
    """
    Create a meaningful capability name based on the task content.
    ...
    # Extract keywords from purpose to create a meaningful name
...
- **Description**: {task_info.get('purpose', 'Task to implement the specified functionality')[:100] + '...' if len(task_info.get('purpose', '')) > 100 else task_info.get('purpose', 'Task to implement the specified functionality')}
...
"""
    ...
    if task_info.get('effort'):
        effort_hours_match = re.search(r'(\d+)-?(\d+)?', task_info['effort'])
...
def generate_task_as_prd_section(task_info: Dict[str, Any]) -> str:
    """
    Generate a PRD section that represents the task with enhanced formatting.
...
"""
    ...
    # Add effort and complexity with standardized formatting
    if task_info.get('effort'):
        prd_section += f"### Effort Estimation\n{task_info['effort']}\n\n"
    if task_info.get('complexity'):
        prd_section += f"### Complexity Level\n{task_info['complexity']}\n\n"
    if task_info.get('dependencies'):
        prd_section += f"### Dependencies\n{task_info['dependencies']}\n\n"
...
"""
    ...
    if task_info.get('test_strategy'):
...
def create_advanced_reverse_engineered_prd(task_files: List[str]) -> str:
    """
    Create an advanced PRD from a list of task markdown files.
    
    Args:
        task_files: List of paths to task markdown files
        
    Returns:
        String containing the advanced generated PRD
    """
...
def main():
    parser = argparse.ArgumentParser(description="Advanced reverse engineer PRD from task markdown files")
    ...
    print(f"Generated advanced PRD with {len(task_files)} tasks and saved to {output_path}")
    print("The advanced PRD follows the RPG method structure that task-master expects.")
    print("First-order improvements implemented:")
    print("- Enhanced capability-feature mapping with meaningful names")
    print("- Improved dependency graph with proper topological ordering")
    print("- Better metadata extraction and standardization")
    print("- Optimized for task-master parsing fidelity")
    print("- Enhanced success criteria formatting")
