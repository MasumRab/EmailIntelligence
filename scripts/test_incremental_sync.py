...
def test_incremental_sync():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create test files
        (source_dir / "test1.md").write_text(
            "# Test Document 1\n\nThis is test content."
        )
        (source_dir / "test2.md").write_text(
            "# Test Document 2\n\nThis is more test content."
        )
        (source_dir / "subdir").mkdir()
        (source_dir / "subdir" / "test3.md").write_text(
            "# Test Document 3\n\nContent in subdirectory."
        )

        # Create some initial files in target that will be detected as changed
        (target_dir / "test1.md").write_text(
            "# Test Document 1\n\nThis is OLD content."
        )
        (target_dir / "test4.md").write_text(
        ...
        print(
            f"Preview - Added: {len(preview['added'])}, Modified: {len(preview['modified'])}, Deleted: {len(preview['deleted'])}"
        )
        ...
        print(
            f"Sync result - Added: {result['added']}, Modified: {result['modified']}, Deleted: {result['deleted']}"
        )
