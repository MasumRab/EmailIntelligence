...
def main():
    ...
    # Identify tasks without subtasks
    tasks_without_subtasks = []
    for task_file in all_task_files:
        with open(task_file, 'r', encoding='utf-8') as f:
            content = f.read()

        # Check if task has subtasks section
        if not re.search(r'Subtasks|## Sub-subtasks', content):
    ...
    print(f"Found {len(tasks_without_subtasks)} medium+ complexity tasks without subtasks")
    ...
    print("Subtasks have been added to medium and high complexity tasks to improve task manageability.")
