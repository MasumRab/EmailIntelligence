...
class MaintenanceTask:
    task_id: str
    task_type: (
        str  # "link_check", "content_update", "format_check", "security_check", etc.
    )
    document_id: str
    description: str
...
class MaintenanceScheduler:
        ...
        with self._lock:
            for agent_id, agent in self.maintenance_agents.items():
                if (
                    ...
                    and agent.current_task_count < agent.max_concurrent_tasks
                ):
    ...
    def _find_highest_priority_task(self) -> Optional[str]:
        ...
        with self._lock:
            ...
            pending_tasks = [
                ...
                and self.maintenance_tasks[task_id].status == "pending"
            ]
            ...
            highest_priority_task = max(
                ...
                key=lambda tid: priority_order.get(
                    self.maintenance_tasks[tid].priority, 0
                ),
            )
    ...
    def _check_schedules(self):
        ...
        with self._lock:
            for schedule_id, schedule in self.maintenance_schedules.items():
                if (
                    schedule.enabled
                    and schedule.next_run_time <= now
                    and schedule.last_run_time < schedule.next_run_time
                ):
                    # Create a new maintenance task based on the schedule
                    task_id = self.create_maintenance_task(
                        task_type=schedule.task_type,
                        document_id=schedule.task_params.get(
                            "document_id", "*"
                        ),  # All documents if not specified
    ...
    def get_task_results(self, task_id: str) -> List[MaintenanceResult]:
        ...
        with self._lock:
            return [
                result
                for result in self.maintenance_results.values()
        ...
        with self._lock:
            return {
                ...
                "active_agents": len(
                    [
                        a
                        for a in self.maintenance_agents.values()
                        if a.status == "active"
                    ]
                ),
                ...
                "failed_tasks": len(
                    [t for t in self.maintenance_tasks.values() if t.status == "failed"]
                ),
                "task_queue_length": len(self.task_queue),
                "total_schedules": len(self.maintenance_schedules),
                "enabled_schedules": len(
                    [s for s in self.maintenance_schedules.values() if s.enabled]
                ),
            }
    ...
    def load_scheduler_data(self):
        ...
        try:
            ...
            for schedule_id, schedule_data in data.get(
                "maintenance_schedules", {}
            ).items():
                schedule = MaintenanceSchedule(
...
class MaintenanceDashboard:
    ...
    def display_schedules(self):
        ...
        for schedule_id, schedule in schedules.items():
            next_run = datetime.fromtimestamp(schedule.next_run_time).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            last_run = (
                datetime.fromtimestamp(schedule.last_run_time).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    def display_pending_tasks(self):
        ...
        for task_id in queue:
            if task_id in tasks:
                task = tasks[task_id]
                scheduled_time = datetime.fromtimestamp(task.scheduled_time).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
