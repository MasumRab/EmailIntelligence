...
class ConcurrentReviewManager:
    def __init__(self, reviews_file: Path = None):
        ...
        self.agent_reviews: Dict[str, List[str]] = defaultdict(
            list
        )  # agent_id -> session_ids
        self.document_reviews: Dict[str, List[str]] = defaultdict(
            list
    ...
    def start_review_session(
        self, document_id: str, reviewers: List[str], metadata: Dict[str, Any] = None
    ) -> str:
    ...
    def get_sessions_for_agent(self, agent_id: str) -> List[ReviewSession]:
        ...
        with self._lock:
            ...
            return [
                self.review_sessions[sid]
                for sid in session_ids
                if sid in self.review_sessions
            ]
    ...
    def resolve_comment(
        self, session_id: str, comment_id: str, resolved_by: str
    ) -> bool:
    ...
    def get_comment_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            resolved_comments = len(
                [c for c in session.comments if c.status == "resolved"]
            )
    ...
    def get_voting_results(self, session_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "reviewers_participated": len(
                    set(vote.agent_id for vote in session.votes)
                ),
            }
    ...
    def get_session_progress(self, session_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            reviewers_with_feedback = len(
                set(feedback.agent_id for feedback in session.feedback)
            )

            # Calculate completion percentage based on participation
            participation_rate = (
                (reviewers_with_votes + reviewers_with_feedback) / (2 * total_reviewers)
    ...
    def get_review_consensus(self, session_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            participation_consensus = (
                reviewers_participated / total_reviewers if total_reviewers > 0 else 0
            )
            ...
            approval_rate = (
                approve_votes / total_section_votes if total_section_votes > 0 else 0
            )
...
class ReviewDashboard:
    ...
    def display_session_overview(self, session_id: str):
        ...
        print(
            f"  Reviewers with Votes: {progress.get('reviewers_with_votes', 0)}/{len(session.reviewers)}"
        )
        print(
            f"  Reviewers with Feedback: {progress.get('reviewers_with_feedback', 0)}/{len(session.reviewers)}"
        )

        print("\nConsensus:")
        print(
            f"  Participation Consensus: {consensus.get('participation_consensus', 0):.1%}"
        )
        ...
        print(
            f"  Reviewers Participated: {voting_results.get('reviewers_participated', 0)}/{len(session.reviewers)}"
        )
        ...
        if section_consensus:
            ...
            for section_id, result in section_consensus.items():
                print(
                    f"    {section_id}: {result.get('consensus', 'no_votes')} "
    ...
    def display_session_votes(self, session_id: str):
        ...
        for section_id, section_vote_list in section_votes.items():
            ...
            for vote in section_vote_list:
                timestamp = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(vote.timestamp)
                )
    ...
    def display_session_feedback(self, session_id: str):
        ...
        for i, feedback in enumerate(feedback_list, 1):
            timestamp = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(feedback.timestamp)
            )
