...
class PredictiveTimeEstimator:
    ...
    def add_task_result(
        ...
        history = TaskHistory(
            task_id, task_type, estimated_time, actual_time, agent_name, success
        )
        self.history.append(history)
        self._save_history()
    ...
    def predict_completion_time(
        self, task_type: str, estimated_time: int, agent_name: str
    ) -> Optional[float]:
    ...
    def get_prediction_accuracy(self) -> Dict:
        ...
        recent_history = [h for h in self.history if h.success][
            -50:
        ]  # Last 50 successful tasks
        ...
        for h in recent_history:
            predicted = self.predict_completion_time(
                h.task_type, h.estimated_time, h.agent_name
            )
            if predicted:
    ...
    def get_performance_trends(self) -> Dict:
        ...
        for week_key, data in weekly_data.items():
            if data["task_count"] > 0:
                ...
                data["efficiency"] = (
                    data["avg_estimated"] / data["avg_actual"]
                    if data["avg_actual"] > 0
                    else 1.0
                )
