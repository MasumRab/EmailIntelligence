...
def calculate_completeness_score(content: str) -> float:
    ...
    score = (sections * 10 + content_lines + checklists * 5) / 100.0

    # Adjust score based on placeholder density
    placeholders = count_placeholders(content)
    if len(content) > 0:
        placeholder_density = placeholders / len(content) * 1000  # Per thousand characters
        # Reduce score based on placeholder density
        score = max(0, score - placeholder_density * 2)
...
def select_best_task_version(task_paths: List[Path]) -> Tuple[Path, str]:
    ...
    for path in task_paths:
        try:
            ...
            # Use a combined score that favors high completeness and low placeholders
            combined_score = completeness_score - (placeholder_count * 2)  # Penalize placeholders heavily
...
def main():
    ...
    try:
        ...
        print("   These represent the most complete, placeholder-free versions available")
