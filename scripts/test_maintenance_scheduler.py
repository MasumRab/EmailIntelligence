...
def test_agent_registration():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert "link_check" in agent_status.capabilities, (
            "Should have link_check capability"
        )
...
def test_task_callback_registration():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert "link_check" in scheduler.task_execution_callbacks, (
...
def test_create_maintenance_task():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create and register an agent
        agent = MaintenanceAgent(agent_id="test-agent-1", capabilities=["link_check"])
...
def test_create_recurring_schedule():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert schedule_status.schedule_pattern == "daily", (
            "Schedule pattern should match"
        )
...
def test_scheduler_statistics():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        agent1 = MaintenanceAgent(
            agent_id="agent-1", capabilities=["link_check", "format_check"]
        )
        agent2 = MaintenanceAgent(agent_id="agent-2", capabilities=["content_update"])
...
def test_scheduler_dashboard():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create and register an agent
        agent = MaintenanceAgent(
            agent_id="dashboard-test-agent", capabilities=["link_check", "format_check"]
        )
        scheduler.register_agent(agent)
...
def test_scheduler_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create and register an agent
        agent = MaintenanceAgent(
            agent_id="persistence-test-agent", capabilities=["link_check"]
        )
        ...
        assert "link_check" in loaded_agent.capabilities, (
...
def test_find_available_agents():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert "agent-2" not in available_agents, (
            "Should not find agent-2 for link_check"
        )
...
def test_task_prioritization():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create and register an agent
        agent = MaintenanceAgent(
            agent_id="priority-test-agent", capabilities=["link_check"]
        )
        scheduler.register_agent(agent)
        ...
        assert highest_priority_task_id == critical_task_id, (
            "Critical task should be highest priority"
        )
