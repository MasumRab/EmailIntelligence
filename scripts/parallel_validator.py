...
class ParallelValidationManager:
    ...
    def create_validation_task(
        self, validation_type: ValidationType, files: List[str], worker_id: str
    ) -> ValidationTask:
    ...
    def run_validation_task(self, task: ValidationTask) -> ValidationTask:
        ...
        if task.worker_id not in self.workers:
            ...
            available_workers = [
                wid for wid, worker in self.workers.items() if not worker.is_active
            ]
    ...
    def run_parallel_validation(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_task = {
                executor.submit(self.run_validation_task, task): task for task in tasks
            }
    ...
    def get_all_workers_status(self) -> Dict[str, Dict]:
        ...
        return {
            worker_id: self.get_worker_status(worker_id) for worker_id in self.workers
        }
    ...
    def get_validation_summary(self) -> Dict:
        ...
        passed_tasks = len(
            [t for t in self.validation_history if t.result == ValidationResult.PASS]
        )
        failed_tasks = len(
            [t for t in self.validation_history if t.result == ValidationResult.FAIL]
        )
        warning_tasks = len(
            [t for t in self.validation_history if t.result == ValidationResult.WARNING]
        )
