...
def test_task_recording():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        validation_durations = predictor.task_type_stats["content-validation"][
            "durations"
        ]
...
def test_basic_prediction():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        prediction = predictor.predict_completion_time(
            task_id="new-task-1", task_type="test-task", agent_id="test-agent"
        )
        ...
        assert prediction.confidence > 0.5, (
            "Confidence should be reasonable with good data"
        )
...
def test_agent_performance_factor():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        fast_factor = predictor._get_agent_performance_factor(
            "fast-agent", "standard-task"
        )
        slow_factor = predictor._get_agent_performance_factor(
            "slow-agent", "standard-task"
        )
        avg_factor = predictor._get_agent_performance_factor(
            "average-agent", "standard-task"
        )
...
def test_prediction_confidence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert good_data_confidence > no_data_confidence, (
            "Good data should give higher confidence"
        )
...
def test_prediction_with_features():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        prediction = predictor.predict_completion_time(
            ...
            task_type="complex-task",
            agent_id="feature-agent",
            features={"complexity": 4, "file_count": 50, "requires_api": True},
        )
...
def test_prediction_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert len(predictor2.task_type_stats["persistence-task"]["durations"]) == 1, (
            "Should have task type stats"
        )
        assert len(predictor2.agent_stats["persistence-agent"]["durations"]) == 1, (
            "Should have agent stats"
        )
