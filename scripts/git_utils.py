...
class GitHelper:
            ...
            for line in result.stdout.split('\n'):
                if line.strip():
                    ...
                    if stripped_line.startswith(('*', '+')):
                        is_current = stripped_line.startswith('*')
    ...
    def add(self, paths: List[Path]) -> bool:
        ...
        try:
            ...
            for p in paths:
                if p.is_absolute():
                    try:
                        path_strs.append(p.relative_to(self.cwd).as_posix())
                    except ValueError:
                        path_strs.append(p.as_posix())
                else:
                    path_strs.append(p.as_posix())
    ...
    def get_staged_docs(self, docs_dir: str = "docs/") -> List[str]:
        ...
        """
        staged_files = self.get_staged_files(docs_dir)
        return [f for f in staged_files if f.lower().endswith('.md')]
