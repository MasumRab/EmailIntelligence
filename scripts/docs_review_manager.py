...
class DocsReviewManager:
    ...
    def create_review_request(
        self, doc_path: str, analysis: Dict, branch: str, priority: str = "medium"
    ):
...
"""
    ...
    def show_dashboard(self):
        ...
        if pending:
            for review in pending:
                priority_icon = (
                    ...
                    if review["priority"] == "medium"
                    else "🟢"
                )
                print(
                    f"  {priority_icon} {review['id']}: {Path(review['doc_path']).name}"
                )
                print(
                    f"      Branch: {review['branch']} | Strategy: {review['strategy']}"
                )
        ...
        if completed:
            ...
            branch_count = sum(
                1 for r in completed if r.get("decision") == "BRANCH_SPECIFIC"
            )
        ...
        if total > 0:
            ...
            print(
                f"  Average Reviews/Day: {total / max(1, (datetime.now() - datetime.fromisoformat(min(r['created_at'] for r in pending + completed))).days):.1f}"
            )
    ...
    def cleanup_old_reviews(self, days: int = 30):
        ...
        for review in queue["completed_reviews"]:
            review_date = datetime.fromisoformat(
                review.get("approved_at", review["created_at"])
            )
...
def main():
    ...
    parser.add_argument(
        "--dashboard", action="store_true", help="Show review dashboard"
    )
    parser.add_argument("--details", help="Show details for specific review ID")
    parser.add_argument(
        "--approve", help="Approve review (format: review_id:decision:reviewer:reason)"
    )
    parser.add_argument(
        "--create", help="Create review for document", metavar="DOC_PATH"
    )
    parser.add_argument("--branch", default="main", help="Branch for new review")
    parser.add_argument(
        "--cleanup", type=int, metavar="DAYS", help="Clean up reviews older than DAYS"
    )
