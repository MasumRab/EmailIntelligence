...
def get_all_branches() -> List[str]:
    ...
    branches = [
        line.strip().lstrip("*+ ") for line in output.split("\n") if line.strip()
    ]
    return branches
...
def sync_scripts_from_master(
    master_branch: str, target_branch: str, preserve_custom: bool = True
) -> bool:
    """
    ...
    target_has_scripts = (
        run_git_command(["ls-tree", "-d", f"{target_branch}:scripts"]) != ""
    )
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python script_sync.py <master_branch> [--all-branches] [--no-preserve]"
        )
