...
def test_bottleneck_detection():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        step1 = detector.start_workflow_step(
            step_id="step-1", step_name="content_validation", agent_id="agent-1"
        )

        step2 = detector.start_workflow_step(
            step_id="step-2", step_name="link_check", agent_id="agent-2"
        )

        # Complete the steps
        detector.complete_workflow_step("step-1", success=True)
        detector.complete_workflow_step(
            "step-2", success=False, error_message="Broken link"
        )
...
def test_long_running_step_detection():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        long_step = detector.start_workflow_step(
            "long-running", "quick-step", "agent-1"
        )

        # Manually update start time to simulate a long-running step
        with detector._lock:
            detector.workflow_steps["long-running"].start_time = (
                time.time() - 120
            )  # 2 minutes ago
            detector.workflow_steps["long-running"].duration = 120  # 2 minutes

        # Detect bottlenecks - should find the long-running step
        alerts = detector.detect_long_running_steps(
            duration_threshold_percentile=90, min_duration=10.0
        )
...
def test_queue_backlog_detection():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        detector.agent_workloads["overloaded-agent"] = (
            25  # Above double threshold of 20 (10 * 2)
        )
...
def test_dependency_block_detection():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        blocked_step = detector.start_workflow_step(
            "blocked-step", "blocked-task", "agent-1", dependencies=["missing-dep"]
        )

        # Manually set the start time to simulate a long wait
        blocked_step.start_time = (
            time.time() - 360
        )  # Waiting for 6 minutes (above 5-minute threshold)
...
def test_bottleneck_summary():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        step1 = detector.start_workflow_step(
            "summary-step-1", "validation", "summary-agent"
        )
        step2 = detector.start_workflow_step(
            "summary-step-2", "parsing", "summary-agent"
        )
...
def test_detection_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert (
            initial_summary["completed_steps"] == loaded_summary["completed_steps"]
