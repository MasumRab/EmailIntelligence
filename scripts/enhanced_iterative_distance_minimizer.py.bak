#!/usr/bin/env python3
"""
Enhanced Iterative Task Distance Minimizer

This script implements an enhanced iterative process to minimize the distance between:
1. Original task markdown files
2. Tasks recreated from an enhanced PRD

The process:
1. Generate enhanced PRD from original tasks (with better preservation of dependencies, effort, success criteria, and complexity)
...
Usage:
    python enhanced_iterative_distance_minimizer.py --original-dir /path/to/original/tasks --max-iterations 10
"""
...
from enhanced_reverse_engineer_prd import create_enhanced_reverse_engineered_prd, extract_task_info_from_md


def generate_improved_prd(task_files: List[str], iteration: int, prev_results: Dict[str, Any] = None) -> str:
    """
    Generate an enhanced PRD based on previous iteration results.
    ...
    Returns:
        String containing the enhanced PRD
    """
    # Use the enhanced reverse engineered PRD function
...
def simulate_task_generation_from_prd(prd_content: str, original_task_files: List[str]) -> str:
    ...
    tasks_json = {
        "master": {
            "name": "Task Master",
            "version": "1.0.0",
            "description": "Tasks generated from enhanced PRD (iteration simulation)",
            "tasks": []
        }
    }
...
def iterative_distance_minimization(original_dir: str, max_iterations: int = 10) -> Dict[str, Any]:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Generate enhanced PRD based on previous results
        ...
        prd_path = Path(f"enhanced_generated_prd_iteration_{iteration}.md")
        with open(prd_path, 'w', encoding='utf-8') as f:
            f.write(prd_content)
        
        print(f"Generated enhanced PRD for iteration {iteration}")
...
def print_final_report(results: Dict[str, Any]):
    ...
    """
    print("\n" + "=" * 80)
    print("ENHANCED ITERATIVE DISTANCE MINIMIZATION - FINAL REPORT")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced iterative minimization of distance between original and PRD-generated tasks")
    ...
    print("Starting enhanced iterative task distance minimization...")
