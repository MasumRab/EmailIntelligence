...
class GuideDevCommand(Command):
    ...
    def execute(self) -> int:
        ...
        Execute the command.
        """
        with WorkflowContextManager("guide-dev") as guide:
            guide.stage = "DEV_GUIDE_START"
            print("\nWelcome to the Development Workflow Guide.")
            print(
                "This tool will help you follow the correct process for making changes."
            )
            ...
            while True:
                ...
                print(
                    "  2. Modify a shared setup or configuration file (e.g., in 'setup/', '.flake8')"
                )
                print("  q. Quit")
                ...
                if choice == "1":
                    ...
                    print(
                        "✅ You can work on application code freely on any feature branch."
                    )
                    print(
                        "The orchestration system will automatically keep your local setup files in sync."
                    )
                    ...
                    try:
                        ...
                        if current_branch == "orchestration-tools":
                            ...
                            print(
                                "It is safe to edit shared setup and configuration files here."
                            )
                            print(
                                "Commit and push your changes to this branch, and they will be propagated automatically."
                            )
                        else:
                            guide.stage = "ORCH_CODE_UNSAFE"
                            print(
                                f"❌ You are currently on the '{current_branch}' branch, not 'orchestration-tools'."
                            )
                            print(
                                "Changes to shared files on this branch will be overwritten by the sync hooks."
                            )
                            print("\nRecommendation:")
                            print(
                                "  1. Stash your changes if you have any (`git stash`)."
                            )
                            print(
                                "  2. Switch to the correct branch (`git checkout orchestration-tools`)."
                            )
                            print(
                                "  3. Re-apply your changes if needed (`git stash pop`)."
                            )
                        break
