...
class GuidePrCommand(Command):
    ...
    def execute(self) -> int:
        ...
        Execute the command.
        """
        with WorkflowContextManager("guide-pr") as guide:
            guide.stage = "PR_GUIDE_START"
            print("\nWelcome to the PR Resolution Workflow Guide.")
            print(
                "This tool will help you follow the correct process for merging branches."
            )

            while True:
                print("\nWhat kind of branch are you merging?")
                print(
                    "  1. A branch with changes to shared setup/config files (an 'orchestration' change)."
                )
                print(
                    "  2. A branch with only application code changes (e.g., in 'src/', 'client/')."
                )
                print("  q. Quit")
                ...
                if choice == "1":
                    ...
                    print(
                        "To safely merge these changes into the 'orchestration-tools' branch, you must use the reverse sync script."
                    )
                    print("\nRecommended Command:")
                    print(
                        "  ./scripts/reverse_sync_orchestration.sh <your-branch-name> <commit-sha>"
                    )
                    print(
                    ...
                    print(
                        "You can use 'git merge' to merge your branch into 'main' or 'scientific'."
                    )
                    print("\nRecommendation:")
                    print(
                        "  1. Ensure your feature branch is up to date with the target branch (`git pull origin main`)."
                    )
                    print("  2. Checkout the target branch (`git checkout main`).")
                    print(
                        "  3. Merge your feature branch (`git merge feature/your-feature`)."
                    )
                    print(
                        "  4. Run the full validation suite as outlined in 'final_merge_approach.md' before pushing the merge."
                    )
                    break
                elif choice.lower() == "q":
                    print("Exiting guide.")
