...
class CompletionPredictor:
    def __init__(self, prediction_file: Path = None, max_history: int = 10000):
        ...
        self.task_type_stats: Dict[str, Dict[str, List]] = defaultdict(
            lambda: defaultdict(list)
        )
        self.agent_stats: Dict[str, Dict[str, List]] = defaultdict(
            lambda: defaultdict(list)
        )
        self.feature_stats: Dict[str, Dict[str, List]] = defaultdict(
            lambda: defaultdict(list)
        )
    ...
    def record_task_completion(self, record: TaskRecord):
        ...
        with self._lock:
            ...
            self.task_type_stats[record.task_type]["success_rates"].append(
                1.0 if record.success else 0.0
            )

            # Update agent statistics
            self.agent_stats[record.agent_id]["durations"].append(record.duration)
            self.agent_stats[record.agent_id]["success_rates"].append(
                1.0 if record.success else 0.0
            )
    ...
    def _get_feature_adjustment(
        self, task_type: str, features: Dict[str, Any]
    ) -> float:
            ...
            for feature_name, feature_value in features.items():
                ...
                try:
                    ...
                    if avg_value != 0:
                        ...
                        adjustment = (
                            correlation * value_ratio * statistics.mean(durations)
                        )
    ...
    def _calculate_prediction_confidence(
        self, task_type: str, agent_id: str, features: Dict[str, Any]
    ) -> float:
        ...
        with self._lock:
            ...
            if agent_durations:
                agent_sample_confidence = min(
                    1.0, len(agent_durations) / 10.0
            ...
            if features:
                ...
                for feature_name in features.keys():
                    ...
                    if feature_key in self.feature_stats:
                        ...
                        feature_confidences.append(
                            0.3
                        )  # Low confidence for unknown features
                ...
                if feature_confidences:
                    ...
                    confidence_factors.append(
                        0.4
                    )  # Medium-low confidence if no features
            else:
                confidence_factors.append(
                    0.6
                )  # Higher confidence if no features needed
        ...
        with self._lock:
            ...
            for record in self.task_records:
                if (
                    ...
                    and record.task_id not in similar_tasks
                ):
...
class PredictionDashboard:
    ...
    def display_prediction(self, prediction: PredictionResult):
        ...
        print("=" * 50)
        print(
            f"Predicted Duration: {prediction.predicted_duration:.1f} seconds "
    ...
    def display_prediction_trends(self):
        ...
        if trends["trend_data_available"]:
            ...
            print(
                f"Confidence-Accuracy Correlation: {trends['confidence_correlation_with_accuracy']:.2f}"
            )
        else:
