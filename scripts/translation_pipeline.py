...
class DistributedTranslationManager:
    def __init__(self, translation_file: Path = None):
        self.translation_file = translation_file or Path(
            ".distributed_translations.json"
        )
        self.translation_jobs: Dict[str, TranslationJob] = {}
        self.translation_memory: Dict[str, TranslationMemoryEntry] = {}
        self.agent_assignments: Dict[str, List[str]] = defaultdict(
            list
        )  # agent_id -> job_ids
        self.language_pairs: Dict[str, List[str]] = defaultdict(
            list
        )  # source_lang -> target_langs
        self.agent_capabilities: Dict[str, List[Tuple[str, str]]] = defaultdict(
            list
    ...
    def register_agent_capability(
        self, agent_id: str, source_lang: str, target_lang: str
    ):
    ...
    def create_translation_job(
        ...
        job_id = self.start_translation_job(
            document_id, source_language, target_languages, metadata
        )
        if source_segments:
    ...
    def complete_translation_unit(
        self, unit_id: str, translated_text: str, quality_score: float = 0.8
    ) -> bool:
        ...
        with self._lock:
            ...
            unit.translation_time = (
                time.time() - unit.translation_time if unit.translation_time > 0 else 0
            )
    ...
    def _add_to_translation_memory(
        self,
        source_text: str,
        target_text: str,
        source_lang: str,
        ...
        if key in self.translation_memory:
            ...
            new_confidence = (
                entry.confidence_score * entry.usage_count + confidence_score
            ) / (entry.usage_count + 1)
    ...
    def get_job_progress(self, job_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            completed_units = len(
                [u for u in job.translation_units if u.status == "completed"]
            )
            failed_units = len(
                [u for u in job.translation_units if u.status == "failed"]
            )
            in_progress_units = len(
                [u for u in job.translation_units if u.status == "in_progress"]
            )
            pending_units = len(
                [u for u in job.translation_units if u.status == "pending"]
            )
            ...
            for target_lang in job.target_languages:
                lang_units = [
                    u for u in job.translation_units if u.target_language == target_lang
                ]
                total_lang = len(lang_units)
                completed_lang = len([u for u in lang_units if u.status == "completed"])
                progress_percent = (
                    (completed_lang / total_lang * 100) if total_lang > 0 else 0
                )
    ...
    def get_agent_assignments(self, agent_id: str) -> List[TranslationJob]:
        ...
        with self._lock:
            ...
            return [
                self.translation_jobs[jid]
                for jid in job_ids
                if jid in self.translation_jobs
            ]
    ...
    def get_translation_quality_report(
        self, job_id: str, target_language: str
        ...
        with self._lock:
            ...
            if not job:
                return TranslationQualityReport(
                    job_id, target_language, 0, 0, 0, 0.0, 0.0, 0.0
                )

            # Get units for the specific language
            lang_units = [
                u for u in job.translation_units if u.target_language == target_language
            ]
            ...
            if completed_units:
                avg_quality = sum(u.quality_score for u in completed_units) / len(
                    completed_units
                )
            else:
                avg_quality = 0.0
    ...
    def _calculate_consistency_score(self, units: List[TranslationUnit]) -> float:
        ...
        return (
            (consistent_count / total_repeated * 100) if total_repeated > 0 else 100.0
        )
    ...
    def get_translation_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            completed_jobs = len(
                [j for j in self.translation_jobs.values() if j.status == "completed"]
            )
            in_progress_jobs = len(
                [j for j in self.translation_jobs.values() if j.status == "in_progress"]
            )
            ...
            return {
                ...
                "completed_units": completed_units,
                "failed_units": failed_units,
                "unit_completion_rate": (completed_units / total_units * 100)
                if total_units > 0
                else 0,
                "failed_unit_rate": (failed_units / total_units * 100)
                if total_units > 0
                else 0,
...
class TranslationDashboard:
    ...
    def display_job_overview(self, job_id: str):
        ...
        for lang, lang_progress in language_progress.items():
            print(
                f"  {lang}: {lang_progress['completed_units']}/{lang_progress['total_units']} "
    ...
    def display_translation_quality(self, job_id: str):
        ...
        for target_lang in job.target_languages:
            report = self.translation_manager.get_translation_quality_report(
                job_id, target_lang
            )
    ...
    def display_agent_assignments(self, agent_id: str):
        ...
        for job in jobs:
            ...
            print(
                f"  Languages: {job.source_language} → {', '.join(job.target_languages)}"
            )
            print(f"  Status: {job.status}")
    ...
    def display_translation_memory_stats(self):
        ...
        for i, entry in enumerate(sorted_entries, 1):
            ...
            print(
                f"     Quality: {entry.quality_score:.1f}%, Used: {entry.usage_count} times"
            )
