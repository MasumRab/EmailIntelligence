...
class PrioritySyncQueue:
    ...
    def get_next_task(self) -> Optional[SyncTask]:
        ...
        while self.task_queue and (self.task_queue[0].task_id in self.completed_tasks or
                                  self.task_queue[0].task_id in self.failed_tasks):
    ...
    def get_queue_stats(self) -> Dict:
        ...
        pending_count = len([t for t in self.task_queue
                           if t.task_id not in self.completed_tasks and
                              t.task_id not in self.failed_tasks])
...
class SyncPrioritizer:
    ...
    def create_sync_task(self, worker_id: str, files: List[str],
                        sync_reason: str = "routine") -> SyncTask:
    ...
    def _determine_priority(self, sync_reason: str, files: List[str]) -> Priority:
        ...
        for file_path in files:
            if any(keyword in file_path.lower() for keyword in ['api', 'config', 'settings']):
                return Priority.HIGH
    ...
    def _determine_sync_type(self, sync_reason: str) -> SyncType:
        ...
        elif "background" in sync_reason.lower() or "maintenance" in sync_reason.lower():
    ...
    def get_priority_distribution(self) -> Dict:
        ...
        for task in self.priority_queue.task_queue:
            if task.task_id not in self.priority_queue.completed_tasks and \
               task.task_id not in self.priority_queue.failed_tasks:
...
def main():
    ...
    task1 = prioritizer.create_sync_task("worker1", ["docs/api.md", "docs/guide.md"], "urgent")
    task2 = prioritizer.create_sync_task("worker2", ["docs/changelog.md"], "routine")
    task3 = prioritizer.create_sync_task("worker3", ["config/settings.json"], "critical")
