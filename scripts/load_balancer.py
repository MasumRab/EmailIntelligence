...
class LoadBalancer:
    ...
    def update_agent_performance(
        self, agent_name: str, task_completion_time: int, success: bool = True
    ):
        ...
        if agent_name in self.performance_history:
            ...
            if len(self.performance_history[agent_name]) > 100:
                self.performance_history[agent_name] = self.performance_history[
                    agent_name
                ][-100:]
    ...
    def get_load_balancing_stats(self) -> Dict:
        ...
        for agent in self.router.agents:
            stats["agent_efficiencies"][agent.name] = self.get_agent_efficiency(
                agent.name
            )
    ...
    def _get_average_utilization(self) -> float:
        ...
        total_utilization = sum(
            agent.get_utilization_rate() for agent in self.router.agents
        )
