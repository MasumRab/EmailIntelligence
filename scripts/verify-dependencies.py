...
class DependencyVerifier:
    ...
    def check_import_availability(self, package_name: str) -> Tuple[bool, str]:
        ...
        try:
            # Handle package name differences (e.g., python-dotenv -> dotenv)
    ...
    def generate_conditional_requirements(self) -> str:
        ...
        # Pre-compute joined strings to avoid backslashes in f-string expressions
        newline = "\n"
        base_reqs_str = newline.join(f"{req}>=1.0.0" for req in base_reqs)
        ml_reqs_str = newline + "# ".join(f"{req}>=1.0.0" for req in categories['ml'])
        data_reqs_str = newline + "# ".join(f"{req}>=1.0.0" for req in categories['data'])
        db_reqs_str = newline + "# ".join(f"{req}>=1.0.0" for req in categories['database'])
        gpu_reqs_str = newline + "# ".join(f"{req}>=1.0.0" for req in categories['gpu'])
