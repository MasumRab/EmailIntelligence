...
class ChangeDetector:
    ...
    def save_hash_cache(self):
        ...
        try:
            ...
            for path, file_hash in self.hash_cache.items():
                data[path] = {"hash": file_hash.hash, "mtime": file_hash.mtime}
    ...
    def detect_changes(
        self, source_dir: Path, target_dir: Path, extensions: Set[str] = None
    ) -> Dict[str, List[str]]:
...
class IncrementalSync:
    ...
    def sync_changes(self, extensions: Set[str] = None) -> Dict[str, int]:
        ...
        changes = self.change_detector.detect_changes(
            self.source_dir, self.target_dir, extensions
        )
