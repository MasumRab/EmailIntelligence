...
class DocsMaintenance:
    ...
    def check_system_health(self) -> Dict:
        ...
        # Check sync scripts
        sync_scripts = ['sync-common-docs.sh', 'sync_common_docs.py', 'auto_sync_docs.py']
        ...
        for worktree in worktrees:
            docs_dirs.extend([
                self.project_root / "worktrees" / worktree / "docs" / "common" / "docs",
                self.project_root / "worktrees" / worktree / self.config.get('sync_rules', {}).get('branch_docs_paths', {}).get(worktree, f"docs/{worktree.split('-')[1]}")
            ])
    ...
    def check_documentation_integrity(self) -> Dict:
        ...
        for worktree in worktrees:
            ...
            if missing:
                integrity['missing_files'].extend([f"{worktree}:{file}" for file in missing])

            # Find orphaned files (files in worktree but not in base)
            orphaned = worktree_files - base_files
            if orphaned:
                integrity['orphaned_files'].extend([f"{worktree}:{file}" for file in orphaned])
        ...
        total_issues = len(integrity['missing_files']) + len(integrity['orphaned_files']) + len(integrity['inconsistent_sync']) + len(integrity['misplaced_files'])
    ...
    def _find_misplaced_documentation(self) -> List[str]:
        ...
        forbidden_patterns = [
            'src/', 'python_backend/', 'client/', 'backend/', 'modules/',
            'tests/', 'scripts/', 'deployment/', 'config/', 'data/',
            'models/', 'plugins/', 'shared/', 'venv/', 'node_modules/',
            '__pycache__/', '.git/', 'logs/', '.github/',
            'jules-scratch/', 'mock_models/', 'email_intelligence.egg-info/',
            'temp_retrieve/', 'performance_metrics_log.jsonl'
        ]
        ...
        try:
            ...
            if destination_path:
                ...
                if destination_path.exists():
                    ...
                    backup_path = destination_path.with_suffix(f"{destination_path.suffix}.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
                    shutil.copy2(destination_path, backup_path)
                    print(f"    Backed up existing file: {backup_path.name}")

                # Move the file
                shutil.move(source_path, destination_path)
                print(f"    Moved misplaced file: {misplaced_path} -> {destination_path.relative_to(self.project_root)}")
                return True
    ...
    def _determine_correct_location(self, misplaced_path: str) -> Optional[Path]:
        ...
        if '---' in first_lines and ('id: task-' in first_lines or 'status:' in first_lines):
            ...
            if 'worktrees/docs-main' in misplaced_path:
                return self.project_root / "worktrees" / "docs-main" / "backlog" / "tasks" / file_name
            elif 'worktrees/docs-scientific' in misplaced_path:
                return self.project_root / "worktrees" / "docs-scientific" / "backlog" / "tasks" / file_name
            else:
                # Default to main worktree
                return self.project_root / "worktrees" / "docs-main" / "backlog" / "tasks" / file_name

        # Check if it's documentation content (not a task)
        elif any(keyword in first_lines for keyword in ['# ', '## ', '### ', 'overview', 'introduction', 'guide']):
            # This is general documentation - should go in branch-specific docs
            if 'worktrees/docs-main' in misplaced_path:
                return self.project_root / "worktrees" / "docs-main" / "docs" / "main" / file_name
            elif 'worktrees/docs-scientific' in misplaced_path:
                return self.project_root / "worktrees" / "docs-scientific" / "docs" / "scientific" / file_name
            else:
    ...
    def generate_maintenance_report(self, results: Dict):
        ...
        report_path = self.project_root / "logs" / f"maintenance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

        report = {
            'timestamp': datetime.now().isoformat(),
            'results': results,
            'summary': {
                'overall_health': 'good' if not any(r.get('issues', []) for r in results.values() if isinstance(r, dict)) else 'needs_attention',
                'total_issues': sum(len(r.get('issues', [])) for r in results.values() if isinstance(r, dict)),
                'integrity_score': results.get('integrity', {}).get('integrity_score', 0)
            }
        }
...
def main():
    ...
    parser.add_argument(
        "--full", "-f",
        action="store_true",
        help="Run full maintenance suite"
    )
    parser.add_argument(
        "--health",
        action="store_true",
    ...
    parser.add_argument(
        "--cleanup", "-c",
        action="store_true",
        help="Perform cleanup tasks only"
    )
    parser.add_argument(
        "--integrity", "-i",
        action="store_true",
    ...
    if args.fix_misplaced:
        ...
        if misplaced_files:
            ...
            print(f"✅ Successfully moved {moved_count} out of {len(misplaced_files)} files")
        else:
            print("✅ No misplaced documentation files found")
    elif args.full or not any([args.health, args.cleanup, args.integrity, args.fix_misplaced]):
