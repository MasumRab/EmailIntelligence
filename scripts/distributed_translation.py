...
class DistributedTranslationManager:
    def __init__(self, translation_file: Path = None):
        ...
        self.agent_capabilities: Dict[str, List[str]] = defaultdict(
            list
        )  # agent_id -> language_pairs
        self._lock = threading.RLock()
        self.load_translation_data()

    def register_agent_capability(
        self, agent_id: str, source_language: str, target_language: str
    ) -> bool:
    ...
    def translate_segment(
        ...
        with self._lock:
            ...
            segment.translation_time = (
                time.time() - job.start_time if job.start_time > 0 else 0
            )
    ...
    def _add_to_translation_memory(
        ...
        key = hashlib.md5(
            f"{source_text}-{source_language}-{target_language}".encode()
        ).hexdigest()
    ...
    def get_job_progress(self, job_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            completed_segments = len(
                [s for s in job.segments if s.translation_status == "completed"]
            )
            in_progress_segments = len(
                [s for s in job.segments if s.translation_status == "in_progress"]
            )
            pending_segments = len(
                [s for s in job.segments if s.translation_status == "pending"]
            )
            ...
            return {
                ...
                "progress_percentage": (completed_segments / total_segments * 100)
                if total_segments > 0
                else 0,
    ...
    def get_job_quality_metrics(self, job_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            average_quality_score = (
                total_quality_score / total_reports if total_reports > 0 else 0.0
            )
            ...
            return {
                ...
                "reports_per_segment": total_reports / len(job.segments)
                if job.segments
                else 0,
            }
        ...
        with self._lock:
            ...
            if total_entries > 0:
                total_confidence = sum(
                    entry.confidence_score for entry in self.translation_memory.values()
                )
    ...
    def get_agent_translation_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Calculate average confidence
            total_confidence = sum(
                segment.confidence_score for segment in agent_segments
            )
            average_confidence = (
            ...
            for job_id, job_data in data.get("translation_jobs", {}).items():
                segments = [
                    TranslationSegment(
                        ...
                        translation_status=segment_data.get(
...
class TranslationDashboard:
    ...
    def display_job_overview(self, job_id: str):
        ...
        print(
            f"Assigned Agents: {len(job.assigned_agents)} ({', '.join(job.assigned_agents)})"
        )
        ...
        print("\nQuality Metrics:")
        print(
            f"  Average Quality Score: {quality_metrics.get('average_quality_score', 0):.2f}"
        )
        print(
            f"  Segments with Reports: {quality_metrics.get('segments_with_quality_reports', 0)}/{quality_metrics.get('total_segments', 0)}"
        )
    ...
    def display_job_segments(self, job_id: str):
        ...
        for i, segment in enumerate(job.segments, 1):
            ...
            print(
                f"   Source ({segment.source_language}): {segment.source_text[:100]}{'...' if len(segment.source_text) > 100 else ''}"
            )
            if segment.translated_text:
                print(
                    f"   Target ({segment.target_language}): {segment.translated_text[:100]}{'...' if len(segment.translated_text) > 100 else ''}"
                )
