...
class TaskCompletionTracker:
    def __init__(self, tracking_file: Path = None, max_history: int = 10000):
        ...
        self.agent_task_counts: Dict[str, Dict[str, int]] = defaultdict(
    ...
    def record_task_completion(self, record: TaskCompletionRecord):
        ...
        with self._lock:
            ...
            self.task_type_stats[record.task_type]["successes"].append(
    ...
    def get_completion_rate(
        self, agent_id: str = None, task_type: str = None, time_window_hours: int = 24
    ) -> float:
        ...
        with self._lock:
            ...
            relevant_records = [
                ...
                if record.end_time >= cutoff_time
    ...
    def get_agent_performance_report(
        ...
        with self._lock:
            ...
            avg_time = self.get_average_completion_time(
                agent_id, time_window_hours=time_window_hours
            )
    ...
    def get_task_type_performance_report(
        self, task_type: str, time_window_hours: int = 24
    ) -> Dict[str, Any]:
        """Get detailed performance report for a task type."""
        with self._lock:
            completion_rate = self.get_completion_rate(
                task_type=task_type, time_window_hours=time_window_hours
            )
            avg_time = self.get_average_completion_time(
                task_type=task_type, time_window_hours=time_window_hours
            )
            ...
            successes = self.task_type_stats[task_type]["successes"][
                -1000:
            ]  # Last 1000 samples
            if successes:
                success_rate_stats = {
                    "mean": statistics.mean(successes) * 100,
                    "stdev": statistics.stdev(successes) * 100
                    if len(successes) > 1
                    else 0.0,
                }
            else:
    ...
    def get_underperforming_agents(
        self, threshold_rate: float = 80.0, time_window_hours: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            for agent_id in self.agent_task_counts.keys():
                ...
                if rate < threshold_rate:
                    report = self.get_agent_performance_report(
                        agent_id, time_window_hours
                    )
    ...
    def get_performance_alerts(
        self, completion_rate_threshold: float = 80.0, time_window_hours: int = 24
    ) -> List[Dict[str, Any]]:
