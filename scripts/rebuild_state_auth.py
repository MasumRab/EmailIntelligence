...
commented_raw = run_cmd('GH_TOKEN=$GITHUB_TOKEN gh pr list --search \'in:comments "Blocked by PR"\' --state open --limit 500 --json number')
commented_prs = set(pr['number'] for pr in json.loads(commented_raw)) if commented_raw else set()
print(f"Found {len(commented_prs)} commented PRs.")
...
for pr in all_prs:
    ...
    if num in commented_prs:
        comments_raw = run_cmd(f'GH_TOKEN=$GITHUB_TOKEN gh pr view {num} --json comments')
        if comments_raw:
            try:
                comments = json.loads(comments_raw).get('comments', [])
                for c in comments:
                    body = c.get('body', '')
                    if "blocked by pr" in body.lower():
...
print(f"Successfully rebuilt JSON state with dependencies for {len(state)} PRs.")
