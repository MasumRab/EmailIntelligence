...
def test_performance_monitoring():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert retrieved_metrics1[0].tasks_completed == 5, (
            "Tasks completed should match"
        )
        ...
        assert retrieved_metrics2[0].tasks_completed == 3, (
            "Tasks completed should match"
        )
...
def test_performance_summary():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert summary["total_tasks_completed"] == 55, (
            "Should sum all tasks (1+2+3+...+10)"
        )
        assert summary["sample_count"] == 10, "Should have 10 samples"
        assert summary["average_success_rate"] > 90.0, (
            "Average success rate should be > 90%"
        )
