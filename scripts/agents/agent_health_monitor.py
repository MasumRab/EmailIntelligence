...
class AgentHealthMetrics:
    ...
    def get_health_score(self) -> float:
        ...
        if self.heartbeat_timestamps:
            ...
            heartbeat_score = max(
                0.0, 1.0 - (time_since_heartbeat.total_seconds() / 300.0)
            )
        else:
            heartbeat_score = 0.0
...
class AgentHealthMonitor:
            ...
            try:
                with open(self.metrics_file, "r") as f:
                    ...
                    for agent_name, metrics_data in data.items():
                        ...
                        metrics.task_success_rate = metrics_data.get(
                            "task_success_rate", []
                        )
                        metrics.heartbeat_timestamps = metrics_data.get(
                            "heartbeat_timestamps", []
                        )
                        metrics.alerts = metrics_data.get("alerts", [])
                        metrics.last_check = metrics_data.get(
    ...
    def update_system_metrics(
        self, agent_name: str, cpu_percent: float, memory_percent: float
    ):
        """Update system metrics for an agent."""
        if agent_name in self.agent_metrics:
            self.agent_metrics[agent_name].add_system_metrics(
        ...
        for agent_name, metrics in self.agent_metrics.items():
            if metrics.heartbeat_timestamps:
                last_heartbeat = datetime.fromisoformat(
                    metrics.heartbeat_timestamps[-1]
                )
    ...
    def get_agent_health(self, agent_name: str) -> Optional[Dict]:
        ...
        return {
            ...
            "success_rate": metrics.task_success_rate[-1]
            if metrics.task_success_rate
            else None,
    ...
    def get_system_overview(self) -> Dict:
        ...
        attention_agents = len(
            [h for h in health_data.values() if h and h["needs_attention"]]
        )
