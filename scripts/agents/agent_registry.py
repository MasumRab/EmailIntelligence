...
class AgentProfile:
    ...
    def add_training(
        self, training_name: str, completion_date: str, score: Optional[float] = None
    ):
    ...
    def update_performance(
        ...
        if success_rate is not None:
            ...
            self.performance_metrics["success_rate"] = (
                ...
                if (current_tasks + tasks_completed) > 0
                else success_rate
            )
        ...
        if avg_completion_time is not None:
            ...
            self.performance_metrics["avg_completion_time"] = (
                (
                    (
                        current_time * current_tasks
                        + avg_completion_time * tasks_completed
                    )
                    / (current_tasks + tasks_completed)
                )
                if (current_tasks + tasks_completed) > 0
                else avg_completion_time
            )
...
class AgentCapabilityRegistry:
    ...
    def get_agents_with_skill(
        self, skill_name: str, min_proficiency: int = 1
    ) -> List[AgentProfile]:
    ...
    def match_agent_to_task(
        self, task_type: str, required_skills: Optional[List[str]] = None
    ) -> List[AgentProfile]:
        ...
        if self.registry_file.exists():
            try:
                ...
                for name, agent_data in data.get("agents", {}).items():
                    ...
                    profile.training_completed = agent_data.get(
                        "training_completed", []
                    )
                    profile.performance_metrics = agent_data.get(
                        "performance_metrics",
                        {
                            "tasks_completed": 0,
                            "success_rate": 0.0,
                    ...
                    profile.last_updated = agent_data.get(
                ...
                for name, cap_data in data.get("capabilities", {}).items():
                    capability = Capability(
                        cap_data["name"], cap_data["description"], cap_data["category"]
                    )
                    capability.created_at = cap_data.get(
    ...
    def get_registry_stats(self) -> Dict:
        ...
        return {
            ...
            "recently_updated": [
                profile.agent_name
                for profile in self.agents.values()
                if datetime.fromisoformat(profile.last_updated)
