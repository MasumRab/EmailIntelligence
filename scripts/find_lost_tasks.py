...
def load_current_tasks(script_dir):
    ...
    if invalid_file.exists():
        try:
            ...
            tasks = extract_tasks_regex(open(invalid_file).read())
            for task in tasks:
                current_tasks.add(get_task_signature(task))
...
def main():
    ...
    parser.add_argument(
        "--commits", type=int, default=50, help="Max commits to check (default: 50)"
    )
            ...
            for task in tasks:
                ...
                if sig not in current_tasks:
                    all_historical_tasks[sig].append(
                        {"commit": commit, "file": task_file, "task": task}
                    )
    ...
    print(
        f"Found {len(all_historical_tasks)} unique tasks in history that are not in current files:\n"
    )
