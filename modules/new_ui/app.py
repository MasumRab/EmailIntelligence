...
with gr.Blocks() as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🎯 Smart Filters"):
            ...
            with gr.Row():
                with gr.Column():
                    filter_name = gr.Textbox(label="Filter Name", placeholder="e.g., Urgent Work Emails")
                    filter_criteria = gr.Code(
                        label="Filter Criteria (JSON)",
                        language="json",
                        value='{\n  "subject_keywords": ["urgent", "important"],\n  "from_patterns": ["@company.com"]\n}',
                    )
                    filter_priority = gr.Slider(label="Priority", minimum=1, maximum=10, value=5, step=1)
