...
def create_overview_tab():
    ...
    with gr.Row():
        with gr.Column():
            ...
            gr.Textbox(label="System Uptime", interactive=False)

            gr.Textbox(label="Last Updated", interactive=False)
...
async def perform_health_checks() -> HealthCheck:
    ...
    overall_status = (
        "healthy" if all(check.get("status") == "healthy" for check in health_results.values()) else "unhealthy"
    )
