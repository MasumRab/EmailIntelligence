...
def create_inbox_ui():
    ...
    with gr.Blocks() as inbox_tab:
        ...
        email_list = gr.DataFrame(
            headers=["Subject", "From", "Date"], label="Emails", interactive=True
        )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                email_content = gr.Textbox(
                    label="Email Content", lines=15, interactive=False
                )
            with gr.Column(scale=1):
        ...
        async def on_email_select(evt: gr.SelectData, df: pd.DataFrame):
            ...
            if evt.index is not None and not df.empty:
                try:
                    ...
                    content += (
                        "This is a sample email content for demonstration purposes. "
                    )
                    ...
                    metadata = {
                        ...
                        "size": "2.4 KB",
                        "tags": ["inbox", "unread"]
                        if "Unread" in df.columns and selected_row.get("Unread", False)
                        else ["inbox"],
                    }
                ...
                if hasattr(data_source, "search_emails"):
                    ...
                    if emails:
                        ...
                        if not df.empty:
                            ...
                            columns_to_show = ["subject", "sender", "date"]
                            available_columns = [
                                col for col in columns_to_show if col in df.columns
                            ]
                            if available_columns:
                                display_df = df[available_columns].copy()
                                display_df.columns = [
                                    col.title() for col in available_columns
                                ]
                                return display_df
        ...
        # Event handlers
        refresh_btn.click(
            fn=lambda: asyncio.run(refresh_inbox()), inputs=[], outputs=[email_list]
        )
