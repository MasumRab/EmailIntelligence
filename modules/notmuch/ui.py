    ...
    with gr.Blocks() as notmuch_tab:
        ...
        with gr.Row():
            tag_input = gr.Textbox(
                label="Tags", placeholder="Enter comma-separated tags..."
            )
        ...
        async def update_tags_callback(
            message_id: str, tags_str: str, current_query: str
        ):
            if not message_id:
        ...
        update_button.click(
            fn=lambda msg_id, tags, query: asyncio.run(
                update_tags_callback(msg_id, tags, query)
            ),
