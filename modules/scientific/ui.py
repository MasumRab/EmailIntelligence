...
async def analyze_batch_emails(emails_data: str):
    ...
    try:
        ...
        for email in emails:
            ...
            try:
                ...
                if hasattr(data_source, "ai_engine") and data_source.ai_engine:
                    ...
                    result_entry = {
                        "subject": subject,
                        "topic": analysis_result.get("topic", ["General"])[0]
                        if isinstance(analysis_result.get("topic"), list)
                        else analysis_result.get("topic", "General"),
                        "sentiment": analysis_result.get("sentiment", {}).get(
                            "label", "unknown"
                        ),
                        "intent": analysis_result.get("intent", {}).get(
                            "type", "unknown"
                        ),
                        "urgency": analysis_result.get("urgency", {}).get(
                            "level", "unknown"
                        ),
                    }
        ...
        if results:
            ...
            df = pd.DataFrame(
                columns=["subject", "topic", "sentiment", "intent", "urgency"]
            )
        ...
        with gr.Row():
            ...
            with gr.Column():
                batch_output = gr.Dataframe(
                    label="Batch Analysis Results", interactive=False
                )
