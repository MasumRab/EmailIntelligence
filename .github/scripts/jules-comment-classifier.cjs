...
function classifyContent(body) {
  ...
  const hasWalkthrough = WALKTHROUGH_MARKERS.some((m) =>
    m === MARKERS.WALKTHROUGH_END ? body.includes(m) : body.toLowerCase().includes(m.toLowerCase())
  )
...
async function fetchUnresolvedThreads(github, owner, repo, prNumber) {
  ...
  return threads
    ...
    .map((t) => {
      ...
      const authorType =
        last?.author?.__typename || (authorLogin?.endsWith('[bot]') ? 'Bot' : 'User')
      return {
        isResolved: false,
        lastComment: last
          ? {
...
async function classifyAndFilter(github, owner, repo, prNumber, opts = {}) {
  ...
  if (includeReviewComments)
    fetches.push({
      key: 'reviewComments',
      promise: fetchReviewComments(github, owner, repo, prNumber),
    })
  if (includeReviews)
    fetches.push({ key: 'reviews', promise: fetchReviews(github, owner, repo, prNumber) })
  if (includeThreads)
    fetches.push({
      key: 'unresolvedThreads',
      promise: fetchUnresolvedThreads(github, owner, repo, prNumber),
    })
  ...
  fetches.forEach((f, i) => {
    if (settled[i].status === 'fulfilled') {
      ...
      core?.warning?.(
        `Fetch failed for ${f.key}: ${settled[i].reason?.message || settled[i].reason}`
      )
      results[f.key] = []
    }
  })
  ...
  const hasWalkthrough =
    issueComments.some((c) => c.contentClass.hasWalkthrough) ||
  ...
  return {
    ...
    _meta: {
      surfaces: [
        'issue-comments',
        ...(includeReviewComments ? ['review-comments'] : []),
      ...
      retainedReviewComments:
        reviewComments.length -
        reviewComments.filter((c) => {
          return (
            c.authorSource === SOURCES.JULES_BOT ||
