...
def get_conda_env_info():
    ...
    return {
        ...
        "prefix": conda_prefix,
        "python_exe": env_vars.get("CONDA_PREFIX", "") + "/python"
        if conda_prefix
        else None,
    }
...
def install_notmuch_matching_system():
    try:
        ...
        if not run_command(
            ...
            logger.warning(
                f"Could not install notmuch {major_minor}, trying latest version"
            )
        run_command(
...
def main():
    ...
    # Orchestration check command
    parser.add_argument(
        "--check", action="store_true", help="Run orchestration environment checks."
    )
    parser.add_argument(
        "--critical-files",
        action="store_true",
    ...
    parser.add_argument(
        "--env-check",
        action="store_true",
        help="Check orchestration environment validation.",
    )
...
def _execute_check_command(args) -> int:
    ...
    if args.critical_files or (not args.env_check):
        if not check_critical_files():
            success = False
    ...
    if not args.critical_files and not args.env_check:
        if not validate_orchestration_environment():
            success = False
