...
try:
    ...
    # Only show warning if not running tests
    if "PYTEST_CURRENT_TEST" not in os.environ:
        logging.warning(
            f"Could not import core modules: {e}. Some features may be unavailable."
        )
...
def validate_orchestration_environment() -> bool:
    ...
    if not is_conda_available():
        if env_name:
            logger.warning(
                f"Conda not available, cannot activate environment '{env_name}'. Please install Conda."
            )
        else:
    ...
    if conda_info["is_active"]:
        if conda_info["env_name"] == env_name:
            logger.info(
                f"Already in specified conda environment: {conda_info['env_name']}"
            )
            return True
        else:
            logger.warning(
