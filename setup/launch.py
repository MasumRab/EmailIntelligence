...
from setup.environment import (
    ...
    create_venv, setup_dependencies, download_nltk_data
)
...
def _handle_legacy_args(args) -> int:
    ...
    if args.use_conda:
        ...
        if not get_conda_env_info()["is_active"] and not activate_conda_env(args.conda_env):
            ...
            if conda_info["env_name"] == args.conda_env:
                ...
                logger.warning(
                    ...
                    f"but '{args.conda_env}' was requested. "
                    f"Please activate '{args.conda_env}' manually before running the script."
                )
                return 1
