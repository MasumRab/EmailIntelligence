...
def install_package_manager(venv_path: Path, manager: str):
    ...
    try:
        proc = subprocess.run(["python", "-m", "pip", "install", manager], executable=str(python_exe), check=True, text=True, capture_output=True)
        if proc.stdout:
            logger.debug(proc.stdout)
        if proc.stderr:
            logger.warning(proc.stderr)
    except (subprocess.CalledProcessError, FileNotFoundError) as e:
        logger.error(f"Failed: {manager}")
...
def setup_dependencies(venv_path: Path, use_poetry: bool = False):
    ...
    if use_poetry:
        ...
        try:
            proc = subprocess.run(["python", "-m", "pip", "install", "--upgrade", "pip"], executable=str(python_exe), check=True, text=True, capture_output=True)
            if proc.stdout:
                logger.debug(proc.stdout)
            if proc.stderr:
                logger.warning(proc.stderr)
        except (subprocess.CalledProcessError, FileNotFoundError) as e:
            logger.error(f"Failed: Upgrading pip")
            if isinstance(e, subprocess.CalledProcessError):
                logger.error(f"Stderr: {e.stderr}")
        ...
        try:
            ...
            try:
                proc = subprocess.run(["python", "-m", "pip", "install", "poetry"], executable=str(python_exe), check=True, text=True, capture_output=True)
                if proc.stdout:
                    logger.debug(proc.stdout)
                if proc.stderr:
                    logger.warning(proc.stderr)
            except (subprocess.CalledProcessError, FileNotFoundError) as e:
                logger.error(f"Failed: Installing Poetry")
                if isinstance(e, subprocess.CalledProcessError):
                    logger.error(f"Stderr: {e.stderr}")
        ...
        try:
            ...
            try:
                proc = subprocess.run(["python", "-m", "pip", "install", "uv"], executable=str(python_exe), check=True, text=True, capture_output=True)
                if proc.stdout:
                    logger.debug(proc.stdout)
                if proc.stderr:
                    logger.warning(proc.stderr)
            except (subprocess.CalledProcessError, FileNotFoundError) as e:
                logger.error(f"Failed: Installing uv")
                if isinstance(e, subprocess.CalledProcessError):
                    logger.error(f"Stderr: {e.stderr}")
...
"""
    ...
    result = subprocess.run(
        ["python", "-c", nltk_download_script], executable=str(python_exe), cwd=ROOT_DIR, capture_output=True, text=True
    )
    if result.returncode != 0:
...
def check_uvicorn_installed() -> bool:
    ...
    try:
        result = subprocess.run(
            ["python", "-c", "import uvicorn"], executable=str(python_exe), capture_output=True, text=True
        )
        if result.returncode == 0:
