...
def run_command(cmd: List[str], description: str, **kwargs) -> bool:
    ...
    # Validate command path to prevent directory traversal
    if cmd and len(cmd) > 0:
        cmd_executable = cmd[0]
        if not validate_path_safety(cmd_executable):
            logger.error(f"Unsafe command executable: {cmd_executable}")
            return False
...
def install_notmuch_matching_system():
    try:
        ...
        # Sanitize the version to prevent command injection
        import re
...
def install_nodejs_dependencies(directory: str, update: bool = False) -> bool:
    ...
    if not validate_path_safety(str(directory_path), str(ROOT_DIR)):
        logger.error(f"Unsafe directory path: {directory_path}")
        return False
    ...
    return run_command(cmd, desc, cwd=directory_path, shell=(os.name == "nt"))
...
def start_node_service(service_path: Path, service_name: str, port: int, api_url: str):
    ...
    env["VITE_API_URL"] = api_url.replace('"', '').replace("'", "")
...
if __name__ == "__main__":
    main()
, major_minor):
            logger.error(f"Invalid notmuch version: {major_minor}")
            return
