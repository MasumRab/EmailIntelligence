...
# Add the project root to the Python path so we can import from deployment
# This allows launch.py to work when located in the setup subdirectory
...
def find_project_root() -> Path:
    ...
    # Since launch.py is now in the setup directory, we need to go up one level
    # to reach the project root where pyproject.toml is located
    current = Path(__file__).resolve().parent.parent  # Go up two levels from setup/
    return current
