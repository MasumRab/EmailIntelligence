...
def setup_dependencies(venv_path: Path, use_poetry: bool = False):
    ...
    if use_poetry:
        ...
        try:
            subprocess.run([python_exe, "-c", "import poetry"], check=True, capture_output=True, shell=False)
        ...
        try:
            subprocess.run([python_exe, "-c", "import uv"], check=True, capture_output=True, shell=False)
        except subprocess.CalledProcessError:
