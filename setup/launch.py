...
from setup.validation import (
    ...
    PYTHON_MIN_VERSION, PYTHON_MAX_VERSION  # Re-export for tests
)
...
# --- Setup Functions ---
# Mostly handled in setup.environment, but we need create_venv wrapper here for legacy args
# if not exported from setup.environment properly.
# Assuming handle_setup from setup.environment handles everything.
...
def download_nltk_data(venv_path=None):
    # This might be needed if setup.environment doesn't export it
    # But prepare_environment in setup.environment calls it internally?
    # Let's import it if needed, or rely on prepare_environment.
    # The legacy args call download_nltk_data(venv_path) directly sometimes.
    from setup.environment import download_nltk_data as _download_nltk_data
    _download_nltk_data(venv_path)

def create_venv(venv_path: Path, recreate: bool = False):
    from setup.environment import create_venv as _create_venv
    _create_venv(venv_path, recreate)

def setup_dependencies(venv_path: Path, use_poetry: bool = False):
    from setup.environment import setup_dependencies as _setup_dependencies
    _setup_dependencies(venv_path, use_poetry)

def install_nodejs_dependencies(directory: str, update: bool = False) -> bool:
    from setup.services import install_nodejs_dependencies as _install_nodejs_dependencies
    return _install_nodejs_dependencies(directory, update)
