...
| Symbol | Meaning |
|--------|---------|
| 🔴 **MUST** | Non-negotiable rule. Violation = rejection. |
| 🟡 **SHOULD** | Strong recommendation. Deviation requires justification. |
| 🟢 **MAY** / **OPTIONAL** | Nice to have. Left to engineer judgment. |

**Key Rules**:
- 🔴 **Agentic-first**: JSON output, explicit exit codes, structured errors
- 🔴 **TDD required**: tests before code (critical for AI-validated code)
- 🔴 **Constitution first**: commit before spec work
...
When implementing, prioritize MUST rules first, then SHOULD.
...
All CLI/TUI tools **MUST** be designed for AI agent consumption as the primary use case, with humans as secondary consumers.

**Rationale**: This project creates tools for autonomous agents. Rules in Sections I-XIII apply to ALL code. Additional rules in Section XII apply specifically to tools. If other rules conflict with agentic-first design, this takes precedence.
...
**Rationale**: AI-generated code accumulates technical debt faster than human-written code. Consistent standards (type hints, docstrings, linting) enable AI agents to understand interfaces and patterns. Without standards, each AI session re-invents conventions, creating inconsistency.
...
**Rationale**: AI agents can generate plausible but incorrect code. TDD validates AI outputs by defining expected behavior BEFORE generation. Tests serve as executable specifications that AI must satisfy. Without TDD, AI-generated code passes "looks right" but fails "works right".
...
**Rationale**: AI agents need clear, stable interfaces to understand how to use your code. API-first design makes your system "agent-discoverable" - agents can inspect APIs without reading implementation. This is essential for agentic tool-building.
...
### XII. Agentic-First Tool Design (NON-NEGOTIABLE)

**Scope**: This section applies to all CLI/TUI tools created in this project, including but not limited to: `dev.py`, `launch.py`, orchestration scripts, and any command-line utilities.

**Core Principle**: All tools MUST be designed with AI agents as primary consumers, with humans as secondary consumers. This ensures composability, automation, and reliable agentic workflows.

#### Agentic Tool Requirements

1. **Machine-Parseable Output**
   - Every command MUST support `--json` output for agent consumption
   - JSON output MUST be valid, parseable, and schema-consistent
   - Human-readable output (`--text` or default) is secondary

2. **Explicit Exit Codes**
   - `0` = success
   - `1` = general error
   - `2` = usage error (invalid arguments)
   - `3+` = specific error codes documented per command

3. **Structured Error Handling**
   - Errors MUST return JSON with: `{code, message, details, hint}`
   - Errors MUST NOT contain stack traces in default mode
   - `--verbose` flag enables debug output

4. **Composability**
   - Commands MUST be designed for piping between agents
   - Output of one command MUST be input-compatible with related commands
   - No interactive prompts in default mode; use `--interactive` flag

5. **Idempotency**
   - All commands MUST be safe to re-run without side effects
   - `--force` flag for non-idempotent operations when necessary

6. **Self-Documenting**
   - `--help` MUST provide complete usage for agent prompting
   - Examples MUST include both human and agent use cases
   - `--dry-run` MUST show what would happen without execution

#### How This Affects Specification Creation

Every Functional Requirement (FR) in spec.md MUST include:
- JSON output schema for the command
- Exit code definitions
- Error response format
- Example agent prompts

Every User Story MUST include:
- Agent interaction flow (how an agent invokes and parses output)
- Failure recovery path

### XIII. Constitution Governance (NON-NEGOTIABLE)

**The Constitution First Principle**: The Constitution MUST be committed to git BEFORE any spec work begins. When updating, change constitution before changing code. This ensures specs align with principles from the start.

**The Cascade**:
```
Constitution → Research → Spec → Plan → Tasks → Implementation
```
If Constitution doesn't exist when Research runs, specs may include architectural decisions that contradict standards—surfacing only during implementation when expensive to fix.

#### Constitution Quality Rules

1. **Use Accurate Terminology**
   - Only mention patterns YOUR project actually implements
   - Do NOT reference concepts your system doesn't use
   - Example: Don't say "use middleware for errors" if your project has no middleware

2. **Avoid External Standards References**
   - Do NOT say "follow RFC 9457 for error handling"
   - Your project may only follow PART of that standard
   - Be explicit: "Errors return {code, message} JSON"

3. **Avoid Vague Rules**
   - "Write clean code" → "No function > 40 lines"
   - "Be secure" → "Validate all inputs, no secrets in code"

#### Constitution Completeness Test

Before starting any spec work, verify:
- [ ] All rules are testable and specific
- [ ] All rules apply to your project type
- [ ] No external standard references without explicit local rules
- [ ] Terminology matches actual codebase patterns
- [ ] Version updated with amendment date
...
**Version**: 1.6.0 | **Ratified**: 2025-11-10 | **Last Amended**: 2026-03-15

### Amendment Log

- **v1.6.0 (2026-03-15)**: Major restructure. Added Quick Reference card, Rule Hierarchy (MUST/SHOULD), elevated Agentic-First to Principle 0, added Rationale explanations.
- **v1.5.0 (2026-03-15)**: Added Section XIII - Constitution Governance. Includes Constitution First Principle, The Cascade workflow, Quality Rules (accurate terminology, avoid external standards, avoid vague rules), and Constitution Completeness Test checklist.

- **v1.4.0 (2026-03-15)**: Added Section XII - Agentic-First Tool Design. All CLI/TUI tools must be designed for AI agent consumption with machine-parseable JSON output, explicit exit codes, and structured error handling.
