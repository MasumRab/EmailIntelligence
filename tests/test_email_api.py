...
def _create_mock_email(id: int, subject: str) -> dict:
...
def test_get_emails_all(client, mock_db_manager: AsyncMock):
    """Test retrieving all emails successfully."""
    mock_data = [_create_mock_email(1, "Subject 1"), _create_mock_email(2, "Subject 2")]
...
def test_get_emails_by_category(client, mock_db_manager: AsyncMock):
    """Test retrieving emails filtered by category."""
    mock_data = [_create_mock_email(1, "Category Email")]
...
def test_get_emails_by_search(client, mock_db_manager: AsyncMock):
    """Test retrieving emails via search."""
    mock_data = [_create_mock_email(1, "Search Result")]
...
def test_get_email_by_id_found(client, mock_db_manager: AsyncMock):
    """Test retrieving a single email by its ID when it exists."""
    mock_data = _create_mock_email(1, "Specific Email")
...
def test_get_email_by_id_not_found(client, mock_db_manager: AsyncMock):
...
def test_get_emails_db_error(client, mock_db_manager: AsyncMock):
...
@patch("server.python_backend.email_routes.filter_manager.apply_filters_to_email_data", new_callable=AsyncMock)
def test_create_email_success(mock_apply_filters, mock_analyze_email, client, mock_db_manager: AsyncMock):
    """Test successful creation of a new email."""
    email_data = {
        "sender": "test@example.com", "senderEmail": "test@example.com", "subject": "New Email",
        "content": "Content", "preview": "Preview", "time": "2023-10-26T10:00:00Z"
    }
    mock_ai_result = AIAnalysisResult(data={"category_id": 1, "confidence": 0.95, "suggested_labels": ["inbox"], "action_items": []})
    mock_analyze_email.return_value = mock_ai_result
    mock_apply_filters.return_value = {}
    mock_db_manager.create_email.return_value = _create_mock_email(100, "New Email")
...
def test_update_email_success(client, mock_db_manager: AsyncMock):
    """Test successful update of an existing email."""
    email_id = 1
    update_data = {"subject": "Updated Subject"}
    mock_db_manager.update_email.return_value = _create_mock_email(email_id, "Updated Subject")
...
def test_update_email_not_found(client, mock_db_manager: AsyncMock):
