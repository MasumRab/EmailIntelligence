...
async def test_create_email_and_category_count(db_manager: DatabaseManager):
    ...
    email_data = {
        "message_id": "test_msg_1", "thread_id": "thread_1", "subject": "Test Subject",
        "sender": "test@example.com", "sender_email": "test@example.com", "content": "This is a test email.",
        "preview": "This is a test email.", "time": "2025-01-01T12:00:00Z", "category_id": work_category["id"],
    }
...
async def test_write_behind_cache(db_manager: DatabaseManager):
    ...
    email_data = {
        "message_id": "test_cache_1", "subject": "Cache Test", "sender": "cache@test.com",
        "sender_email": "cache@test.com", "content": "Cache content", "preview": "Cache preview",
...
async def test_batch_email_creation(db_manager: DatabaseManager):
    ...
    emails_data = [
        {
            "message_id": f"batch_{i}", "subject": f"Batch {i}", "sender": "batch@test.com",
            "sender_email": "batch@test.com", "content": "Batch content", "preview": "Batch preview",
...
async def test_batch_email_update(db_manager: DatabaseManager):
    ...
    emails_data = [
        {
            "message_id": f"update_{i}", "subject": "Update", "sender": "update@test.com",
            "sender_email": "update@test.com", "content": "Update content", "preview": "Update preview",
            "time": "2025-01-05T12:00:00Z", "is_read": False
        }
        for i in range(3)
    ]
    await db_manager.create_emails_batch(emails_data)
    ...
    async with db_manager.get_cursor() as cur:
        await cur.execute("SELECT COUNT(*) FROM emails WHERE sender = ? AND is_read = 1", ("update@test.com",))
        count = await cur.fetchone()
        assert count[0] == 3
