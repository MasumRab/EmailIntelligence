...
async def test_search_emails_offloads_io(db_manager, tmp_path):
    ...
    with patch("asyncio.to_thread", side_effect=asyncio.to_thread) as mock_to_thread:
        ...
        for call in mock_to_thread.call_args_list:
            if call.args and getattr(call.args[0], "__name__", "") == "_scan_content_for_search":
                found_call = True
                break

        assert found_call, "asyncio.to_thread should have been called with _scan_content_for_search"
