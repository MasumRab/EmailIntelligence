...
class TestActionExtractionAPI(unittest.TestCase):
    ...
    def test_extract_actions_from_text_success(self, mock_analyze_email):
        ...
        mock_ai_result = AIAnalysisResult(data={
            "topic": "test", "sentiment": "neutral", "intent": "test", "urgency": "low",
            "confidence": 0.9, "categories": [], "keywords": [], "reasoning": "",
        ...
        self.assertEqual(response_data[0]['action_phrase'], "Please review the document by tomorrow")
    ...
    def test_extract_actions_missing_content(self):
        ...
        self.assertTrue(any("content" in error['loc'] for error in response_data['detail'] if 'loc' in error))
    ...
    def test_extract_actions_ai_engine_exception(self, mock_analyze_email):
        ...
        response_data = response.json()
        self.assertIn("detail", response_data)
        self.assertTrue("Failed to extract action items: AI Engine processing error" in response_data['detail'])
