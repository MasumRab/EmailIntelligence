...
# Skip entire module - ai_routes has been migrated to modules/
pytest.skip("ai_routes has been migrated to modules/", allow_module_level=True)
...
def mock_ai_engine():
    ...
    with patch(
        "backend.python_backend.ai_routes.ai_engine", new_callable=AsyncMock
    ) as mock_ae:
        yield mock_ae
...
def test_extract_actions_no_actions_found(client, mock_ai_engine):
    ...
    request_payload = {
        ...
        "content": "This is just a general update.",
    }
