...
class TestHookRecursionPrevention:
        ...
        if hook_path.exists():
            ...
            try:
                ...
                result = subprocess.run(
                    [str(hook_path)], capture_output=True, text=True
                )
    ...
    def test_worktree_subtree_detection(self):
        ...
        if hook_path.exists():
            ...
            assert (
                ".git" in content
            ), "Hook should check .git directory/file for setup detection"
            ...
            assert (
                "Consider using worktrees" in content
            ), "Should suggest worktrees for subtrees"
    ...
    def test_worktree_safe_operations(self):
        ...
        if hook_path.exists():
            ...
            if "IS_WORKTREE" in content and "git checkout" in content:
                ...
                assert (
                    'IS_WORKTREE" == true' in content
                ), "Should have worktree-specific logic"
                assert (
                ...
                ), "Should check for conflicts in subtrees"
                assert (
    ...
    def test_disable_flag_functionality(self):
        ...
        if hook_path.exists():
            ...
            assert (
                "DISABLE_ORCHESTRATION_CHECKS" in content
            ), "Should check for disable flag"
            assert (
                "Orchestration checks disabled" in content
            ), "Should provide disable message"
    ...
    def test_infinite_loop_prevention_on_branch_switch(self):
        ...
        if hook_path.exists():
            ...
            assert (
                "ORCHESTRATION_SYNC_ACTIVE" in content
            ), "Should check for sync active flag"

            # Should set the flag before doing git operations
            assert (
                "export ORCHESTRATION_SYNC_ACTIVE=1" in content
            ), "Should set sync active flag"
            ...
            early_check_content = "\n".join(early_checks)
            assert (
                "ORCHESTRATION_SYNC_ACTIVE" in early_check_content
    ...
    def test_git_checkout_operations_are_protected(self):
        ...
        if hook_path.exists():
            ...
            assert (
                flag_setting_line is not None
            ), "Should set ORCHESTRATION_SYNC_ACTIVE flag"
