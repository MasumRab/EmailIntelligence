...
from strategy.multi_phase_generator import (
    MultiPhaseStrategyGenerator, StrategyType, RiskCategory,
    MultiPhaseStrategy, EnhancementPreservation, RiskFactor
)
...
class TestMultiPhaseStrategyGenerator:
    ...
    async def test_single_strategy_generation(self, generator, sample_conflict_data, sample_context):
    ...
    async def test_enhancement_preservation_analysis(self, generator, sample_conflict_data, sample_context):
    ...
    async def test_risk_factor_generation(self, generator, sample_conflict_data, sample_context):
        ...
        risk_factors = generator._generate_risk_factors(
            sample_conflict_data, sample_context, StrategyType.CONSERVATIVE_MERGE, "medium"
        )
    ...
    async def test_execution_phases_generation(self, generator, sample_conflict_data, sample_context):
    ...
    async def test_hybrid_strategy_generation(self, generator, sample_conflict_data, sample_context):
        ...
        if len(strategies) >= 2:
            hybrid = generator._generate_hybrid_strategy(strategies, sample_conflict_data, sample_context)
    ...
    async def test_strategy_ranking(self, generator):
        ...
        strategy1 = MultiPhaseStrategy(
            id="strategy1", name="High Confidence", strategy_type=StrategyType.CONSERVATIVE_MERGE,
            approach="Test approach", steps=[], pros=["Pro 1"], cons=["Con 1"],
            confidence=0.9, estimated_time=30, risk_level="low",
            resource_requirements={}, requires_approval=False,
            success_criteria=[], rollback_strategy="",
            validation_approach="", enhancement_preservation=[], risk_factors=[],
            execution_phases=[]
        )
        
        strategy2 = MultiPhaseStrategy(
            id="strategy2", name="Low Confidence", strategy_type=StrategyType.FAST_TRACK,
            approach="Test approach", steps=[], pros=["Pro 2"], cons=["Con 2"],
            confidence=0.5, estimated_time=20, risk_level="high",
            resource_requirements={}, requires_approval=False,
            success_criteria=[], rollback_strategy="",
            validation_approach="", enhancement_preservation=[], risk_factors=[],
            execution_phases=[]
    ...
    async def test_risk_level_calculation(self, generator):
        ...
        low_risk = RiskFactor(
            id="low_risk", category=RiskCategory.TECHNICAL,
            description="Low risk factor", probability=0.1, impact=0.2,
            mitigation_strategy="Standard mitigation", residual_risk=0.05,
            owner="Tech Lead"
        )
        
        high_risk = RiskFactor(
            id="high_risk", category=RiskCategory.BUSINESS,
            description="High risk factor", probability=0.8, impact=0.9,
            mitigation_strategy="Comprehensive mitigation", residual_risk=0.6,
    ...
    async def test_parallel_execution_detection(self, generator, sample_conflict_data, sample_context):
    ...
    async def test_resource_requirement_generation(self, generator, sample_conflict_data, sample_context):
