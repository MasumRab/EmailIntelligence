...
from server.python_backend.models import FilterRequest, EmailFilterCriteria, EmailFilterActions
...
def mock_filter_manager():
    ...
    with patch("server.python_backend.filter_routes.filter_manager", new_callable=MagicMock) as mock_fm:
        yield mock_fm

@pytest.fixture
def mock_performance_monitor():
    """Fixture to mock the performance monitor."""
    with patch("server.python_backend.filter_routes.performance_monitor", new_callable=AsyncMock) as mock_pm:
...
def test_generate_intelligent_filters_success(client, mock_db_manager, mock_filter_manager, mock_performance_monitor):
