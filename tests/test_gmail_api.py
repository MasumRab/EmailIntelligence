...
from server.python_backend.main import app, get_db # Assuming get_db is for DatabaseManager, not directly used here but good to keep consistent
...
class TestGmailAPI(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_gmail_service_patch = patch('server.python_backend.main.gmail_service', autospec=True)
    ...
    def test_sync_gmail_success(self):
        ...
        request_data = {"maxEmails": 10, "queryFilter": "is:unread", "includeAIAnalysis": True, "strategies": []}
        mock_sync_result = {
            "success": True,
            "processed_count": 5,
    ...
    def test_sync_gmail_google_api_error_401(self):
        ...
        self.mock_gmail_service.sync_gmail_emails.side_effect = HttpError(resp=mock_resp, content=error_content)

        response = self.client.post("/api/gmail/sync", json=request_data)

        self.assertEqual(response.status_code, 401)
        data = response.json()
        self.assertEqual(data["detail"], "Gmail API authentication failed. Check credentials or token.")
    ...
    def test_sync_gmail_google_api_error_500(self):
        ...
        data = response.json()
        self.assertEqual(data["detail"], "Gmail API returned an unexpected error. Please try again later.")
        self.mock_performance_monitor.record_sync_performance.assert_not_called()
    ...
    def test_sync_gmail_generic_exception(self):
        ...
        data = response.json()
        self.assertTrue("Gmail sync failed due to an unexpected error: Some unexpected error" in data["detail"])
        self.mock_performance_monitor.record_sync_performance.assert_not_called()

    def test_smart_retrieval_success(self):
        print("Running test_smart_retrieval_success")
        request_data = {"strategies": ["urgent"], "maxApiCalls": 50, "timeBudgetMinutes": 10}
        mock_retrieval_result = {"status": "completed", "emails_found": 5, "details": "..."}
    ...
    def test_smart_retrieval_google_api_error(self):
        ...
        self.mock_gmail_service.execute_smart_retrieval.side_effect = HttpError(resp=mock_resp, content=error_content)

        response = self.client.post("/api/gmail/smart-retrieval", json=request_data)
        self.assertEqual(response.status_code, 403)
        self.assertEqual(response.json()["detail"], "Gmail API permission denied. Ensure API is enabled and scopes are correct.")

    def test_smart_retrieval_generic_exception(self):
        print("Running test_smart_retrieval_generic_exception")
        request_data = {"strategies": ["urgent"]}
        self.mock_gmail_service.execute_smart_retrieval.side_effect = Exception("Smart retrieval failed")

        response = self.client.post("/api/gmail/smart-retrieval", json=request_data)
        self.assertEqual(response.status_code, 500)
        self.assertIn("Smart retrieval failed due to an unexpected error: Smart retrieval failed", response.json()["detail"])
    ...
    def test_get_retrieval_strategies_exception(self):
        ...
        self.mock_gmail_service.get_retrieval_strategies.side_effect = Exception("Error fetching strategies")
    ...
    def test_get_gmail_performance_no_data(self):
        ...
        self.mock_gmail_service.get_performance_metrics.return_value = None # Or {} as per main.py logic

        response = self.client.get("/api/gmail/performance")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"status": "no_data"}) # main.py returns this if metrics is None or empty
        self.mock_gmail_service.get_performance_metrics.assert_called_once()

    def test_get_gmail_performance_exception(self):
        print("Running test_get_gmail_performance_exception")
        self.mock_gmail_service.get_performance_metrics.side_effect = Exception("Error fetching metrics")
