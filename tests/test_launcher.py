...
from setup.services import start_gradio_ui
...
class TestDependencyManagement:
    ...
    def test_setup_dependencies_success(self, mock_subprocess_run):
        ...
        with patch("setup.launch.get_python_executable", return_value="python"):
            with patch("setup.launch.install_notmuch_matching_system"):
...
class TestServiceStartup:
    ...
    def test_start_backend_success(self, mock_popen):
        from setup.services import start_backend
        """Test successful backend startup."""
        mock_process = MagicMock()
        mock_popen.return_value = mock_process

        venv_path = ROOT_DIR / "venv"
        with patch("setup.utils.ProcessManager.add_process") as mock_add_process:
...
class TestLauncherIntegration:
    ...
    def test_version_compatibility_matrix(self):
        ...
        for version_tuple, should_pass in test_cases:
            with patch("setup.validation.sys.version_info", version_tuple), patch("setup.launch.sys.version_info", version_tuple):
                if should_pass:
                    try:
                        check_python_version()
                    except SystemExit:
