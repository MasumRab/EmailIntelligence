...
def test_workflow_runner():
    ...
    nodes = {
        ...
        "node2": Node(
            node_id="node2", name="Add", operation=add, inputs=["a", "b"], outputs=["sum_result"]
        ),
    }
    ...
    workflow = Workflow(name="Test Workflow", nodes=nodes, connections={})

    # 2. Provide an initial context
    initial_context = {"text": "hello from the test", "a": 15, "b": 10}
