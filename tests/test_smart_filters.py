...
class TestSmartFilterManager(unittest.TestCase):
    ...
    def test_load_all_filters(self):
        ...
        filter1_data = EmailFilter(
            "f1", "Filter 1", "", {}, {}, 1, 0, datetime.now(), datetime.now(), 0, 0, {}
        )
        filter2_data = EmailFilter(
            "f2", "Filter 2", "", {}, {}, 1, 0, datetime.now(), datetime.now(), 0, 0, {}
        )
        ...
        )  # _load_all_filters (as per previous refactor) loads active only
    ...
    def test_apply_filter_to_email_exclusion(self):
        ...
        filter_criteria = {
            ...
            "exclude_patterns": ["newsletter", r"automated message"],
        }
        filter_obj = EmailFilter(
            "test_excl",
            "Exclusion Test",
    ...
    def test_create_intelligent_filters(self):
        ...
        if created_filters:
            ...
            loaded = self.manager._load_filter(
                a_filter_id
    ...
    def test_evaluate_filter_performance(self):
        ...
        filter_data = EmailFilter(
            filter_id="perf_test_001",
            name="Perf Test Filter",
            description="",
        ...
        test_emails_for_perf = [
            {
                "subject": "urgent call",
                "expected_filter_match": True,
            },  # TP: filter finds "urgent", actual is True
            {
                "subject": "another subject",
                "expected_filter_match": False,
            },  # TN: filter doesn't find "urgent", actual is False
            {
                "subject": "urgent meeting",
                "expected_filter_match": True,
            },  # TP: filter finds "urgent", actual is True
            {
                "subject": "not urgent but important",
                "expected_filter_match": True,
            },  # TP: filter finds "urgent", actual is True (Corrected expectation)
            {
                "subject": "something else",
                "expected_filter_match": True,
            },  # FN: filter doesn't find "urgent", actual is True
        ]

        performance = self.manager.evaluate_filter_performance(
            "perf_test_001", test_emails_for_perf
        )
    ...
    def test_prune_ineffective_filters_high_fp_rate(self):
        ...
        filter_data = EmailFilter(
            ...
            false_positive_rate=0.5,  # High FP
            created_date=datetime.now(),
            last_used=datetime.now(),
            usage_count=100,
            performance_metrics={},
    ...
    def test_prune_ineffective_filters_optimize(self, mock_get_performance):
        ...
        mock_perf_data = FilterPerformance(
            fid, 0.5, 0.5, 0.5, 0.5, 10, 100, 5, 5, 5
        )  # Low accuracy/f1
        ...
        with patch.object(
            self.manager, "_optimize_filter", side_effect=lambda f: f
        ) as mock_optimizer:
            results = self.manager.prune_ineffective_filters()
