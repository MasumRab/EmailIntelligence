...
# After completing sub-subtask 002.2.3
memory.add_work_log(
    action="Completed Task 002.2.3: Directory Similarity Metric",
    details="Jaccard similarity implemented, normalized to [0,1], test coverage 100%"
)
memory.update_todo("task_002_2_3", "completed")
memory.save_session()
```
...
After completing sub-subtask 002.2.8 (Unit Testing), optionally validate output format:

```bash
python scripts/compare_task_files.py \
  --validate src/analyzers/codebase_structure.py \
...
After completing Task 002.2, see what's next:

```bash
python scripts/next_task.py

# Output shows: Task 002.4 (BranchClusterer) ready (when 002.1 & 002.3 also done)
```
...
| compare_task_files.py | Output validation | After 002.2.8 | No |
