    ...
    try:
        ...
        if emails_data:
            ...
            for email in emails_data:
                ...
                label_ids_json = json.dumps(email.get("label_ids", [])) if email.get("label_ids") else "[]"
                to_addresses_json = json.dumps(email.get("to_addresses", [])) if email.get("to_addresses") else "[]"
                cc_addresses_json = json.dumps(email.get("cc_addresses", [])) if email.get("cc_addresses") else "[]"
                bcc_addresses_json = json.dumps(email.get("bcc_addresses", [])) if email.get("bcc_addresses") else "[]"
                references_json = json.dumps(email.get("references", [])) if email.get("references") else "[]"
                analysis_metadata_json = json.dumps(email.get("analysis_metadata", {})) if email.get("analysis_metadata") else "{}"
