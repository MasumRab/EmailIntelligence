...
class FictionalityDemo:
        ...
        return {
            ...
            "edge_cases": [
                ...
                },
                {
                    ...
                    "confidence": "UNCERTAIN",
                    "indicators": [
                        "Realistic scenario but lacks specifics",
                        "Common uncertainty",
                    ],
                },
            ],
    ...
    async def _demo_single_analysis(self):
        ...
        if result.success and result.analysis:
            ...
            if analysis.fictionality_indicators:
                print(
                    "   Fictionality Indicators: {}".format(
                        len(analysis.fictionality_indicators)
                    )
                )
            ...
            if analysis.realism_indicators:
                print(
                    "   Realism Indicators: {}".format(len(analysis.realism_indicators))
                )
            ...
            if analysis.strategy_adjustments:
                print(
                    "   Strategy Adjustments: {}".format(
                        len(analysis.strategy_adjustments)
                    )
                )
    ...
    async def _demo_realistic_vs_fictional(self):
        ...
        scenarios = [("Realistic", realistic, "🟢"), ("Fictional", fictional, "🔴")]
        ...
        for scenario_type, scenario, emoji in scenarios:
            ...
            if result.success and result.analysis:
                analysis = result.analysis
                print(
                    f"   Score: {analysis.fictionality_score:.3f} (Expected: {scenario['expected_score']})"
                )
                print(f"   Confidence: {analysis.confidence_level.value}")
    ...
    async def _demo_batch_processing(self):
        ...
        results = await self.analyzer.batch_analyze_fictionality(
            contexts, max_concurrent=2
        )
        ...
        scores = [
            r.analysis.fictionality_score for r in results if r.success and r.analysis
        ]
        if scores:
    ...
    async def _demo_filtering_analytics(self):
        ...
        try:
            ...
            if analyses:
                ...
                for range_name, count in distribution.items():
                    if count > 0:
                        ...
                        print(
                            "   {}: {} ({:.1f}%)".format(range_name, count, percentage)
                        )
            else:
            ...
            if metrics:
                ...
                print(
                    "   Avg Processing Time: {:.3f}s".format(
    ...
    async def _demo_performance_metrics(self):
        ...
        print(
            "   Total Processing Time: {:.2f}s".format(stats["total_processing_time"])
        )
        print(
            "   Average Processing Time: {:.3f}s".format(
    ...
    async def _demo_strategy_integration(self):
        ...
        for case_name, scenario in test_cases:
            ...
            if result.success and result.analysis:
                analysis = result.analysis
                print(
                    "   Fictionality Score: {:.3f}".format(analysis.fictionality_score)
                )
    ...
    async def _demo_interactive_analysis(self):
        ...
        print(
            "Enter your own content for fictionality analysis (or press Enter to skip):"
        )
        ...
        user_input = input(
            "Sample content: {}\nYour content (or press Enter): ".format(sample_content)
        ).strip()
    ...
    async def _demo_summary(self):
        ...
        print(
            "🔬 Fictionality Analyses Completed: {}".format(stats["analyses_completed"])
        )
        print("⏱️ Total Processing Time: {:.2f}s".format(stats["total_processing_time"]))
        print(
            "⚡ Average Analysis Time: {:.3f}s".format(stats["average_processing_time"])
        )
        print("💾 Cache Hit Rate: {:.2%}".format(stats["cache_hit_rate"]))
        ...
        print(
            "   4. Review integration examples in examples/fictionality_integration.py"
        )
