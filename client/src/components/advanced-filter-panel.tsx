...
export function AdvancedFilterPanel({ categories, onApplyFilters }: AdvancedFilterPanelProps) {
  ...
  const removeCondition = (id: string) => {
    if (customConditions.length > 1) {
      setCustomConditions(customConditions.filter(condition => condition.id !== id));
    }
  };
  ...
  const applyFilters = () => {
    ...
    if (requiredKeywords.trim()) {
      filters.required_keywords = requiredKeywords.split(',').map(k => k.trim()).filter(k => k);
    }
    if (excludedKeywords.trim()) {
      filters.excluded_keywords = excludedKeywords.split(',').map(k => k.trim()).filter(k => k);
    }
    
    // Sender-based filters
    if (requiredSenders.trim()) {
      filters.required_senders = requiredSenders.split(',').map(s => s.trim()).filter(s => s);
    }
    if (excludedSenders.trim()) {
      filters.excluded_senders = excludedSenders.split(',').map(s => s.trim()).filter(s => s);
    }
    
    // Recipient-based filters
    if (requiredRecipients.trim()) {
      filters.required_recipients = requiredRecipients.split(',').map(r => r.trim()).filter(r => r);
    }
    if (excludedRecipients.trim()) {
      filters.excluded_recipients = excludedRecipients.split(',').map(r => r.trim()).filter(r => r);
    }
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs text-muted-foreground">Required Keywords</Label>
              <Input
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs text-muted-foreground">Required Senders</Label>
              <Input
            ...
            <div>
              <Label className="text-xs text-muted-foreground">Excluded Senders</Label>
              <Input
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs text-muted-foreground">Required Recipients</Label>
              <Input
            ...
            <div>
              <Label className="text-xs text-muted-foreground">Excluded Recipients</Label>
              <Input
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs text-muted-foreground">Required Category</Label>
              <Select value={requiredCategory} onValueChange={setRequiredCategory}>
                <SelectTrigger>
            ...
            <div>
              <Label className="text-xs text-muted-foreground">Excluded Category</Label>
