...
class EmailIntelligenceCLI:
    ...
    def setup_resolution(
        ...
        try:
            ...
            conflicts = self._detect_conflicts(
                worktree_a_path, worktree_b_path, source_branch, target_branch
            )
    ...
    def _detect_conflicts(
        ...
        if not source_branch or not target_branch:
            raise ValueError(
                "Branch names are required for conflict detection. Please provide source and target branches."
            )
        ...
        try:
            ...
            for conflict in conflicts:
                ...
                c_dict = (
                    conflict.model_dump() if hasattr(conflict, "model_dump") else conflict.dict()
                )
...
"""
    ...
    def _display_compliance_result(self, result, filename: str):
        ...
        if result.violations:
            ...
            for violation in result.violations[:3]:
                ...
                print(
                    f"    {icon} [{violation.violation_type.value.upper()}] {violation.rule_name}"
                )
    ...
    def _save_constitutional_results(
        self, pr_number: int, all_results: List[Dict[str, Any]], metadata_file: Path
    ):
    ...
    def develop_spec_kit_strategy(
        ...
        alignment_rules: Optional[str] = None,
        interactive: bool = False,
    ) -> Dict[str, Any]:
        """
    ...
    def _interactive_strategy_development(
        ...
        while True:
            choice = (
                input("\nSelect action: (a)ccept strategy, (m)odify strategy, (q)uit: ")
                .lower()
                .strip()
            )
            if choice == "a":
    ...
    def _generate_spec_kit_strategy(
        ...
        for i, conflict_data in enumerate(conflicts[:5]):  # Limit to first 5 conflicts for demo
            file_name = Path(conflict_data.get("file", "unknown")).name
            ...
            # Reconstruct Conflict object for analysis
            try:
                # Handle blocks reconstruction
                blocks_data = conflict_data.get("blocks", [])
                blocks = []
                for b in blocks_data:
                    blocks.append(ConflictBlock(**b))

                # Handle enum conversions if necessary, or rely on pydantic coercion
                # If type/severity are strings in json, pydantic might handle them if they match enum values
                # But to be safe, we map them if we can, or just pass them and hope pydantic validates/coerces

                conflict_obj = Conflict(
                    id=conflict_data.get("id", f"conflict-{i}"),
                    type=conflict_data.get("type", ConflictTypeExtended.MERGE_CONFLICT),
                    severity=conflict_data.get("severity", RiskLevel.MEDIUM),
                    description=conflict_data.get("description", f"Conflict in {file_name}"),
                    file_paths=conflict_data.get(
                        "file_paths", [conflict_data.get("file", file_name)]
                    ),
                    blocks=blocks,
                ...
                alignment_score = analyzer.calculate_alignment_score(conflict_obj)

            except Exception as e:
                self._warn(f"Failed to calculate real alignment score for {file_name}: {e}")
                # Fallback to mock score if reconstruction fails
                alignment_score = (
                    0.75 + (int(hashlib.md5(file_name.encode()).hexdigest()[-2:], 16) % 25) / 100.0
                )

            step = {
                "step": i + 1,
                "file": conflict_data.get("file", file_name),
                "conflicts": len(conflict_data.get("blocks", [])) or 1,
