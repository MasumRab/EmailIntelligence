...
# Modular command system integration
from src.cli.commands.integration import setup_modular_cli, handle_modular_command
...
class EmailIntelligenceCLI:
    ...
    def __init__(self):
        ...
        self.constitutional_engine = ConstitutionalEngine(
            constitution_file=self.config.get("constitutional_framework", {}).get(
    ...
    def _conflict_to_template(
        self, conflict: Dict[str, Any], metadata: Dict[str, Any]
    ) -> str:
    ...
    def _display_compliance_result(self, result, filename: str):
        ...
        status_emoji = (
            "✅" if result.compliance_level.value in ["compliant", "excellent"] else "⚠️"
        )
        print(
            f"  {status_emoji} {filename}: {result.overall_score:.1%} ({result.compliance_level.value})"
        )
    ...
    def _save_constitutional_results(
        self, pr_number: int, results: List[Dict[str, Any]], metadata_file: Path
    ):
        ...
        metadata["constitutional_analysis"] = {
            ...
            "overall_score": sum(r["result"].overall_score for r in results)
            / len(results)
            if results
            else 0.0,
        }
    ...
    def _check_git_repository(self):
        ...
        try:
            ...
            self._error_exit(
                "Not in a git repository. Please run from a git repository root."
            )
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if not self.config_file.exists():
            ...
            self._info(
                "Created default configuration at ~/.emailintelligence/config.yaml"
            )
    ...
    def setup_resolution(
        ...
        try:
            ...
            conflicts = self._detect_conflicts_interface_based(
            ...
            metadata_file = (
                self.resolution_branches_dir / f"pr-{pr_number}-metadata.json"
            )
    ...
    def _detect_conflicts(
        self, worktree_a_path: Path, worktree_b_path: Path
    ) -> List[Dict[str, Any]]:
    ...
    def _detect_conflicts_interface_based(
        self, worktree_a_path: Path, worktree_b_path: Path
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self._info(
                f"🔍 Detected {len(conflict_list)} potential conflicts using new detector"
            )
    ...
    def analyze_constitutional(
        ...
        return asyncio.run(
            self._analyze_constitutional_async(
    ...
    async def _analyze_constitutional_async(
        ...
        print(
            f"Analyzing {len(conflicts_data)} conflicts against constitutional rules..."
        )
        ...
        if all_results:
            avg_score = sum(r["result"].compliance_score for r in all_results) / len(
                all_results
            )
        else:
            avg_score = 0.0
        ...
        return {
                ...
                "results": [
                    {
                        ...
                        if r["result"].compliance_score > 0.8
                        else "non_compliant",
            ...
            "critical_issues": [
                ...
                if "critical" in v.lower()
            ],
            "recommendations": [
                r for r in all_results for r in r["result"].recommendations
            ],
        }
    ...
    def _display_constitutional_analysis_result(self, result, filename: str):
        ...
        print(
            f"  {status_emoji} {filename}: {result.compliance_score:.1%} ({len(result.violations)} violations)"
        )

    def _display_constitutional_overall_summary(
        ...
        total_recommendations = sum(
            len(r["result"].recommendations) for r in all_results
        )
    ...
    def _load_constitutions(
        self, constitution_files: List[str]
    ) -> List[Dict[str, Any]]:
    ...
    def _perform_constitutional_analysis(
        ...
        if constitution_count > 0:
            analysis_results["overall_compliance"] = (
                total_compliance / constitution_count
            )
    ...
    def _assess_constitutional_compliance(
        ...
        for req in requirements:
            ...
            if requirement_type in ["MUST", "REQUIRED"]:
                ...
                if requirement_type == "MUST":
                    compliance_status = (
                        "CONFORMANT" if hash_digit > "2" else "NON_CONFORMANT"
                    )
                else:
                    compliance_status = (
                        "CONFORMANT" if hash_digit > "5" else "PARTIALLY_CONFORMANT"
                    )
            else:
                # SHOULD/SHOULD_NOT requirements have lower compliance impact
                compliance_status = (
                    "CONFORMANT" if hash_digit > "7" else "PARTIALLY_CONFORMANT"
                )
            ...
            if compliance_status == "CONFORMANT":
                ...
                assessment["partially_conformant_requirements"].append(
                    requirement_assessment
                )
                assessment["recommendations"].append(
        ...
        assessment["compliance_score"] = (
            conformant_count / len(requirements) if requirements else 1.0
        )
        ...
        for assessment in analysis_results["constitutional_assessments"]:
            ...
            if assessment["conformant_requirements"]:
                ...
                for req in assessment["conformant_requirements"]:
                    report_lines.append(
                        f"- ✅ **{req['name']}**: {req['type']} (CONFORMANT)"
                    )
                report_lines.append("")
            ...
            if assessment["non_conformant_requirements"]:
                ...
                for req in assessment["non_conformant_requirements"]:
                    report_lines.append(
                        f"- ❌ **{req['name']}**: {req['type']} (NON_CONFORMANT)"
                    )
                report_lines.append("")
            ...
            if assessment["partially_conformant_requirements"]:
                ...
                for req in assessment["partially_conformant_requirements"]:
                    report_lines.append(
                        f"- ⚠️ **{req['name']}**: {req['type']} (PARTIALLY CONFORMANT)"
                    )
                report_lines.append("")

        # Add recommendations
        if analysis_results.get("recommendations"):
            report_lines.extend(["## Resolution Recommendations", ""])
    ...
    def _display_interactive_analysis(self, report: str):
        ...
        while True:
            choice = (
                input("\nSelect action: (c)ontinue with recommendations, (q)uit: ")
                .lower()
                .strip()
            )
    ...
    def develop_spec_kit_strategy(
        ...
        if metadata.get("status") != "constitution_analyzed":
            self._warn(
                "Constitutional analysis not yet completed. Running analysis first..."
            )
        ...
        return {
            ...
            "enhancement_preservation": strategy.get(
    ...
    def _generate_spec_kit_strategy(
        ...
        for i, conflict in enumerate(
            conflicts[:5]
        ):  # Limit to first 5 conflicts for demo
            file_name = Path(conflict["file"]).name
            # Mock alignment score based on filename hash
            alignment_score = (
                int(hashlib.md5(file_name.encode()).hexdigest()[:2], 16) / 255
            )
            ...
            strategy_option = strategy_options[
                int(hashlib.md5(file_name.encode()).hexdigest()[-1], 16) % 4
            ]
    ...
    def _generate_strategy_report(
        self, pr_number: int, strategy: Dict[str, Any]
    ) -> str:
    ...
    def _interactive_strategy_development(
        self, pr_number: int, strategy_report: str
    ) -> Dict[str, Any]:
        ...
        while True:
            ...
            if choice in ["y", "yes"]:
                ...
                self._info(
                    "Strategy rejected. You can modify the approach and regenerate."
                )
    ...
    def align_content(
        ...
        if alignment_results["phase_results"]:
            avg_scores = [
                r.get("alignment_score", 0.0)
                for r in alignment_results["phase_results"]
            ]
            alignment_results["overall_alignment_score"] = sum(avg_scores) / len(
                avg_scores
            )
    ...
    def _execute_phase(
        self, phase: Dict[str, Any], metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if alignment_scores:
            phase_result["alignment_score"] = sum(alignment_scores) / len(
                alignment_scores
            )
            self._info(
                f"   📊 Alignment Score: {current_score} → {phase_result['alignment_score']:.0%}"
            )
    ...
    def _execute_phase_dry_run(
        ...
        return {
            ...
            "would_resolve_conflicts": sum(
                step.get("conflicts", 1) for step in phase.get("steps", [])
            ),
        }

    def _execute_phase_interactive(
        self, phase: Dict[str, Any], metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for step in phase.get("steps", []):
            ...
            while True:
                choice = (
                    input(f"   Apply changes for '{step_name}'? (y/n/s): ")
                    .lower()
                    .strip()
                )
                if choice in ["y", "yes"]:
                    ...
                    print(
                        "   Please enter 'y' to apply, 'n' to skip, or 's' to skip for now."
                    )
    ...
    def _display_alignment_results(self, results: Dict[str, Any]):
        ...
        self._info(
            f"   ├─ Overall Alignment Score: {results['overall_alignment_score']:.0%}"
        )
    ...
    def validate_resolution(
        ...
        if metadata.get("status") != "content_aligned":
            self._warn(
                "Content alignment not yet completed. Running alignment first..."
            )
        ...
        validation_results = self._perform_validation(
            metadata, validation_level, test_suites
        )
    ...
    def _perform_validation(
        ...
        if test_suites:
            ...
            checks = [
                check
                for check in checks
                if any(req in check for req in requested_suites)
            ]
    ...
    def _display_validation_results(self, results: Dict[str, Any]):
        ...
        for check in results["validation_checks"]:
            ...
            print(
                f"{status_emoji} {check['check'].replace('_', ' ').title()}: {check['status'].upper()}"
            )
        ...
        if results["overall_status"] == "passed":
            ...
            self._warn(
                "⚠️ Resolution validation completed with warnings - review recommended"
            )
        else:
            self._error(
                "❌ Resolution validation failed - manual intervention required"
            )
    ...
    async def auto_resolve_conflicts(
        ...
        if strategy_file and Path(strategy_file).exists():
            ...
            strategy = await self.strategy_generator.generate_resolution_strategy(
                conflicts
            )
            strategy_data = {
        ...
        resolution_results = await self.auto_resolver.execute_resolution(
            resolution_plan
        )
        ...
        metadata["status"] = (
            "auto_resolved" if resolution_results["success"] else "partial_resolution"
        )
    ...
    def _convert_metadata_to_conflicts(
        self, metadata_conflicts: List[Dict[str, Any]]
    ) -> List[Conflict]:
        ...
        for meta_conflict in metadata_conflicts:
            ...
            conflict = Conflict(
                ...
                resolution_strategy=meta_conflict.get(
                    "resolution_strategy", "standard_merge"
                ),
                estimated_resolution_time=meta_conflict.get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Command Modes:
  legacy     Use the legacy command system (original behavior)
  modular    Use the new modular command system (recommended)

Examples:
  # Legacy commands
  eai legacy setup-resolution --pr 123 --source-branch feature/auth --target-branch main

  # Modular commands (recommended)
  eai modular analyze /path/to/repo --base-branch main
  eai modular validate
  eai modular analyze-history --output report.txt

  # Legacy examples
  eai legacy analyze-constitutional --pr 123 --constitution ./constitutions/auth.yaml
  eai legacy develop-spec-kit-strategy --pr 123 --worktrees --interactive
  eai legacy align-content --pr 123 --interactive --checkpoint-each-step
  eai legacy validate-resolution --pr 123 --comprehensive
        """,
    )

    # Set up command mode selection
    ...
    legacy_subparsers = legacy_parser.add_subparsers(
        dest="command", help="Legacy commands"
    )
    ...
    # Set up modular commands using integration module
    from src.cli.commands.integration import add_modular_arguments

    add_modular_arguments(modular_subparsers)

    # Set up legacy commands
    subparsers = legacy_subparsers

    # Setup resolution command
    setup_parser = subparsers.add_parser(
        "setup-resolution", help="Setup resolution workspace"
    )
    setup_parser.add_argument(
        "--pr", type=int, required=True, help="Pull request number"
    )
    setup_parser.add_argument(
        "--source-branch", required=True, help="Source branch with conflicts"
    )
    setup_parser.add_argument(
        "--target-branch", required=True, help="Target branch for merging"
    )
    setup_parser.add_argument(
        "--constitution", action="append", help="Constitution file(s)"
    )
    setup_parser.add_argument("--spec", action="append", help="Specification file(s)")
    setup_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview setup without creating worktrees",
    )

    # Analyze constitutional command
    analyze_parser = subparsers.add_parser(
        "analyze-constitutional", help="Analyze conflicts against constitution"
    )
    ...
    analyze_parser.add_argument(
        "--constitution",
        action="append",
        help="Constitution file(s) to analyze against",
    )
    analyze_parser.add_argument(
        "--interactive", action="store_true", help="Enable interactive analysis mode"
    )

    # Develop strategy command
    strategy_parser = subparsers.add_parser(
        "develop-spec-kit-strategy", help="Develop spec-kit resolution strategy"
    )
    strategy_parser.add_argument(
        "--pr", type=int, required=True, help="Pull request number"
    )
    strategy_parser.add_argument(
        "--worktrees", action="store_true", help="Use worktree-based analysis"
    )
    strategy_parser.add_argument(
        "--alignment-rules", help="Path to alignment rules file"
    )
    strategy_parser.add_argument(
        "--interactive",
        action="store_true",
        help="Enable interactive strategy development",
    )
    strategy_parser.add_argument(
        "--review-required", action="store_true", help="Require team review"
    )

    # Align content command
    align_parser = subparsers.add_parser(
        "align-content", help="Execute content alignment"
    )
    ...
    align_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview alignment without applying changes",
    )
    align_parser.add_argument(
        "--preview-changes", action="store_true", help="Show preview of changes"
    )
    align_parser.add_argument(
        "--interactive",
        action="store_true",
        help="Interactive alignment with confirmation",
    )
    align_parser.add_argument(
        "--checkpoint-each-step", action="store_true", help="Checkpoint after each step"
    )

    # Validate resolution command
    validate_parser = subparsers.add_parser(
        "validate-resolution", help="Validate completed resolution"
    )
    ...
    validate_parser.add_argument(
        "--quick", action="store_true", help="Run quick validation check"
    )
    validate_parser.add_argument(
        "--tests", help="Specific test suites to run (comma-separated)"
    )

    # Auto-resolve command
    auto_resolve_parser = subparsers.add_parser(
        "auto-resolve", help="Automatically resolve conflicts"
    )
    ...
    # Handle modular commands first
    if hasattr(args, "command_mode") and args.command_mode == "modular":
        if hasattr(args, "modular_command") and args.modular_command:
            exit_code = handle_modular_command(args)
            sys.exit(exit_code)
        else:
            parser.print_help()
            return
    ...
    if hasattr(args, "command_mode") and args.command_mode == "legacy":
        if not hasattr(args, "command") or not args.command:
            parser.print_help()
            return
        command = args.command
    else:
        # Backward compatibility: if no command_mode, assume legacy
        if not hasattr(args, "command") or not args.command:
            parser.print_help()
            return
        command = args.command
    ...
    try:
        ...
        elif command == "analyze-constitutional":
        ...
        elif command == "develop-spec-kit-strategy":
        ...
        elif command == "validate-resolution":
